...
The monorepo uses GitHub Actions for continuous integration and deployment. All workflows are defined in `.github/workflows/`.
...
2. **Test**
   ...
   - Continues on error to collect all test results
...
4. **Changeset Validation** (PRs only)
   ...
   - Warns if missing (doesn't fail by default)
