...
export function setupConsoleFiltering(options: {
    ...
    const shouldFilterLogs = isDevelopment || (isProduction && enableInProduction && !process.env[disableProductionLogFiltering || 'DISABLE_PRODUCTION_LOG_FILTERING']);
    ...
    console.error = (...args: unknown[]) => {
        ...
        const shouldSuppress = CONSOLE_FILTER_PATTERNS.some(pattern => pattern.test(message));
        if (!shouldSuppress) {
