...
let cachedAdminModule: { getFirestoreAdmin: () => AdminFirestore } | null =
  null;
  ...
  return withFirestore(
    async (adminDb) => {
      const doc = await adminDb
        ...
        .get();
      return doc.exists
        ? { exists: true, data: () => doc.data(), id: doc.id }
        : null;
    },
    async (db) => {
      ...
      return docSnap.exists()
        ? { exists: true, data: () => docSnap.data(), id: docSnap.id }
        : null;
    },
  );
}
...
export async function getCollectionSnapshot(
  ...
  Array<{ data: () => Record<string, unknown> | undefined; id: string }>
> {
  return withFirestore(
    async (adminDb) => {
