...
import { handleFirebaseStorageError } from './errorHandler';
import { ApiError } from '../../logging';
...
export class ImageService {
  ...
  async getAvailableImages(directory: string = ''): Promise<string[]> {
    try {
      ...
      const apiError = handleFirebaseStorageError(error, 'getAvailableImages');
  ...
  async getImageUrlsWithPlaceholders(
    ...
    try {
      ...
      const apiError = handleFirebaseStorageError(error, 'getImageUrlsWithPlaceholders');
  ...
  async getImageCollection(
    ...
    try {
      ...
      const apiError = handleFirebaseStorageError(error, 'getImageCollection');
  ...
  async imageExists(imagePath: string): Promise<boolean> {
    try {
      ...
      const apiError = handleFirebaseStorageError(error, 'imageExists');
  ...
  async getImageMetadata(imagePath: string): Promise<ImageMetadata | null> {
    try {
      ...
      const apiError = handleFirebaseStorageError(error, 'getImageMetadata');
