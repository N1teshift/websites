...
export function createDataFetchHook<TData, TParams = void>(
  ...
  return function useDataFetch(params: TParams): DataFetchResult<TData> {
        ...
        ? async () => {
            try {
              ...
              if (
                ...
                return transform
                  ? transform(apiResponse.data)
                  : apiResponse.data;
              }

              // Assume response is already the data
              return transform
                ? transform(response as TData)
                : (response as TData);
            } catch (err) {
    ...
    const [nonSwrData, setNonSwrData] = useState<TData | null>(
      initialData ?? null,
    );
    const [nonSwrLoading, setNonSwrLoading] = useState<boolean>(
      isEnabled && !useSwrMode,
    );
    ...
    // Use a serialized version of params for dependency tracking to handle object params
    const paramsKey =
    ...
    useEffect(() => {
      ...
      // Include paramsKey to trigger refetch when params change (works for both primitives and objects)
      // fetchData is stable (memoized with params), but we track paramsKey to detect changes
      // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [useSwrMode, isEnabled, paramsKey, ...dependencies]);
