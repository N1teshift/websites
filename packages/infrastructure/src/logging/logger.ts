...
if (typeof global !== 'undefined') {
  ...
  const CLEANUP_KEY = Symbol.for('infrastructure.logger.cleanup.initialized');
  const CLEANUP_INTERVAL_KEY = Symbol.for('infrastructure.logger.cleanup.interval');
...
export function logError(
  ...
  // Send to error tracking if available (using dynamic import to avoid circular dependencies)
  if (typeof window !== 'undefined') {
    ...
    try {
      // Dynamic import to avoid circular dependencies
      import('../monitoring/errorTracking').then(({ captureError }) => {
        captureError(error, {
          ...context,
          category,
        });
