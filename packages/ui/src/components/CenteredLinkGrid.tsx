...
type DisplayItem =
    | { type: "leaf"; item: LeafLinkItem; key: string; depth: number; groupSize: number }
    | { type: "split"; item: SplitLinkItem; key: string; depth: number; toggleKey: string; isCollapsed?: boolean };
...
const CenteredLinkGrid: React.FC<CenteredLinkGridProps> = ({ links }) => {
    ...
    const handleSplitToggle = React.useCallback((splitKey: string) => {
        setOpenSplits((current) => (current.includes(splitKey) ? current.filter((key) => key !== splitKey) : [...current, splitKey]));
    }, []);

    const getChildSpanClasses = React.useCallback((groupSize: number | undefined) => {
        if (!groupSize || groupSize <= 3) return "col-span-1 md:col-span-1 lg:col-span-2";
    ...
    const flattenLinks = React.useCallback(
        (items: LinkItem[], parentKey?: string, depth = 0): DisplayItem[] => {
            ...
            return items.flatMap((link, index) => {
                ...
                const baseKey = link.id ?? (isSplitCard ? `split-${link.titleKey}` : `${(link as LeafLinkItem).href}-${link.titleKey}`);
                const linkKey = parentKey ? `${parentKey}::${baseKey}-${index}` : `${baseKey}-${index}`;
                if (isSplitCard) {
                    const splitItem = link as SplitLinkItem;
                    const isActive = openSplits.includes(linkKey);
                    if (isActive) {
                        const header: DisplayItem = { type: "split", item: splitItem, key: `${linkKey}::header`, depth, isCollapsed: true, toggleKey: linkKey };
                        return [header, ...flattenLinks(splitItem.splitLinks, linkKey, depth + 1)];
                    }
                    return [{ type: "split", item: splitItem, key: linkKey, depth, toggleKey: linkKey }];
                }
                return [{ type: "leaf", item: link as LeafLinkItem, key: linkKey, depth, groupSize: siblingCount }];
            });
        },
        [openSplits]
    );

    const displayItems = React.useMemo(() => flattenLinks(links), [links, flattenLinks]);
    return (
                    ...
                    {displayItems.map((displayItem) => {
                        ...
                        const wrapperShapeClass = isSplit
                            ...
                            : isNested
                                ? "min-h-[6.5rem]"
                                : "aspect-link-card";
                        const textSizeClass = isCollapsedSplit ? "text-sm" : isNested ? "text-base" : "text-xl";
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            <span className={`break-words ${isCollapsedSplit ? "tracking-wide uppercase text-sm" : ""}`}>
                                                {t(link.titleKey)}
                                            </span>
                                        </Card>
                                    </button>
