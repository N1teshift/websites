...
import {
  saveTestResults,
  fetchTestStats,
} from "@/features/modules/math/tests/services/testResultsService";
...
export class FirestoreService {
  ...
  async saveTestResults(summary: unknown, results: TestResultData<Record<string, unknown>>[]) {
    ...
    try {
      ...
      if (
        ...
        "passedTests" in summary &&
        "totalTests" in summary
      ) {
        const summaryObj = summary as Record<string, unknown>;
        this.logger.info(
          `📊 Summary: passed=${summaryObj.passedTests}/${summaryObj.totalTests}, Cost: ${summaryObj.estimatedCost}`
        );
      }

      if (results.length > 0) {
        this.logger.info(
          `📊 First result (sample): ${results[0]?.test?.name}, passed: ${results[0]?.passed}`
        );
      }
  ...
  async fetchTestStats() {
    ...
    try {
      ...
      this.logger.error(
        "Error fetching test statistics",
        error instanceof Error ? error : new Error(String(error))
      );
      throw error;
    }
   ...
   */
  private parseExerciseId(exerciseId: string): {
    localId: string;
    isValid: boolean;
    error?: string;
  } {
    ...
    try {
      ...
      if (!idParseResult.isValid) {
        this.logger.error(
          ...
          new Error(idParseResult.error || "Unknown ID parsing error"),
          {
            exerciseId,
      ...
      if (!exerciseDoc.exists) {
        this.logger.error(
          "Exercise document not found",
          new Error(`Exercise with ID ${localExerciseId} does not exist`),
          {
            localExerciseId,
            bookId,
            sectionId,
        ...
        if (!testsSnapshot.empty) {
          ...
          for (const testDoc of testsSnapshot.docs) {
            ...
            if (!historySnapshot.empty) {
              this.logger.info(
                `Deleting ${historySnapshot.size} history entries for test ${testId}`
              );
              const batch = firestore.batch();
              let batchCount = 0;
            ...
            batchPromises.push(
              firestore.collection("tests").doc(categoryId).collection("tests").doc(testId).delete()
            );
          }
        }
