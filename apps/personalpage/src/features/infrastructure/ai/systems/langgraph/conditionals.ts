 ...
 */
export function should_descend_or_complete(state: GraphStateType): "push_context_and_prepare_descend" | "complete_current_level" | "handle_error" {
    ...
    if (requiredSubType && subPrompts.length > 0) {
        ...
        logger.info(`Decision: Type ${state.currentObjectType} requires sub-type ${requiredSubType} and has ${subPrompts.length} prompts -> push_context_and_prepare_descend`);
        return "push_context_and_prepare_descend";
    } else {
        // Case 2: No sub-objects required or no prompts found. Complete this level.
        if (requiredSubType && subPrompts.length === 0) {
             logger.warn(`Warning: Type ${state.currentObjectType} requires sub-type ${requiredSubType}, but no sub-prompts were extracted. Proceeding to complete level.`);
        } else {
             logger.info(`Decision: Type ${state.currentObjectType} requires no sub-objects or no prompts found -> complete_current_level`);
        }
 ...
 */
export function should_continue_or_finalize(state: GraphStateType): "extract_settings" | "complete_current_level" | "finalize_object" | "handle_error" {
    ...
    // Priority 3: Parent Assembled (Stack MUST NOT be empty, settings exist, no prompt)
         ...
         logger.info("Decision: Parent assembled, stack not empty, process parent -> complete_current_level");
         return "complete_current_level"; // Loop back to process the assembled parent
    }
    // Error Case: Unexpected state
    else {
         logger.error(`Decision: Unexpected state -> handle_error. Stack Empty: ${stackIsEmpty}, Has Settings: ${hasCurrentSettings}, Has Type: ${hasCurrentObjectType}, Has Prompt: ${hasCurrentPrompt}`);
         return "handle_error";
    }
} 
