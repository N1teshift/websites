 ...
 */
import {
  MathInput, CoefficientsSettings, ObjectType, CoefficientSettings,
  DEFAULT_COEFFICIENT_SETTINGS, DEFAULT_COEFFICIENTS_SETTINGS,
  ExpressionSettings, EquationSettings, TermSettings
...
import { isMathInput, validateSettingsMatch, TestSettings, ResponseSettings } from '../../shared/validation/index';
...
export class MathObjectGenerator {
  ...
  async generate(
    userPrompt: string
  ): Promise<{objects: MathInput[]}> {

    this.logger.info(`Starting math object generation for prompt: ${userPrompt.substring(0, 50)}...`);
    if (this.testCase) {
        this.logger.info(`TestCase '${this.testCase.name}' provided; value validation will be performed.`);
    }
    ...
    try {
      ...
      const { objectType, context, usage: identificationUsage } = 
      ...
      const { objects } = 
        await this.generateObjectsForType(userPrompt, objectType, context, totalUsage);

      // Step 3: Validate the generated objects
      this.logger.info(`Initiating validation for ${objects.length} generated MathInput object(s)...`);
      ...
      this.logger.error(`Error in math object generation: ${error instanceof Error ? error.message : String(error)}`); 
      throw error;
    }
  }
    ...
    for (let i = 0; i < objects.length; i++) {
      ...
      try {
         // === Structural and Type Validation ===
          if (!isMathInput(obj)) {
             throw new Error(`Structural validation failed for ${objIndexInfo}. Check warnings for details.`);
          }
         ...
         if (obj.objectType === 'coefficient') {
              ...
              if (!isCoefficientSettingsCombinationValid(
                  settings.numberSet,
                  settings.rules,
                  settings.representationType,
                  settings.range as [number, number] | undefined
              )) {
                 throw new Error(`Mathematical consistency validation failed for ${objIndexInfo}: Invalid combination.`);
              }
              this.logger.debug(`Passed mathematical consistency validation for ${objIndexInfo}.`);
         }
         else {
              this.logger.debug(`Skipping mathematical consistency validation for ${objIndexInfo} (not yet implemented for type '${obj.objectType}').`);
         }

         // === Value Validation against Source TestCase ===
         if (this.testCase) {
             this.logger.debug(`Performing value validation against TestCase '${this.testCase.name}' for ${objIndexInfo}...`);
             ...
             const currentResponseSettings: ResponseSettings = {
                 ...
                 settings: (obj as Record<string, unknown>)[`${obj.objectType}Settings`] as AllPossibleSettings
             };

             // Perform the validation against the source TestCase settings
             const valueValidationError = validateSettingsMatch(sourceSettingsForValidation, currentResponseSettings);
             ...
             this.logger.debug(`Skipping value validation against TestCase for ${objIndexInfo} (no TestCase provided).`);
         }
  ...
  private async identifyObjectType(
    ...
    try {
      ...
      if (validationObjectType) {
        ...
        this.logger.error(`Type identification failed with validation type: ${validationObjectType}`); 
        throw new Error(`Failed to identify object type for prompt with validation type '${validationObjectType}': ${error instanceof Error ? error.message : String(error)}`);
      }
  ...
  private async extractSingleObjectSettings(
    ...
    this.logger.debug(`Attempting to extract single settings for type: ${objectType} with prompt: '${userPrompt}'...`);

    // --- Try Generating Default Settings First (Special Case) ---
    if (objectType === 'coefficients') {
        const defaultSettings = this._tryGenerateDefaultCoefficientsSettings(userPrompt, parentContext);
        if (defaultSettings) {
        ...
        this.logger.debug(`Could not generate default coefficients settings based on context. Proceeding with LLM extraction.`);
    }
    ...
    this.logger.debug(`Single settings extracted via LLM for ${objectType}: ${JSON.stringify(result.output.settings, null, 2)}`);
  ...
  private _tryGenerateDefaultCoefficientsSettings(
    ...
    const parentSettings = (typeof parentContext.parentSettings === 'object' && parentContext.parentSettings !== null)
    ...
    this.logger.debug(`Default Check: Prompt='${userPrompt}' (${isDefaultPrompt}), ParentType='${parentContext.parentType}' (${isParentTerm}), TermIDs=${termIds ? JSON.stringify(termIds) : 'undefined'}`);

    if (isDefaultPrompt && isParentTerm && termIds) {
      const termIdsLength = termIds.length;
      if (termIdsLength > 0) {
        this.logger.debug(`Using default coefficients settings based on parent term's termIds length: ${termIdsLength}`);
        ...
        this.logger.debug(`Parent term has empty termIds, cannot generate default coefficients based on length.`);
  ...
  private async processObjectAndSubObjects(
    ...
    this.logger.debug(`Sub-object config for ${objectType}: ${subObjectConfig ? `Type: ${requiredSubType}, Key: ${subObjectsSettingsKey}, IsArray: ${isArrayProperty}` : 'None'}`);

    // 2. Extract settings for the current object type itself
    const { settings: currentSettings, context: currentContext } = await this.extractSingleObjectSettings(
      userPrompt,
      objectType,
      parentContext,
      usage
    ...
    this.logger.debug(`Extracted settings for ${objectType}: ${JSON.stringify(currentSettings, null, 2)}`);
    ...
    if (subObjectConfig && requiredSubType && subObjectsSettingsKey) {
      this.logger.debug(`Handling required sub-type: ${requiredSubType} for ${objectType}. Settings key: '${subObjectsSettingsKey}'`);
      ...
      if (typeof parentSettingsObject === 'object' && parentSettingsObject !== null) {
         ...
         if (isArrayProperty && !(subObjectsSettingsKey in indexableSettings)) {
            ...
            this.logger.debug(`Initialized empty array for sub-object key '${subObjectsSettingsKey}' in ${objectType} settings.`);
         }
        ...
        this.logger.warn(`Settings object for ${objectType} not found after extraction. Cannot add sub-objects.`);
      ...
      if (currentContext && // Ensure context is not null/undefined
          ...
          Array.isArray(currentContext[extractorContextKey].subObjectPromptParts)) {
            ...
            this.logger.debug(`Found subObjectPromptParts in context.${extractorContextKey}: ${JSON.stringify(subPrompts)}`);
      } else {
        // Log if not found or if the structure is unexpected
        this.logger.debug(`No valid subObjectPromptParts array found under context.${extractorContextKey} for ${objectType}. Context received: ${JSON.stringify(currentContext)}`);
      }
      ...
      if (requiredSubType === 'coefficients' &&
          ...
          Array.isArray(subPrompts) && subPrompts.length === 1 && subPrompts[0] === 'default' && // Specifically check for a single "default" prompt
          ...
          (Array.isArray((parentSettingsObject as Record<string, unknown>)?.[subObjectsSettingsKey]) &&
           ((parentSettingsObject as Record<string, unknown>)[subObjectsSettingsKey] as unknown[]).length > 0) // Assert as unknown[] before checking length
          ) {
          skipSubObjectLoop = true;
          this.logger.debug(`Default coefficients appear to be pre-filled for key \'${subObjectsSettingsKey}\'. Skipping explicit sub-object extraction loop.`);
      }
      ...
      if (!skipSubObjectLoop && subPrompts && Array.isArray(subPrompts) && subPrompts.length > 0) {
        this.logger.debug(`Found ${subPrompts.length} sub-object prompts for ${subObjectsSettingsKey}. Extracting individually...`);
        ...
        if (!isArrayProperty && subPrompts.length > 1) {
             this.logger.warn(`Multiple subObjectPromptParts found for non-array property '${subObjectsSettingsKey}' of ${objectType}. Using only the first prompt.`);
        }
        ...
        if (isArrayProperty) {
           this.logger.debug(`Collected sub-object settings array for \'${subObjectsSettingsKey}\': ${JSON.stringify(indexableSettings?.[subObjectsSettingsKey], null, 2)}`);
        } else {
           this.logger.debug(`Assigned sub-object settings for \'${subObjectsSettingsKey}\': ${JSON.stringify(indexableSettings?.[subObjectsSettingsKey], null, 2)}`);
        }

      } else if (skipSubObjectLoop) {
         this.logger.debug(`Skipping sub-object extraction loop for \'${subObjectsSettingsKey}\' as default settings were applied or detected.`);
      } else {
         // Use parentSettingsObject directly here, as indexableSettings is out of scope
         this.logger.debug(`No valid subObjectPromptParts found or default handling applied for \'${subObjectsSettingsKey}\'. Property may be empty or pre-filled. Current value: ${JSON.stringify((parentSettingsObject as Record<string, unknown>)?.[subObjectsSettingsKey])}`);
         ...
         if (typeof parentSettingsObject === 'object' && parentSettingsObject !== null) {
            ...
            if (isArrayProperty && !(subObjectsSettingsKey in settingsObj)) {
               ...
               this.logger.debug(`Initialized empty array for \'${subObjectsSettingsKey}\' as fallback.`);
            }
         }
      ...
      if (typeof parentSettingsObject === 'object' && parentSettingsObject !== null && parentSettingsObject.hasOwnProperty('subObjectPromptParts')) {
      ...
      this.logger.debug(`Completed ${objectType} MathObject with sub-objects: ${JSON.stringify(mathObject, null, 2)}`);
    ...
    // else if (requiredSubType && !subObjectsSettingsKey) { ... }
      ...
      const currentSettingsObject = (mathObject as Record<string, unknown>)[`${objectType}Settings`];
      if (typeof currentSettingsObject === 'object' && currentSettingsObject !== null && currentSettingsObject.hasOwnProperty('subObjectPromptParts')) {
          // Use Record<string, unknown> for the cast
      ...
      allObjects.push(mathObject);
      this.logger.debug(`Completed base ${objectType} MathObject: ${JSON.stringify(mathObject, null, 2)}`);
    }
  }
} 
