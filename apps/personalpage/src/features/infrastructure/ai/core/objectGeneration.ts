...
import { GeneratedMathObjects } from "@/features/infrastructure/ai/types";
 ...
 */
async function generateSettingsUsingLangGraph(
  userPrompt: string
  ...
  try {
    ...
    logger.error("Error generating math objects using LangGraph", error instanceof Error ? error : new Error(String(error)));
 ...
 */
async function generateSettingsUsingLegacy(
  userPrompt: string
): Promise<GeneratedMathObjects> {
  ...
  try {
    ...
    const usage = getGlobalTokenUsage(); 
    logger.info("Legacy MathObjectGenerator generation successful. Returning objects and usage", { usage });
    ...
    logger.error("Error generating math objects using legacy MathObjectGenerator", error instanceof Error ? error : new Error(String(error)));
