...
export function validateBasicObjectStructure(
  ...
  if (typeof obj.objectType !== 'string' || !isValidObjectType(obj.objectType)) {
    return { message: `Object at index ${index} has invalid objectType: ${String(obj.objectType)}` };
  }
...
function validateCoefficientSettings(
  ...
  if (typeof settings.numberSet !== 'string') {
    return { message: `Coefficient object at index ${index} has missing or invalid numberSet (should be a string)` };
  }
  
  if (typeof settings.representationType !== 'string') {
    return { message: `Coefficient object at index ${index} has missing or invalid representationType (should be a string)` };
  }
  
  if (!Array.isArray(settings.range) || settings.range.length !== 2) {
    return { message: `Coefficient object at index ${index} has missing or invalid range (should be an array of 2 numbers)` };
  }
  
  if (!Array.isArray(settings.rules)) {
    return { message: `Coefficient object at index ${index} has missing or invalid rules (should be an array)` };
  }
...
function validateCoefficientsSettings(
  ...
  if (!Array.isArray(settings.coefficients)) {
    return { message: `Coefficients object at index ${index} has missing or invalid coefficients (should be an array)` };
  }
  
  if (typeof settings.collectionCount !== 'number') {
    return { message: `Coefficients object at index ${index} has missing or invalid collectionCount (should be a number)` };
  }
  
  if (!Array.isArray(settings.rules)) {
    return { message: `Coefficients object at index ${index} has missing or invalid rules (should be an array)` };
  }
  ...
  for (let i = 0; i < settings.coefficients.length; i++) {
    ...
    if (typeof coefficient !== 'object' || coefficient === null) {
      return { message: `Coefficient at position ${i} in coefficients object ${index} is not a valid object` };
    }
...
function validateTermSettings(
  ...
  if (!Array.isArray(settings.termIds)) {
    return { message: `Term object at index ${index} has missing or invalid termIds (should be an array)` };
  }
  
  if (typeof settings.coefficients !== 'object' || settings.coefficients === null) {
    return { message: `Term object at index ${index} has missing or invalid coefficients (should be an object)` };
  }
  
  if (!Array.isArray(settings.power) || settings.power.length !== 2) {
    return { message: `Term object at index ${index} has missing or invalid power (should be an array of 2 numbers)` };
  }
  
  if (typeof settings.powerOrder !== 'boolean') {
    return { message: `Term object at index ${index} has missing or invalid powerOrder (should be a boolean)` };
  }
  
  if (typeof settings.variableName !== 'string') {
    return { message: `Term object at index ${index} has missing or invalid variableName (should be a string)` };
  }
...
function validateTermsSettings(
  ...
  if (!Array.isArray(settings.terms)) {
    return { message: `Terms object at index ${index} has missing or invalid terms (should be an array)` };
  }
  
  if (settings.terms.length !== 2) {
    return { message: `Terms object at index ${index} must have exactly 2 term objects, found ${settings.terms.length}` };
  }
  
  if (typeof settings.combinationType !== 'string') {
    return { message: `Terms object at index ${index} has missing or invalid combinationType (should be a string)` };
  }
  
  if (!Array.isArray(settings.power) || settings.power.length !== 2) {
    return { message: `Terms object at index ${index} has missing or invalid power (should be an array of 2 numbers)` };
  }
  
  if (typeof settings.powerOrder !== 'boolean') {
    return { message: `Terms object at index ${index} has missing or invalid powerOrder (should be a boolean)` };
  }
  ...
  for (let termIdx = 0; termIdx < settings.terms.length; termIdx++) {
    ...
    if (typeof term !== 'object' || term === null) {
      return { message: `Term at position ${termIdx} in terms object ${index} is not a valid object` };
    }
    ...
    if ((term as Record<string, unknown>).objectType) {
      ...
      console.warn(`Warning: Term ${termIdx} in terms object ${index} should not have objectType property`);
    }
...
function validateExpressionSettings(
  ...
  if (!Array.isArray(settings.expressions)) {
    return { message: `Expression object at index ${index} has missing or invalid expressions (should be an array)` };
  }
  
  if (typeof settings.combinationType !== 'string') {
    return { message: `Expression object at index ${index} has missing or invalid combinationType (should be a string)` };
  }
  
  if (!Array.isArray(settings.power) || settings.power.length !== 2) {
    return { message: `Expression object at index ${index} has missing or invalid power (should be an array of 2 numbers)` };
  }
  
  if (typeof settings.powerOrder !== 'boolean') {
    return { message: `Expression object at index ${index} has missing or invalid powerOrder (should be a boolean)` };
  }
  ...
  for (let exprIdx = 0; exprIdx < settings.expressions.length; exprIdx++) {
    ...
    if (typeof expr !== 'object' || expr === null) {
      return { message: `Expression component at position ${exprIdx} in expression object ${index} is not a valid object` };
    }
    
    // Check for objectType (should not be present in nested expressions)
    if ((expr as Record<string, unknown>).objectType) {
      // Just a warning, not a critical error
      console.warn(`Warning: Expression component ${exprIdx} in expression object ${index} should not have objectType property`);
    }
    ...
    if (exprObj.terms && Array.isArray(exprObj.terms)) {
      ...
      return { message: `Unknown expression component type at position ${exprIdx} in expression object ${index}` };
    }
  }
...
function validateEquationSettings(
  ...
  // Validate properties with existence + type checking in one step
  if (!Array.isArray(settings.terms)) {
    return { message: `Equation object at index ${index} has missing or invalid terms (should be an array)` };
  }
  
  const termsLength = settings.terms.length;
  
  if (termsLength < 1 || termsLength > 2) {
    return { message: `Equation object at index ${index} should have 1 or 2 terms, found ${termsLength}` };
  }
  ...
  for (let termIdx = 0; termIdx < settings.terms.length; termIdx++) {
    ...
    if (typeof term !== 'object' || term === null) {
      return { message: `Term at position ${termIdx} in equation object ${index} is not a valid object` };
    }
 ...
 */
export function validateMathObjectSettings(obj: Record<string, unknown>, index: number): ValidationError | null {
  ...
  const validators: Record<ObjectType, (settings: Record<string, unknown> | undefined, index: number) => ValidationError | null> = {
