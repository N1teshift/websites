...
import { BookOpen, Users, User, Target, BarChart3, MessageSquare, Database, LucideIcon } from 'lucide-react';
...
const ProgressReportPage: React.FC = React.memo(() => {
    ...
    const handleDataChange = useCallback((updatedStudents: StudentData[]) => {
        if (!data) return;
    ...
    const handlePendingEditsChange = useCallback((hasPendingEdits: boolean) => {
        setUnsavedChanges(hasPendingEdits);
    }, [setUnsavedChanges]);

    const handleSaveGrades = useCallback((grades: Map<string, number>) => {
        if (!data) return;
                        ...
                        <div className="text-center py-12">
                            <div className="max-w-2xl mx-auto">
                                ...
                                <p className="text-gray-600 mb-4">
                                    The Objectives feature is currently only available for the main teacher&apos;s data (Math assessments).
                                </p>
                                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-left">
                                    <p className="text-blue-900 mb-2">
                                        <strong>Current Data:</strong> {data.metadata?.teacher_name || `Teacher ${objectivesTeacherType}`}
                                    </p>
                                    <p className="text-blue-800">
                                        This feature is specifically designed for Cambridge Learning Objectives tracking used in the main Math curriculum. 
                                        For English assessment data, objectives are tracked differently within the English curriculum.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    return (
                        <div className="text-center py-12">
                            <div className="max-w-2xl mx-auto">
                                ...
                                <p className="text-gray-600 mb-4">
                                    The Grade Generator is currently only available for the main teacher&apos;s data (Math assessments).
                                </p>
                                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-left">
                                    <p className="text-blue-900 mb-2">
                                        <strong>Current Data:</strong> {data.metadata?.teacher_name || `Teacher ${teacherType}`}
                                    </p>
                                    <p className="text-blue-800">
                                        This feature is specifically designed for SD1, SD2, SD3 test calculations used in the main Math curriculum. 
                                        For English assessment data, please use the Class View to see all test scores and percentages.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="sm:hidden border-b border-gray-200 bg-gradient-to-b from-white to-gray-50/50">
                        <div className="px-4 py-3">
                            <div className="relative">
                                <select
                                    value={activeSection}
                                    onChange={(e) => handleSectionChange(e.target.value as ProgressReportActiveSection)}
