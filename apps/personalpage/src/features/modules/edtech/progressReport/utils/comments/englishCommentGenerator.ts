...
export function generateEnglishDiagnosticComment(
  ...
  if (weakestArea && weakestArea.percent < 70) {
    comment +=
      " " +
      template.sections.weakIntro +
      " " +
      weakestArea.section.toLowerCase() +
      " " +
      template.sections.weakEnding;
  }
...
export function generateEnglishUnit1Comment(
  ...
  comment = comment.replace(
    /{Total_Score}/g,
    t1TotalScore !== null ? t1TotalScore.toString() : "—"
  );
  comment = comment.replace(
    /{Total_Percent}/g,
    t1TotalPercent !== null ? Math.round(t1TotalPercent).toString() : "—"
  );
