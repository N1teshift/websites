...
import { 
    getPDForObjective
} from '@progressReport/student-data/config/pdKdMappings';
import { 
...
export function createMissionCandidate(student: StudentData): StudentMissionCandidate | null {
    ...
    const missingPoints = unmasteredObjectives.reduce(
        (sum, obj) => sum + obj.missing_points, 
        0
    );

    const lastAssessed = unmasteredObjectives
...
export function groupCandidatesByPriority(
    ...
    return {
        critical: candidates.filter(c => c.missing_points >= 5).sort((a, b) => b.missing_points - a.missing_points),
        moderate: candidates.filter(c => c.missing_points >= 3 && c.missing_points < 5).sort((a, b) => b.missing_points - a.missing_points),
        minor: candidates.filter(c => c.missing_points < 3).sort((a, b) => b.missing_points - a.missing_points)
    };
}
...
export function shouldAutoComplete(mission: CambridgeMission): boolean {
    ...
    return Object.values(mission.objectives).every(obj => 
 ...
 */
export function addMissionToStudent(
    student: StudentData,
    mission: CambridgeMission
): StudentData {
    return {
