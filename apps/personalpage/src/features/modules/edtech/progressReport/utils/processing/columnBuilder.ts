  ...
  return students.some((student) => {
    ...
    if (scoreType) {
      if (scoreType === "percentage") {
        if (
          assessment.evaluation_details?.percentage_score !== undefined &&
...
function hasEnglishTestFields(
  students: StudentData[],
  assessmentId: string
): { isDiagnostic: boolean; isSummative: boolean } {
  for (const student of students) {
    const assessment = getLatestAssessmentById(student, assessmentId);
    if (assessment) {
      ...
      if (
        ...
        assessment.gr2 !== undefined ||
        assessment.gr3 !== undefined
      ) {
...
export function buildAssessmentColumns(
  ...
  assessments.forEach((assessment) => {
    const shortTitle =
      assessment.type === "sav_darb" ? "P1" : shortenColumnTitle(assessment.title, assessment.id);
    ...
    if (englishTest.isDiagnostic && assessment.type === "diagnostic") {
      ...
      const isHiddenExtColumn =
        assessment.title === "Experimental Classwork Data" ||
