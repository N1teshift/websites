...
export function calculateGradeFromSum(
  sum: number | null,
  maxPointsFor10: number = 16
): number | null {
  if (sum === null) {
    return null;
  }
...
export function calculateOptimalMaxPoints(
  ...
  for (let maxPts = 10; maxPts <= maxAvailablePoints; maxPts += 1) {
    const grades = studentsWithAllTests
      .map((student) => {
        const testScores = getTestScores(student, sdGroup);
        const sum = calculateTestSum(testScores);
