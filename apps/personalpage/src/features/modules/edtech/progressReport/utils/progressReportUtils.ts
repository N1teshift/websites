...
export const calculateStudentStats = (student: StudentData): StudentStats => {
  ...
  const latestCambridgeTest =
    cambridgeTests.length > 0 ? cambridgeTests[cambridgeTests.length - 1] : undefined;
...
export const filterAssessmentsByType = (assessments: Assessment[], type: string): Assessment[] => {
