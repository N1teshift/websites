...
export function getCellColorClass(
  ...
  const isDiagnostic =
    columnLabel.match(/^D\d+$/i) !== null || assessmentId.toLowerCase().includes("diagnostic");
  if (isDiagnostic) {
  ...
  const isBinaryND =
    rules.binaryND.includes(columnLabel) ||
  ...
  const isCambridgeSD =
    rules.cambridgeSD.includes(columnLabel) || columnLabel.match(/^SD\d+C\d?$/) !== null;
  if (isCambridgeSD) {
  ...
  const isMYPSD = columnLabel.toUpperCase().includes("MYP") || rules.mypSD.includes(columnLabel);
  if (isMYPSD) {
