...
import type { AllScoreTypes, EnglishTestScoreType } from '../../components/common/ClassPerformanceChartEnhanced';
...
export function calculateAssessmentStatistics(
    ...
    const supportsMultipleScores = selectedAssessment.type === 'test' || selectedAssessment.type === 'summative' || selectedAssessment.type === 'diagnostic';
    ...
    if (isEnglishTestScoreType) {
        ...
        statValue = completionData.length > 0
        ...
        const scores = students
            .map(s => {
                ...
                if (assessment.evaluation_details) {
                    ...
                    if (chartScoreType === 'percentage') detailScore = assessment.evaluation_details.percentage_score;
                    else if (chartScoreType === 'myp') detailScore = assessment.evaluation_details.myp_score;
                    else if (chartScoreType === 'cambridge' || chartScoreType === 'cambridge_1' || chartScoreType === 'cambridge_2') {
    ...
    if (isEnglishTestScoreType) {
        ...
        } else if (chartScoreType === 'paper1' || chartScoreType === 'paper2' || chartScoreType === 'paper3') {
            assessmentLabel = 'Average Score (0-50)';
        } else if (['lis1', 'lis2', 'read', 'voc1', 'voc2', 'gr1', 'gr2', 'gr3'].includes(chartScoreType as string)) {
