...
export const getLatestKDAssessment = (student: StudentData): Assessment | null => {
    ...
    const kdAssessments = student.assessments.filter(a => 
        a.column?.toUpperCase().includes('KD') || 
        a.type === 'summative'
    );
...
export const getLatestND1Assessment = (student: StudentData): Assessment | null => {
    ...
    const nd1Assessments = student.assessments.filter(a => 
        a.column?.toUpperCase() === 'ND1'
    );
...
export const calculateND1CompletionRate = (student: StudentData): number => {
    ...
    const avgCompletion = completionValues.reduce((sum, val) => sum + val, 0) / completionValues.length;
