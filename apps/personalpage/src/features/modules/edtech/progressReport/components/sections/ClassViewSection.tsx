...
import ClassPerformanceChartEnhanced, { ChartMode, AllScoreTypes } from '../common/ClassPerformanceChartEnhanced';
...
import { createStudentSortComparators, parseAssessmentColumnId } from '../../utils/processing/studentSortComparators';
...
const ClassViewSection: React.FC<ClassViewSectionProps> = ({
    ...
    const availableAssessments = useMemo(() => {
        const assessments = getUniqueAssessments(students, ['summative', 'homework', 'homework_graded', 'homework_reflection', 'test', 'diagnostic']);
    ...
    const { columns, visibleColumns, setColumns, updateColumns } = useColumnVisibility(initialColumns, {
                ...
                comparators[column.id] = (a: StudentData, b: StudentData) => {
                    ...
                    const getScore = (assessment: Assessment): number => {
                        if (scoreType && assessment.evaluation_details) {
                            ...
                            if (scoreType === 'percentage') detailScore = assessment.evaluation_details.percentage_score;
                            else if (scoreType === 'myp') detailScore = assessment.evaluation_details.myp_score;
                            else if (scoreType === 'cambridge') detailScore = assessment.evaluation_details.cambridge_score;
    ...
    const statCards = selectedClass && classStats ? [
        { label: t('students'), value: classStats.studentCount, color: 'blue' as const },
        {
            label: classStats.assessmentLabel,
            value: classStats.isPercentage ? formatPercentage(classStats.assessmentValue) : classStats.assessmentValue,
                    ...
                    <div className="bg-white border border-gray-200 rounded-lg p-6">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="flex items-center gap-3">
                                <ColumnCategoryShortcuts columns={columns} onChange={setColumns} students={students} />
