...
const CommentsGeneratorSection: React.FC<CommentsGeneratorSectionProps> = ({
  ...
  const templates = useMemo(() => {
    ...
    if (isEnglishTeacher) {
      ...
      return allTemplates.filter(
        (t) => t.id === "english-diagnostic-1" || t.id === "english-unit-1"
      );
    } else {
      // Main teachers: only show Math templates
      return allTemplates.filter((t) => t.id === "default-unit1");
    }
  ...
  // Generate comments
  const generatedComments = useGeneratedComments(studentCommentData, activeTemplate, regenerateKey);
