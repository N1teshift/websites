...
const parseExerciseScore = (
  score: string
): { exerciseNumber: number | null; difficultyPath: "A" | "B" | "C" | null; isValid: boolean } => {
...
const extractLearningJourneys = (
  ...
  return filteredStudents.map((student) => {
    ...
    const progress: ExerciseProgress[] = extColumns.map(({ column, date, lesson }) => {
      const assessment = student.assessments?.find((a) => a.column === column && a.date === date);
...
const LearningJourneyTimeline: React.FC<LearningJourneyTimelineProps> = ({
  students,
  selectedClass,
}) => {
  ...
  const getDifficultyColor = (difficulty: "A" | "B" | "C" | null): string => {
    ...
    switch (difficulty) {
      case "A":
        return "#22c55e"; // green for easy
      case "B":
        return "#3b82f6"; // blue for normal
      case "C":
        return "#ef4444"; // red for challenging
      default:
        return "#94a3b8";
    }
  };
  ...
  const journeysWithData = journeys.filter((j) =>
    j.progress.some((p) => p.exerciseNumber !== null)
  );
  const journeysWithoutData = journeys.filter(
    (j) => !j.progress.some((p) => p.exerciseNumber !== null)
  );

  // Collect students with unparseable data
  const journeysWithUnparseableData = journeys.filter(
    (j) => j.unparseableData && j.unparseableData.length > 0
  );
    ...
    <div className="space-y-4">
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className="text-sm font-medium text-yellow-800">
                Data Quality Warning: {journeysWithUnparseableData.length} student(s) with
                unparseable EXT data
              </h3>
              <div className="mt-2 text-sm text-yellow-700">
                <p className="mb-2">
                  The following students have EXT data that could not be parsed (expected format:
                  number + optional A/B/C):
                </p>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 overflow-x-auto">
        <svg width="100%" height={svgHeight} className="min-w-[800px]">
          ...
          {/* Main timeline axis */}
          <line x1="10%" y1={timelineY} x2="90%" y2={timelineY} stroke="#d1d5db" strokeWidth="2" />
          ...
          <g key="marker-123">
            <line
              ...
              strokeWidth="2"
            />
            <text x="90%" y={timelineY - 12} textAnchor="middle" fontSize="12" fill="#6b7280">
              123
            </text>
          </g>
          ...
          {journeys.map((journey, index) => {
            ...
            return (
              <g key={journey.student.id}>
                ...
                {hasNoData && (
                  <text x="12%" y={laneY + 5} fontSize="10" fill="#9ca3af" fontStyle="italic">
                    (No EXT data)
                  </text>
                )}
