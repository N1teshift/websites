...
import {
  buildAllTimelineEvents,
  TimelineEvent,
} from "../../utils/processing/timelineEventsBuilder";
...
const ActivityTimelineChart: React.FC<ActivityTimelineChartProps> = ({
  ...
  const chartData = useMemo(() => {
    if (isActivityMode) {
      ...
      const activityByDate = new Map<
        string,
        { date: string; displayDate: string; count: number; events: TimelineEvent[] }
      >();
  ...
  const ActivityTooltip = ({
    ...
    active?: boolean;
    payload?: { payload: ActivityChartData }[];
  }) => {
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            {data.events?.map((event: TimelineEvent, idx: number) => (
              <div
                ...
                style={{ borderColor: typeColors[event.type] || typeColors.default }}
              >
                <p className="font-medium text-gray-900">{event.details.title}</p>
                <p className="text-gray-600">{event.details.description}</p>
                {event.details.score && (
                  <p className="text-gray-800 font-semibold">Score: {event.details.score}</p>
                )}
                {event.details.comment && (
                  <p className="text-gray-500 italic">{event.details.comment}</p>
                )}
              </div>
            ))}
          </div>
  ...
  const ScoreTooltip = ({
    ...
    active?: boolean;
    payload?: { payload: ScoreChartData }[];
  }) => {
    if (active && payload && payload.length) {
        ...
        {isActivityMode ? (
          ...
          <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <Line
              ...
              strokeWidth={2}
              dot={{
                fill: typeColors[selectedTypes[0]] || "#3B82F6",
                r: 5,
                strokeWidth: 2,
                stroke: "#ffffff",
              }}
