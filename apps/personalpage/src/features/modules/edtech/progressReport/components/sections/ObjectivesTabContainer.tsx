...
export const ObjectivesTabContainer: React.FC<ObjectivesTabContainerProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<"mission-creator" | "my-missions" | "test-completion">(
    "mission-creator"
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="px-6">
        {activeTab === "mission-creator" && (
          <MissionCreator students={students} onStudentUpdate={handleStudentUpdate} />
        )}
        {activeTab === "my-missions" && (
          <MyMissions students={students} onStudentUpdate={handleStudentUpdate} />
        )}
        {activeTab === "test-completion" && (
          <ObjectivesSection students={students} onDataChange={onDataChange} />
        )}
      </div>
    </div>
  );
