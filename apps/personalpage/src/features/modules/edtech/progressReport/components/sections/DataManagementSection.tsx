...
const DataManagementSection: React.FC<DataManagementSectionProps> = ({
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-4">{t("data_management")}</h2>
        <p className="text-gray-600 mb-6">{t("upload_instruction")}</p>
      </div>
      ...
      <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-lg p-6">
        ...
        <p className="text-sm text-gray-600 mb-4">
          Upload your{" "}
          <code className="bg-white px-1.5 py-0.5 rounded text-green-700">raw_data.xlsx</code> file
          to add new assessments from EXT, LNT, SD, ND, and KONS columns. The updated data will{" "}
          <strong>automatically sync</strong> to your dashboard!
        </p>
        <ExcelFileUpload onSuccess={handleExcelSuccess} buttonText="Upload & Process Excel" />
        <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded text-xs text-blue-800">
          <strong>✨ New:</strong> After processing Excel, your dashboard automatically reloads with
          the updated data. No manual export needed!
        </div>
      </div>
          ...
          <div className="space-y-2">
              ...
              <div className="mt-3 p-3 bg-orange-50 border border-orange-200 rounded">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-orange-800 font-medium text-sm">
                    Pending cell edits not saved
                  </span>
                </div>
                <p className="text-xs text-orange-700 mt-1">
                  You have pending cell edits in the Class View. Click &quot;Save Changes&quot;
            ...
            {data.snapshot_metadata && (
              <div className="text-xs text-gray-500 mt-2 space-y-1">
                ...
                <div>
                  Total Assessments: {data.snapshot_metadata.data_statistics.total_assessments}
                </div>
                <div>
                  Total Cambridge Tests:{" "}
                  {data.snapshot_metadata.data_statistics.total_cambridge_tests}
                </div>
              </div>
            )}
          </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-3">ℹ️ Data Management Workflow</h3>
        <div className="text-gray-600 text-sm space-y-3">
          <div>
            <strong className="text-gray-900">1. Initial Load:</strong> Upload your student data
            JSON file to view existing records
          </div>
          <div>
            <strong className="text-gray-900">2. Process Excel:</strong> Upload raw_data.xlsx to add
            new assessments (updates individual student files)
          </div>
          <div>
            <strong className="text-gray-900">3. Export Updated Data:</strong> Download the updated
            JSON collection to backup and view new data
          </div>
          <div className="pt-2 border-t border-blue-200">
            <strong className="text-gray-900">Note:</strong> The dashboard shows data from the
            loaded JSON. After processing Excel, export and re-upload the JSON to see the updated
            assessments.
          </div>
        </div>
      </div>
