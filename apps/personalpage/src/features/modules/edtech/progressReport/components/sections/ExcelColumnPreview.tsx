...
const ExcelColumnPreview: React.FC<ExcelColumnPreviewProps> = ({
    sheets,
    onConfirm,
    onCancel
}) => {
    ...
    const allColumns = useMemo(() => {
        return sheets.flatMap(sheet => 
            sheet.columns.map(col => col.columnName)
        );
    }, [sheets]);
        ...
        <div className="space-y-4">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <p className="text-sm text-gray-600 mb-3">
                    Found <strong>{totalColumns} columns</strong> across <strong>{sheets.length} sheets</strong>.
                    Select which columns you want to import into the database.
                </p>
            ...
            <div className="flex flex-wrap gap-2">
                {Array.from(columnsByType.keys()).map(type => {
                    ...
                    const selectedOfType = typeColumns.filter(col => selectedColumns.has(col.columnName)).length;
                                        ...
                                        <label className="flex items-start gap-3 cursor-pointer">
                                            ...
                                            <div className="flex-1 space-y-1">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className={`text-xs px-2 py-0.5 rounded border ${getTypeColor(column.type)}`}>
                                                        {column.type}
                                                    </span>
                                                    {!column.hasData && (
                                                ...
                                                {column.dateFound && (
                                                    <div className="text-xs text-gray-500">
                                                        📅 Date: {column.dateFound}
                                                    </div>
                                                )}
                                                
                                                {column.sampleValues.length > 0 && column.hasData && (
                                                    <div className="text-xs text-gray-500">
                                                        Sample: {column.sampleValues.filter(v => v !== null).join(', ') || 'Empty'}
                                                    </div>
                                                )}
                                            </div>
                                        </label>
