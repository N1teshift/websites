...
const GradeGeneratorSection: React.FC<GradeGeneratorSectionProps> = ({
    ...
    // Sort students
    const { sortedData: classStudents, handleSort, getSortIcon } = useTableSorting({
    ...
    const handleSaveGrades = () => {
        ...
        if (grades.size === 0) {
            alert(`No grades to save. Make sure students have all three test scores (${selectedSDGroup}).`);
            return;
        }

        if (confirm(`Save calculated grades for ${grades.size} student(s) with threshold ${maxPointsFor10} points?`)) {
            onSaveGrades(grades);
        }
    };
    ...
    const stats = useMemo(() => {
        ...
        return {
            ...
            average: Math.round((validGrades.reduce((sum, g) => sum + g, 0) / validGrades.length) * 10) / 10,
    ...
    const sdLabels = selectedSDGroup === 'SD4+SD5+SD6' ? ['SD4', 'SD5', 'SD6'] : ['SD1', 'SD2', 'SD3'];
        ...
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-purple-50 to-blue-50 border-2 border-purple-200 rounded-lg p-6">
                ...
                <p className="text-sm text-purple-600 mt-2">
                    Current Formula: (Total Points / <span className="font-bold text-purple-900">{maxPointsFor10}</span>) × 10, rounded up (max 10)
                </p>
                <p className="text-xs text-purple-500 mt-1">
                    Max Available: <span className="font-bold">{MAX_AVAILABLE_POINTS}</span> points ({breakdownText})
                </p>
            </div>
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-amber-900 mb-2">
                                Points for Grade 10: <span className="text-xl font-bold text-amber-700">{maxPointsFor10}</span>
                            </label>
                            ...
                            <div className="flex justify-between text-xs text-amber-600 mt-1">
                                <span>10 pts<br/>(generous)</span>
                                <span>16 pts<br/>(50%)</span>
                                <span>24 pts<br/>(75%)</span>
                                <span>{MAX_AVAILABLE_POINTS} pts<br/>(max)</span>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                step="1"
                                value={maxPointsFor10}
                                onChange={(e) => setMaxPointsFor10(Math.round(Math.max(10, Math.min(MAX_AVAILABLE_POINTS, parseFloat(e.target.value) || 16))))}
                            ...
                            <p className="text-xs text-amber-600 mt-1">
                                Range: 10-{MAX_AVAILABLE_POINTS} points
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                step="0.1"
                                value={targetAverage}
                                onChange={(e) => setTargetAverage(Math.max(1, Math.min(10, parseFloat(e.target.value) || 7)))}
                                className="w-full px-4 py-2 border-2 border-amber-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                            />
                        </div>
                        ...
                        <div className={`p-4 rounded-lg border-2 ${
                            averageColor === 'green' ? 'bg-green-50 border-green-300' :
                            averageColor === 'orange' ? 'bg-orange-50 border-orange-300' :
                            'bg-red-50 border-red-300'
                        }`}>
                            <div className="text-sm font-medium mb-1">
                                Current Average: <span className={`text-2xl font-bold ${
                                    averageColor === 'green' ? 'text-green-700' :
                                    averageColor === 'orange' ? 'text-orange-700' :
                                    'text-red-700'
                                }`}>{stats.average}</span>
                            </div>
                            <div className="text-xs">
                                {isCloseToTarget ? (
                                    ...
                                    <span className={averageColor === 'orange' ? 'text-orange-700' : 'text-red-700'}>
                                        {stats.average < targetAverage ? '📉' : '📈'} {averageDiff.toFixed(2)} away from target
                                    </span>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1">
                            ...
                            <p className="text-sm text-red-700 mb-3">
                                The following students cannot have grades calculated because they are missing test scores:
                            </p>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                                {studentsWithMissingData.map((item, idx) => (
                                    <div key={idx} className="bg-white border border-red-300 rounded px-3 py-2 text-sm">
            ...
            {selectedClass && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-orange-50 border-2 border-orange-200 rounded-lg p-4">
                        ...
                        <div className="text-2xl font-bold text-orange-900">{stats.min} - {stats.max}</div>
                    </div>
                </div>
            )}
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {classStudents.length > 0 ? (
                                classStudents.map(student => {
                                    const { sum, grade, testScores } = calculateGrade(student, maxPointsFor10, selectedSDGroup);
                                    
                                    return (
                                        <tr key={`${student.first_name}-${student.last_name}`} className="hover:bg-purple-50">
