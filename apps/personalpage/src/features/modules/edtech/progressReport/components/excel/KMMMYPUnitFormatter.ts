...
import {
  ...
  AssessmentTask,
  ATLCard,
} from "../../../unitPlanGenerator/types/UnitPlanTypes";
import {
  formatArrayAsString,
  formatArrayAsNewlines,
  formatATLSkillsAsNewlines,
  formatAcademicYear,
  formatCommandTermsAsString,
} from "./ExcelFormattingUtils";
...
function formatATLCardsForExcel(cards: ATLCard[]): string {
  ...
  return cards
    .map((card, _index) => {
      return (
        `Category Cluster: ${card.categoryCluster}\n` +
...
export const applyKMMMYPUnitFormatting = async (
  ...
  const whiteCells = [
    ...
    { row: 9, col: 4 }, // C9, D9
    { row: 10, col: 5 },
    { row: 10, col: 6 }, // E10, F10
    { row: 11, col: 3 },
    { row: 11, col: 4 }, // C11, D11
  ];
  ...
  for (let col = 2; col <= 7; col++) {
    // B=2, G=7
    if (col !== 4) {
      // Skip D=4 (D18) as it's already set to white above
  ...
  individualCell.value = formatArrayAsNewlines(
    unitPlan.individualContext ? [unitPlan.individualContext] : []
  );
  ...
  globalLensCell.value = formatArrayAsNewlines(
    unitPlan.globalContextLens ? [unitPlan.globalContextLens] : []
  );
  ...
  differentiationHeader.value =
    "Differentiation and Inclusion Strategies (content, process,product, examples)";
  ...
  if (unitPlan.subunits && unitPlan.subunits.length > 0) {
    unitPlan.subunits.forEach((subunit, index) => {
      ...
      const lessonRowCells = [
        ...
        `H${rowNumber}`,
        `L${rowNumber}`,
        `P${rowNumber}`, // Assessment cells (merged, so only need the first cell of each merge)
      ];
    ...
    const cellsWithBorders = [
      ...
      `F${instructionsRow + 6}`,
      `B${instructionsRow + 1}`,
      `E${instructionsRow + 1}`, // B39, E39 (resources and community engagement value cells)
    ];
