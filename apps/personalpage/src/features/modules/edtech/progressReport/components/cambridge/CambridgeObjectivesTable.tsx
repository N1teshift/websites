...
const CambridgeObjectivesTable: React.FC<CambridgeObjectivesTableProps> = ({ students }) => {
  const [selectedStrands, setSelectedStrands] = useState<Set<string>>(
    new Set(Object.keys(CAMBRIDGE_OBJECTIVE_STRANDS))
  );
  ...
  const filteredObjectives = useMemo(() => {
    ...
    selectedStrands.forEach((strandName) => {
      const objectives =
        CAMBRIDGE_OBJECTIVE_STRANDS[strandName as keyof typeof CAMBRIDGE_OBJECTIVE_STRANDS];
      if (objectives) {
  ...
  const calculateMastery = useCallback(
    (student: StudentData) => {
      ...
      const masteredCount = relevantObjectives.filter(
        (code) => objectives[code]?.current_score === 1
      ).length;
  ...
  const checkForNewerData = (
    ...
    const assessmentId =
      newestAssessment.assessment_id?.toUpperCase() || newestAssessment.column?.toUpperCase();
    const scoreField = getCambridgeScoreField(assessmentId || "", objectiveCode);
  ...
  if (students.length === 0) {
    return <div className="text-center py-8 text-gray-500">No students to display</div>;
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-6 text-sm bg-blue-50 p-3 rounded-lg">
        <div className="flex items-center gap-2">
          <div className="w-8 h-6 bg-green-100 border border-green-300 rounded flex items-center justify-center text-green-800 font-medium">
            1
          </div>
          <span className="text-gray-700">Mastered</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-8 h-6 bg-yellow-100 border border-yellow-300 rounded flex items-center justify-center text-yellow-800 font-medium">
            0.5
          </div>
          <span className="text-gray-700">Partial</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-8 h-6 bg-red-100 border border-red-300 rounded flex items-center justify-center text-red-800 font-medium">
            0
          </div>
          <span className="text-gray-700">Not mastered</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-8 h-6 bg-gray-100 border border-gray-300 rounded flex items-center justify-center text-gray-400 font-medium">
            —
          </div>
            ...
            {sortedStudents.map((student, idx) => {
              ...
              return (
                <tr key={student.id || idx} className="hover:bg-gray-50">
                  ...
                  {filteredObjectives.map((objectiveCode) => {
                    const objective =
                      student.curriculum_progress?.cambridge_objectives?.[objectiveCode];
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`inline-flex items-center justify-center w-8 h-6 rounded ${getScoreColor(score)} font-medium relative`}
                        >
      ...
      <div className="bg-gray-50 p-4 rounded-lg text-sm text-gray-600">
        <p>
          Showing <strong>{filteredObjectives.length}</strong> objectives for{" "}
          <strong>{sortedStudents.length}</strong> students
          {selectedStrands.size > 0 &&
            selectedStrands.size < Object.keys(CAMBRIDGE_OBJECTIVE_STRANDS).length &&
