  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">📋 PD Assessment Mappings</h2>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="text-3xl font-bold text-blue-600">{mappingsArray.length}</div>
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
          <div className="text-3xl font-bold text-purple-600">
            {(
              mappingsArray.reduce((sum, m) => sum + m.objectiveCount, 0) / mappingsArray.length
            ).toFixed(1)}
          </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="p-4 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">PD Mappings</h3>
        </div>
            ...
            <div key={pdNumber} className="hover:bg-gray-50">
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="font-mono font-semibold text-blue-600 text-lg">{pdNumber}</div>
                ...
                <div className="px-4 pb-4 bg-gray-50">
                  <div className="space-y-2">
                    {objectives.map((objective) => (
                      ...
                      >
                        <span className="font-mono text-gray-900 font-medium">{objective}</span>
                        <span className="text-xs text-gray-500">Cambridge Objective</span>
                      </div>
                    ))}
                  </div>
                  <div className="mt-3 p-3 bg-blue-50 rounded border border-blue-200">
                    <p className="text-sm text-blue-900">
                      <strong>Usage:</strong> When students complete {pdNumber} assessment, their
                      scores will automatically update{" "}
                      {objectiveCount === 1 ? "this objective" : "these objectives"} and any related
                      missions.
                    </p>
                  </div>
                </div>
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        <h4 className="font-semibold text-yellow-900 mb-2">📌 About PD Mappings</h4>
        <ul className="text-sm text-yellow-800 space-y-1">
          <li>
            • PD mappings are <strong>global</strong> - each PD number tests the same objectives for
            all students
          </li>
          <li>
            • Mappings are defined in{" "}
            <code className="bg-yellow-100 px-1 rounded">pdKdMappings.ts</code>
          </li>
          <li>
            • When Excel data is imported with PD columns, objectives and missions auto-update
          </li>
          <li>
            • Format in Excel: <code className="bg-yellow-100 px-1 rounded">PD3_2025-10-21</code>,{" "}
            <code className="bg-yellow-100 px-1 rounded">PD3 P_2025-10-21</code>, etc.
          </li>
        </ul>
      </div>
    </div>
