...
export const StudentMissionCard: React.FC<StudentMissionCardProps> = ({
  ...
  const objectivesByStrand = candidate.unmastered_objectives.reduce(
    (acc, obj) => {
      if (!acc[obj.strand]) {
        acc[obj.strand] = [];
      }
  ...
  return (
    <div className="border border-gray-300 rounded-lg p-4 bg-white shadow-sm hover:shadow-md transition-shadow">
      ...
      {candidate.last_assessed && (
        <div className="text-xs text-gray-500 mb-3">Last assessed: {candidate.last_assessed}</div>
      )}
      ...
      <div className="space-y-3 mb-4">
        {Object.entries(objectivesByStrand).map(([strand, objectives]) => (
          <div key={strand}>
            <div className="text-sm font-semibold text-gray-700 mb-1">{strand}</div>
            <div className="space-y-1">
              {objectives.map((obj) => (
                ...
                >
                  ...
                  <span className="text-xs text-blue-600 font-medium">{obj.pd_assessment}</span>
