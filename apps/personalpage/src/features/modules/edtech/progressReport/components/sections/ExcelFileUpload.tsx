            ...
            if (data.success && data.sheets) {
                Logger.info('Preview generated', { 
                    ...
                    sheetDetails: data.sheets.map(s => ({ 
                        ...
                        sdColumns: s.columns.filter(c => c.columnName.startsWith('SD')).map(c => c.columnName)
                    }))
                });
                                ...
                                {result.success && (
                                    <div className="text-sm text-gray-700 space-y-1">
                                        {result.studentsUpdated !== undefined && (
                                            <div>👥 Students updated: <span className="font-semibold">{result.studentsUpdated}</span></div>
                                        )}
                                        {result.assessmentsAdded !== undefined && (
                                            <div>📝 Assessments added: <span className="font-semibold">{result.assessmentsAdded}</span></div>
                                        )}
                                        {result.newStudents !== undefined && result.newStudents > 0 && (
                                            <div>✨ New students: <span className="font-semibold">{result.newStudents}</span></div>
                                        )}
                                        {!!result.updatedData && (
