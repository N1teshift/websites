...
export const CreateMissionDialog: React.FC<CreateMissionDialogProps> = ({
  ...
  const byStrand = objectivesData.reduce(
    (acc, obj) => {
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="space-y-3">
              {Object.entries(byStrand).map(([strand, objectives]) => (
                <div key={strand}>
                  <div className="text-xs font-semibold text-gray-600 mb-1">{strand}</div>
                  <div className="space-y-1">
                    {objectives.map((obj) => (
                      <div key={obj.code} className="flex items-center gap-2 text-sm">
                        ...
                        <span className="text-xs text-blue-600">{obj.pd_assessment}</span>
                        <span
            ...
            <div className="mt-4 pt-3 border-t border-gray-200 flex justify-between text-sm">
              ...
              <span className="font-semibold text-red-600">{totalMissing.toFixed(1)}</span>
            </div>
          </div>
        </div>
