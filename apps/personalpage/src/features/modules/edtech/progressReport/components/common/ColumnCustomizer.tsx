  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {t("columns").charAt(0).toUpperCase() + t("columns").slice(1)} ({visibleCount}/
        {columns.length})
      </button>
...
export const ColumnCategoryShortcuts: React.FC<ColumnCategoryShortcutsProps> = ({
  ...
  const categorizedColumns = useMemo(() => {
    ...
    columns.forEach((col) => {
      ...
      if (
        ...
        assessmentType === "homework_graded" ||
        assessmentType === "homework_reflection"
      ) {
  ...
  const categoryVisibility = useMemo(() => {
    const getVisibilityState = (categoryColumns: string[]) => {
      ...
      const visibleCount = categoryColumns.filter(
        (colId) => columns.find((col) => col.id === colId)?.visible
      ).length;
