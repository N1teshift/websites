...
export type EnglishTestScoreType = 'lis1' | 'lis2' | 'read' | 'voc1' | 'voc2' | 'gr1' | 'gr2' | 'gr3' | 'total_percent' | 
    'paper1' | 'paper2' | 'paper3' | 'paper1_percent' | 'paper2_percent' | 'paper3_percent';

export type AllScoreTypes = 'percentage' | 'myp' | 'cambridge' | 'cambridge_1' | 'cambridge_2' | EnglishTestScoreType;
...
const ClassPerformanceChartEnhanced: React.FC<ClassPerformanceChartEnhancedProps> = ({
    ...
    const availableAssessments = useMemo(() => {
        return getUniqueAssessments(students, ['summative', 'homework', 'homework_graded', 'test', 'diagnostic']);
    }, [students]);
    
    // Build chart options
    const chartOptions = providedChartOptions || availableAssessments.map(a => ({
        id: a.id,
        title: a.title,
        type: 'assessment'
    }));
    ...
    const supportsMultipleScores = currentAssessmentType === 'test' || 
    ...
    // Build X-axis label
    const getXAxisLabel = () => {
        if (currentAssessmentType === 'homework' && (!homeworkHasScores || homeworkViewMode === 'completion')) {
            return t('completion');
        }
