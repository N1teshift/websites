  ...
  return (
    <div className="space-y-6">
      {comments.map((result, index) => (
        <div
          key={result.student.id || `${result.student.first_name}-${result.student.last_name}`}
            ...
            <div>
              ...
              {isMathTemplate && (
                <div className="flex flex-wrap gap-4 mt-2 text-sm text-gray-600">
                  ...
                  <span className="font-medium">MYP Level: {result.mypLevel}</span>
                  {result.p1 !== null && (
                    <>
                      <span>P1: {result.p1}</span>
                      <span className="font-medium">MYP-Based Grade: {result.mypBasedGrade}</span>
              ...
              {result.weakSections.length > 0 && (
                <div className="mt-2">
                  <span className="text-sm text-amber-700 bg-amber-50 px-2 py-1 rounded-md">
                    ...
                    {result.weakSections
                      .map(
                        (w) => `${w.section} (${Math.round(w.score)}${isMathTemplate ? "" : "%"})`
                      )
                      .join(", ")}
                  </span>
                </div>
