...
const colorClasses: Record<
  CardColor,
  { bg: string; border: string; text: string; valueText: string }
> = {
  blue: {
    bg: "bg-blue-50",
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <div className={`text-sm ${colors.text} font-medium capitalize`}>{card.label}</div>
              {card.icon && <span className="text-2xl">{card.icon}</span>}
            </div>
            <div className={`text-3xl font-bold ${colors.valueText}`}>{card.value}</div>
            {card.subtitle && <div className={`text-xs ${colors.text} mt-1`}>{card.subtitle}</div>}
          </div>
        );
      })}
    </div>
