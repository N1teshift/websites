...
export function useAssessmentColumns(
    ...
    return useMemo(() => {
        const cols: ColumnConfig[] = [
            { id: 'name', label: translationLabel, visible: true }
        ];
        
        // Build assessment columns (includes diagnostic and summative English tests)
        const assessmentCols = buildAssessmentColumns(
            students,
            ['summative', 'homework', 'homework_graded', 'homework_reflection', 'test', 'classwork', 'sav_darb', 'board_solving', 'consultation', 'diagnostic']
        );
