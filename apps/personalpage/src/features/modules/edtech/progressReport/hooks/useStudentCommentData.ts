...
import { generateEnglishDiagnosticComment, generateEnglishUnit1Comment } from '../utils/comments/englishCommentGenerator';
...
export function useStudentCommentData(
    ...
    const studentCommentData = useMemo((): StudentCommentData[] => {
        return students.map(student => {
            ...
            const mathValues = [
                mathData.sd1p, mathData.sd1myp, mathData.sd1c1, mathData.sd1c2,
                mathData.sd2p, mathData.sd2myp, mathData.sd2c,
                mathData.sd3p, mathData.sd3myp, mathData.sd3c
            ];
            const d1Values = [
                diagnosticData.d1Paper1Percent, diagnosticData.d1Paper2Percent,
                diagnosticData.d1Paper3Percent, diagnosticData.d1TotalPercent
            ];
            const t1Values = [
                unit1Data.t1Lis, unit1Data.t1Read, unit1Data.t1Voc,
                unit1Data.t1Gr, unit1Data.t1TotalScore, unit1Data.t1TotalPercent
            ];
        ...
        return studentCommentData
            ...
            .map(data => {
                ...
                if (activeTemplate.id === 'english-diagnostic-1') {
                    return generateEnglishDiagnosticComment(data.student, data.diagnosticData, activeTemplate);
