...
export class ExcelReader {
        ...
        for (const [sheetName, className] of Object.entries(CLASS_SHEET_MAPPING)) {
            try {
                ...
                if (sheetData) {
                    ...
                    Logger.info('Loaded sheet', { sheetName, className, studentCount: sheetData.students.length });
                }
            } catch (error) {
    ...
    private async readSheet(sheetName: string, className: string): Promise<SheetData | null> {
        ...
        worksheet.eachRow((row, rowNumber) => {
            ...
            if (hasData && studentData[STANDARD_COLUMNS.FIRST_NAME] && studentData[STANDARD_COLUMNS.LAST_NAME]) {
