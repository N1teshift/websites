...
import {
  ...
  Assessment,
  AssessmentType,
} from "@/features/modules/edtech/progressReport/types/ProgressReportTypes";
import { detectColumnType, DynamicColumnInfo } from "../utils/dynamicColumnMapper";
import {
  processPDAssessment,
  updateCambridgeObjective,
  updateStudentMissionsWithAssessment,
} from "../utils/cambridgeMissionUpdater";
...
export class DataProcessorV5 {
    ...
    for (const [columnName, value] of Object.entries(row)) {
      ...
      if (info.baseType === "TVARK") {
        if (value !== null && value !== undefined && value !== "") {
          ...
          if (!studentData.profile.learning_attributes)
            studentData.profile.learning_attributes = {};
  ...
  private createKDAssessment(
    ...
    if (hasC && data.C) {
      const cKeys = Object.keys(data.C)
        .map((k) => parseInt(k))
        .sort();
      cKeys.forEach((key) => {
        if (key === 0) {
    ...
    return {
      ...
      added: this.currentDate,
      evaluation_details:
        evalDetails as import("@/features/modules/edtech/progressReport/types/ProgressReportTypes").Assessment["evaluation_details"],
  ...
  private processKDCambridgeUpdate(
    ...
    const cKeys = Object.keys(cScores)
      .map((k) => parseInt(k))
      .sort();
   ...
   */
  private generateAssessmentId(columnName: string, baseType: string, number?: number): string {
   ...
   */
  private generateAssessmentTitle(columnName: string, baseType: string, number?: number): string {
    if (baseType === "EXT" && number) return `EXT${number}`;
    if (baseType === "LNT" && number) return `LNT${number}`;
    if (baseType === "ND" && number) return `ND${number}`;
    if (baseType === "SD" && number) return `SD${number}`;
