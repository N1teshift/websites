...
const migrateUnitPlanData = (
  data: Partial<UnitPlanData> & Record<string, unknown>
): UnitPlanData => {
  // Handle old displayMode field
  if ("displayMode" in data && !("outputMapping" in data)) {
    const oldMode = data.displayMode as string;
    data.outputMapping = (oldMode === "full" ? "custom" : oldMode) as
      | "current"
      | "enhanced"
      | "custom";
...
export const useMultipleUnitPlans = () => {
  ...
  const switchToUnitPlan = useCallback(
    (id: string) => {
  ...
  // Add a new unit plan
  const addUnitPlan = useCallback(
    (data?: UnitPlanData, name?: string) => {
  ...
  // Remove a unit plan
  const removeUnitPlan = useCallback(
    (id: string) => {
      setUnitPlans((prev) => {
  ...
  const updateUnitPlan = useCallback(
    (
  ...
  const updateUnitPlanName = useCallback((id: string, newName: string) => {
    setUnitPlans((prev) =>
      prev.map((plan) =>
        plan.id === id ? { ...plan, name: newName, lastModified: new Date() } : plan
      )
    );
  }, []);
  ...
  const updateSubunit = useCallback(
    (subunitIndex: number, field: keyof SubunitData, value: string | number | string[]) => {
      setUnitPlans((prev) =>
        prev.map((plan) => {
          ...
          return {
            ...
            data: {
              ...
              lessonCount:
                field === "lessonsPerSubunit"
                  ? calculateTotalLessonCount(updatedSubunits)
                  : plan.data.lessonCount,
            },
            lastModified: new Date(),
          };
        })
  ...
  const removeSubunit = useCallback(
    (subunitIndex: number) => {
      setUnitPlans((prev) =>
        prev.map((plan) => {
          if (plan.id !== activeUnitPlanId) return plan;
  ...
  const updateQuestion = useCallback(
    (type: QuestionType, index: number, value: string) => {
      setUnitPlans((prev) =>
        prev.map((plan) =>
          plan.id === activeUnitPlanId
            ? {
  ...
  const addTeacher = useCallback(
    (teacherName: string) => {
      if (
        teacherName.trim() &&
        !currentUnitPlan.data.contributingTeachers.includes(teacherName.trim())
      ) {
        setUnitPlans((prev) =>
          prev.map((plan) =>
            plan.id === activeUnitPlanId
        ...
        prev.map((plan) =>
          plan.id === activeUnitPlanId
            ? {
                ...
                data: {
                  ...plan.data,
                  contributingTeachers: plan.data.contributingTeachers.filter(
        ...
        try {
          ...
          if (
            ...
            for (const planData of importedData.plans) {
              if (
                planData.data &&
                typeof planData.data === "object" &&
                "unitTitle" in planData.data
              ) {
                const migratedData = migrateUnitPlanData(planData.data);
                const planName =
                  migratedData.unitTitle ||
                  planData.name ||
                  generateName(migratedData, [...existingNames]);
                ...
                const newPlan: UnitPlanDocument = {
                  ...
                  lastModified: planData.lastModified
                    ? new Date(planData.lastModified)
                    : new Date(),
                };
  ...
  const updateSubunitByPlanId = useCallback(
    (
      planId: string,
      subunitIndex: number,
  ...
  const updateReflectionByPlanId = useCallback(
    (
      planId: string,
      field:
        | "reflectionPriorToTeaching"
