...
interface YearPlanReflectionViewProps {
    ...
    onUpdateReflection: (planId: string, field: 'reflectionPriorToTeaching' | 'reflectionDuringTeaching' | 'reflectionAfterTeaching' | 'reflectionFuturePlanning', value: string) => void;
}

type ReflectionField = 'reflectionPriorToTeaching' | 'reflectionDuringTeaching' | 'reflectionAfterTeaching' | 'reflectionFuturePlanning';
...
const YearPlanReflectionView: React.FC<YearPlanReflectionViewProps> = ({
    ...
    const [editingCell, setEditingCell] = useState<{ planId: string; field: ReflectionField } | null>(null);
    ...
    const getCompletionStats = (plan: UnitPlanDocument) => {
        const completed = reflectionStages.filter(stage => 
        ...
        <div className="space-y-6 p-6">
            <div>
                ...
                <p className="text-text-secondary mb-6">
                    View and edit reflections across all your unit plans. Each unit shows a complete reflection journey 
                    from planning through to future improvements. Click on any reflection to edit it inline.
                </p>
            </div>
                                ...
                                <div className="relative">
                                    ...
                                    <div className="space-y-4">
                                        {reflectionStages.map((stage) => {
                                            const isEditing = editingCell?.planId === plan.id && editingCell?.field === stage.field;
                                            const hasContent = plan.data[stage.field] && plan.data[stage.field].trim().length > 0;
                                                    ...
                                                    <div className="flex items-start gap-4">
                                                        ...
                                                        <div className="flex-1 pb-6">
                                                            ...
                                                            {isEditing ? (
                                                                ...
                                                                <div 
                                                                    onClick={() => handleStartEdit(plan.id, stage.field, plan.data[stage.field])}
            ...
            <div className="bg-surface-card border-2 border-border-default rounded-lg p-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h4 className="text-sm font-semibold text-text-secondary uppercase tracking-wide">Total Units</h4>
                        <p className="text-2xl font-bold text-text-primary mt-1">
                            {unitPlans.length}
                        </p>
                    </div>
                    <div className="text-center">
                        <h4 className="text-sm font-semibold text-text-secondary uppercase tracking-wide">Reflections Completed</h4>
                        <p className="text-2xl font-bold text-brand mt-1">
                            {getTotalCompletionStats().completed} / {getTotalCompletionStats().total}
                        </p>
                    </div>
                    <div className="text-right">
                        <h4 className="text-sm font-semibold text-text-secondary uppercase tracking-wide">Overall Progress</h4>
                        <p className="text-2xl font-bold text-brand mt-1">
                            {Math.round((getTotalCompletionStats().completed / getTotalCompletionStats().total) * 100)}%
                        </p>
                    </div>
                </div>
            </div>
