...
interface ATLSectionProps {
  ...
  updateUnitPlan: (
    ...
    value: string | string[] | number | SubunitData[] | ATLCard[]
  ) => void;
  subject: string;
}

const ATLSection: React.FC<ATLSectionProps> = ({ unitPlan, updateUnitPlan, subject }) => {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex items-center gap-3 pb-4 border-b-2 border-border-default">
        ...
        <h2 className="text-3xl font-bold text-text-primary">{t("atl_skills_strategies")}</h2>
      </div>
              ...
              dataFetcher={(subject, t) => {
                ...
                return availableSkills.map((skill) => {
                  ...
                  const subcategory = skill.subcategory
                    ? t(`atl_subcategory.${skill.subcategory}`) || skill.subcategory
                    : null;
