...
const FormField: React.FC<FormFieldProps> = ({
  ...
  const baseInputClasses =
    "w-full px-4 py-2.5 border-2 border-border-default rounded-xl bg-surface-card text-text-primary placeholder-text-muted transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-brand-light focus:border-brand hover:border-border-accent";
  ...
  return (
    <div className="space-y-2">
      ...
      {fieldName && isFeatureEnabled("fieldCompletion") && (
        <FieldCompletionIndicator fieldName={fieldName as keyof UnitPlanData} value={localValue} />
      )}
    </div>
  );
};
