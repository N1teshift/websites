...
const ResourcesSection: React.FC<ResourcesSectionProps> = ({ unitPlan, updateUnitPlan }) => {
  ...
  const updateResourceItem = (
    field: "printedResources" | "digitalResources" | "guestsResources",
    index: number,
    value: string
  ) => {
    const currentList = Array.isArray(unitPlan[field]) ? (unitPlan[field] as string[]) : [];
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-4 bg-surface-card rounded-2xl border-2 border-border-default p-6 shadow-soft hover:shadow-medium transition-shadow">
          ...
          <div className="space-y-2">
            {(Array.isArray(unitPlan.printedResources) ? unitPlan.printedResources : []).map(
              (item, index) => (
        ...
        <div className="space-y-4 bg-surface-card rounded-2xl border-2 border-border-default p-6 shadow-soft hover:shadow-medium transition-shadow">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-bold text-text-primary flex items-center gap-2">
              <span className="w-8 h-8 rounded-lg bg-gradient-to-br from-secondary-100 to-secondary-200 flex items-center justify-center text-secondary-600">
                💻
              </span>
              {t("digital_resources")}
            </h3>
          ...
          <div className="space-y-2">
            {(Array.isArray(unitPlan.digitalResources) ? unitPlan.digitalResources : []).map(
              (item, index) => (
                <div key={index} className="flex items-center space-x-2">
                  <input
                    type="text"
        ...
        <div className="space-y-4 bg-surface-card rounded-2xl border-2 border-border-default p-6 shadow-soft hover:shadow-medium transition-shadow">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-bold text-text-primary flex items-center gap-2">
              <span className="w-8 h-8 rounded-lg bg-gradient-to-br from-success-100 to-success-200 flex items-center justify-center text-success-600">
                👥
              </span>
              {t("guests")}
            </h3>
          ...
          <div className="space-y-2">
            {(Array.isArray(unitPlan.guestsResources) ? unitPlan.guestsResources : []).map(
              (item, index) => (
                <div key={index} className="flex items-center space-x-2">
                  <input
                    type="text"
