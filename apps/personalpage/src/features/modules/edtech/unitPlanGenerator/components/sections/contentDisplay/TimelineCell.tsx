...
const TimelineCell: React.FC<TimelineCellProps> = ({
  ...
  const hoverBgColor =
    curriculumType === "bup" ? "bg-blue-300 border-blue-500" : "bg-green-300 border-green-500";

  const connectedBgColor =
    curriculumType === "bup" ? "bg-blue-200 border-blue-400" : "bg-green-200 border-green-400";
  ...
  return (
    <div
      className={`
                ...
                ${
                  isHovered
                    ? `${hoverBgColor} scale-105 shadow-xl z-10`
                    : isConnected
                      ? `${connectedBgColor} scale-102 shadow-md`
                      : bgColor
                }
            `}
    ...
    >
      <div className="space-y-2">
        ...
        <h4 className="text-sm font-bold text-gray-900 leading-tight">{topic.title}</h4>

        {/* BUP: Show content preview */}
        {topic.content && <p className="text-xs text-gray-700 line-clamp-3">{topic.content}</p>}
          ...
          <div className="text-xs text-gray-700">
            ...
            <ul className="mt-1 space-y-1">
              ...
              {topic.objectives.length > 2 && (
                <li className="text-gray-500 italic">+ {topic.objectives.length - 2} more...</li>
              )}
            </ul>
          </div>
        )}
        ...
        {topic.connections && topic.connections.length > 0 && (
          <div className="mt-2 pt-2 border-t border-gray-300">
            <span className="text-xs text-gray-600 font-semibold flex items-center">
              ...
              {topic.connections.length === 1 ? "connection" : "connections"}
            </span>
          </div>
        )}
      </div>
