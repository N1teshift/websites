            ...
            {unitPlan.subunits.map((subunit, index) => {
              const hasPriorKnowledge =
                (subunit.priorKnowledgeSubjectSpecific &&
                  subunit.priorKnowledgeSubjectSpecific.length > 0) ||
                (subunit.priorKnowledgeLearningSkills &&
                  subunit.priorKnowledgeLearningSkills.length > 0);
              ...
              const lessonText =
                subunit.lessonsPerSubunit === 1
                  ? "1 lesson"
                  : `${subunit.lessonsPerSubunit} lessons`;

              return (
                <React.Fragment key={index}>
                  <tr
                    style={index % 2 === 0 ? previewStyles.tableRowEven : previewStyles.tableRowOdd}
                  >
                    <td className="px-4 py-3 text-center">
                      <div
                        className="flex flex-col items-center text-sm font-semibold"
                        style={{ color: previewStyles.badge.backgroundColor }}
                      >
                        {subunitWords.map((word, wordIdx) => (
                          <div key={wordIdx}>{word}</div>
                        ))}
                        <div
                          className="text-white px-3 py-1 rounded-full text-xs font-semibold mt-2"
                          style={previewStyles.badge}
                        >
                          {lessonText}
                        </div>
                      </div>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">
                      {subunit.content || "No content specified"}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-700">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">
                      {subunit.activities || "No activities specified"}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-700">
                      {subunit.learningExperiences || "No learning experiences specified"}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-700">
                      {subunit.differentiation || "No differentiation specified"}
                    </td>
                  </tr>
                        ...
                        <div className="space-y-4">
                          ...
                          {hasPriorKnowledge && (
                            <div
                              ...
                              style={previewStyles.knowledgeBlock}
                            >
                              <h4
                                className="font-semibold mb-3 text-base"
                                style={previewStyles.subheading}
                              >
                                Prior Knowledge
                              </h4>
                          ...
                          {hasNewKnowledge && (
                            ...
                            >
                              ...
                              {subunit.topicsTerminology &&
                                subunit.topicsTerminology.length > 0 && (
                                  <div className="mb-3">
                                    <strong
                                      className="block mb-1 text-sm"
                                      style={previewStyles.fieldLabel}
                              ...
                              {subunit.conceptualKnowledge &&
                                subunit.conceptualKnowledge.length > 0 && (
                                  <div className="mb-3">
                                    <strong
                                      className="block mb-1 text-sm"
                                      style={previewStyles.fieldLabel}
                              ...
                              {subunit.proceduralKnowledge &&
                                subunit.proceduralKnowledge.length > 0 && (
                                  <div>
                                    <strong
                                      className="block mb-1 text-sm"
                                      style={previewStyles.fieldLabel}
