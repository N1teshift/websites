...
const YearPlanPreviewSection: React.FC<YearPlanPreviewSectionProps> = ({ unitPlans }) => {
    ...
    const hasCommonInfo = firstPlan && sortedPlans.every(plan => 
    ...
    return (
        <div className="container">
                        ...
                        {hasCommonInfo && firstPlan ? (
                            <>
                                ...
                                <div className="space-y-1" style={previewStyles.headerText}>
                                    <div><span className="font-semibold">Subject:</span> <span className="capitalize">{firstPlan.subject.replace(/-/g, ' ')}</span></div>
                                    <div><span className="font-semibold">MYP Year:</span> MYP Year {firstPlan.mypYear}</div>
                                    <div><span className="font-semibold">Academic Year:</span> {firstPlan.academicYear}</div>
                                    <div><span className="font-semibold">Total Lessons:</span> {yearTotalLessons}</div>
                                </div>
                            </>
                        ) : (
                            <>
                                ...
                                <div className="space-y-1" style={previewStyles.headerText}>
                                    <div><span className="font-semibold">Unit Plans:</span> {sortedPlans.length}</div>
                                    <div><span className="font-semibold">Total Lessons:</span> {yearTotalLessons}</div>
                                </div>
                            </>
                        )}
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {!hasCommonInfo && (
                                            <div className="text-xs text-text-muted mt-1">
                                                ...
                                                {plan.data.subject && <span className="capitalize">{plan.data.subject.replace(/-/g, ' ')} • </span>}
                                    ...
                                    <div className="flex-shrink-0 text-right">
                                        <div className="text-xl font-bold" style={{ color: previewStyles.sectionTitle.color }}>
                                            {totalLessons}
                                        </div>
                ...
                <div className="px-8 py-4 bg-surface-button border-t" style={{ borderTopColor: previewStyles.sectionTitle.borderColor }}>
