...
export function getCambridgeTopics(grade: number): CurriculumTopic[] {
    const stageData: CambridgeStage | undefined = CAMBRIDGE_CURRICULUM.find((s: CambridgeStage) => s.grade === grade);
    if (!stageData) return [];
    ...
    for (const strand of stageData.strands) {
        for (const substrand of strand.substrands) {
            ...
            const objectivesList = substrand.objectives.map((obj: { code: string; description: string; id: string }) => obj.description);
