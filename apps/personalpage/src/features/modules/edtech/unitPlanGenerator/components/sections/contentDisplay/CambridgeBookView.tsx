...
const CambridgeBookView: React.FC<CambridgeBookViewProps> = ({ title, description, book }) => {
              ...
              >
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span
                      className={`text-xs font-semibold px-2 py-0.5 rounded ${strandStyle.badge}`}
                    >
                    ...
                    <span className="text-xs text-gray-500">pp. {unit.pages}</span>
                  </div>
                  <h4 className={`text-base font-bold ${strandStyle.text}`}>{unit.title}</h4>
                  {hasSubsections && (
                    <p className="text-xs text-gray-600 mt-1">
                      {unit.subsections.length}{" "}
                      {unit.subsections.length === 1 ? "section" : "sections"}
                    </p>
                  )}
                </div>
                {hasSubsections && (
