...
const Navigation: React.FC<NavigationProps> = React.memo(
  ({ activeSection, onSectionChange, unitPlan, viewingMode, setViewingMode }) => {
    ...
    const sections = useMemo(
      () => [
        ...
        {
          key: "summative-assessment" as ActiveSection,
        ...
        {
          key: "community-engagement" as ActiveSection,
          label: t("community_engagement"),
          icon: Users,
        },
        ...
        <div className="sm:hidden">
          ...
          >
            <div className="flex items-center space-x-3">
              {React.createElement(
                sections.find((s) => s.key === activeSection)?.icon || BookOpen,
                {
                  className: "w-5 h-5 text-brand",
                  strokeWidth: 2,
                }
              )}
              <span className="font-semibold text-text-primary">{getActiveSectionLabel()}</span>
            </div>
            <ChevronDown
              className={`w-5 h-5 text-text-muted transform transition-transform duration-300 ${isBottomSheetOpen ? "rotate-180" : ""}`}
            />
          </button>
          ...
          >
            ...
            <div className="px-6 py-4 border-b border-border-default bg-surface-card">
              <h3 className="text-lg font-bold text-text-primary">{t("unit_plan_generator")}</h3>
              <p className="text-sm text-text-secondary mt-1">Select a section to navigate</p>
            </div>
              ...
              <div className="px-4 py-2">
                      ...
                      <div className="flex-1 text-left">
                        <div
                          className={`font-semibold ${
                            activeSection === section.key ? "text-brand" : "text-text-primary"
                          }`}
                        >
                          {section.label}
                        </div>
                ...
                >
                  {viewingMode === "individual" ? (
                    <FileStack
                      className={`flex-shrink-0 ${viewingMode === "individual" ? "text-success-600" : "text-secondary-600"}`}
                      size={22}
                      strokeWidth={2}
                    />
                  ) : (
                    <Calendar
                      className={`flex-shrink-0 ${viewingMode === "year" ? "text-secondary-600" : "text-success-600"}`}
                      size={22}
                      strokeWidth={2}
                    />
                  )}
                  <div className="flex-1 text-left">
                    <div
                      className={`font-semibold ${
                        viewingMode === "year" ? "text-secondary-700" : "text-success-700"
                      }`}
                    >
                    ...
                    <div
                      className={`text-xs mt-0.5 font-medium ${
                        viewingMode === "year" ? "text-secondary-600" : "text-success-600"
                      }`}
                    >
          ...
          <div className="px-4 sm:px-6 py-3">
            <div className="flex flex-wrap gap-2">
              {sections.map((section) => {
                ...
                const shouldShow =
                  section.key !== "content-display" ||
                  (unitPlan?.subject === "mathematics" &&
                    (unitPlan?.mypYear === 2 || unitPlan?.mypYear === 3));
              ...
              <button
                ...
                title={
                  viewingMode === "individual"
                    ? "Switch to Year Plan View"
                    : "Switch to Individual Plan View"
                }
              >
                <div className="flex flex-col items-center gap-1.5">
                  {viewingMode === "individual" ? (
