...
const UnitPlanSwitcher: React.FC<UnitPlanSwitcherProps> = ({
    ...
    useEffect(() => {
        if (scrollContainerRef.current) {
            const activeElement = scrollContainerRef.current.querySelector(`[data-plan-id="${activeUnitPlanId}"]`);
            if (activeElement) {
    ...
    const handleRemovePlan = useCallback((e: React.MouseEvent, id: string) => {
        e.stopPropagation();
    ...
    return (
        <div className="border-b-2 border-border-default bg-transparent w-full shadow-soft">
            ...
            >
                ...
                >
                    <Copy className="w-4 h-4 text-success-600 group-hover:scale-110 transition-transform" strokeWidth={2.5} />
