...
const PreviewContentPlanningSection: React.FC<PreviewContentPlanningSectionProps> = ({
  unitPlan,
}) => {
  ...
  const hasPriorKnowledge =
    (isCurrent || isFull) &&
    ((unitPlan.priorKnowledgeSubjectSpecific &&
      unitPlan.priorKnowledgeSubjectSpecific.length > 0) ||
  ...
  const hasLearningExperiences =
    (isEnhanced || isFull) &&
    unitPlan.learningExperienceCards &&
    unitPlan.learningExperienceCards.length > 0;
    ...
    <div>
      ...
      {(unitPlan.outputMapping === "current" || unitPlan.outputMapping === "custom") && (
        <>
          {((unitPlan.priorKnowledgeSubjectSpecific &&
            unitPlan.priorKnowledgeSubjectSpecific.length > 0) ||
            (unitPlan.priorKnowledgeLearningSkills &&
              unitPlan.priorKnowledgeLearningSkills.length > 0) ||
            (unitPlan.topicsTerminology && unitPlan.topicsTerminology.length > 0) ||
            (unitPlan.conceptualKnowledge && unitPlan.conceptualKnowledge.length > 0) ||
            (unitPlan.proceduralKnowledge && unitPlan.proceduralKnowledge.length > 0)) && (
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {unitPlan.priorKnowledgeSubjectSpecific &&
                  unitPlan.priorKnowledgeSubjectSpecific.length > 0 && (
                    <div className="bg-surface-button p-4 rounded-lg border-l-4 border-blue-900">
                      <div className="font-semibold text-blue-900 text-sm uppercase tracking-wide mb-2">
                ...
                {unitPlan.priorKnowledgeLearningSkills &&
                  unitPlan.priorKnowledgeLearningSkills.length > 0 && (
                    <div className="bg-surface-button p-4 rounded-lg border-l-4 border-blue-900">
                      <div className="font-semibold text-blue-900 text-sm uppercase tracking-wide mb-2">
                        Learning Skills Prior Knowledge
                      </div>
                      <ul className="list-disc list-inside text-gray-700 space-y-1">
          ...
          {((unitPlan.informalFormativeAssessment &&
            unitPlan.informalFormativeAssessment.length > 0) ||
            (unitPlan.formalFormativeAssessment &&
              unitPlan.formalFormativeAssessment.length > 0)) && (
            <div className="mb-8">
          ...
          {((unitPlan.differentiationByAccess && unitPlan.differentiationByAccess.length > 0) ||
            (unitPlan.differentiationByProcess && unitPlan.differentiationByProcess.length > 0) ||
            (unitPlan.differentiationByProduct &&
              unitPlan.differentiationByProduct.length > 0)) && (
            <div className="mb-8">
              <h3 className="text-xl font-semibold text-blue-800 mb-4">
                Differentiation Strategies
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {unitPlan.differentiationByAccess &&
                  unitPlan.differentiationByAccess.length > 0 && (
                    <div className="bg-surface-button p-4 rounded-lg border-l-4 border-blue-900">
                      <div className="font-semibold text-blue-900 text-sm uppercase tracking-wide mb-2">
                        By Access
                      </div>
                ...
                {unitPlan.differentiationByProcess &&
                  unitPlan.differentiationByProcess.length > 0 && (
                    <div className="bg-surface-button p-4 rounded-lg border-l-4 border-blue-900">
                      <div className="font-semibold text-blue-900 text-sm uppercase tracking-wide mb-2">
                        By Process
                      </div>
          ...
          <div className="mb-8">
            ...
            <div className="space-y-4">
              {unitPlan.learningExperienceCards.map((card, index) => (
                <div
                  key={index}
                  className="bg-surface-button p-4 rounded-lg border-l-4 border-blue-900"
                >
                  <div className="font-semibold text-blue-900 mb-2">
                    Learning Experience {index + 1}:{" "}
                    {card.learningExperienceName || "Not specified"}
                  </div>
                  <div className="text-gray-700 space-y-2">
                    <div>
                      <strong>Duration:</strong> Day(s){" "}
                      {card.learningExperienceDayRange || "Not specified"} (
                      {card.learningExperienceHoursCount || "N/A"} hours)
                    </div>
                    <div>
                      <strong>Description:</strong>{" "}
                      {card.learningExperienceDescription || "Not specified"}
                    </div>
                    <div>
                      <strong>Formative Assessment:</strong>{" "}
                      {card.learningExperienceFormativeAssessment || "Not specified"}
                    </div>
