          ...
          <div className="flex items-start">
            ...
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-warning-700">
                <p>{t("content_display_not_available_description")}</p>
                <div className="mt-4 space-y-1">
                  <p className="font-semibold">{t("current_settings")}:</p>
                  <ul className="list-disc list-inside ml-4">
                    <li>
                      {t("subject")}:{" "}
                      <span className="font-medium">{unitPlan.subject || t("not_selected")}</span>
                    </li>
                    <li>
                      {t("myp_year")}:{" "}
                      <span className="font-medium">{unitPlan.mypYear || t("not_selected")}</span>
                    </li>
                  </ul>
                </div>
                <div className="mt-4">
                  ...
                  <ul className="list-disc list-inside ml-4">
                    <li>
                      {t("subject")}: <span className="font-medium">Mathematics</span>
                    </li>
                    <li>
                      {t("myp_year")}: <span className="font-medium">2 or 3</span>
                    </li>
                  </ul>
                </div>
              </div>
          ...
          {cambridgeBook && cambridgeBook.units.length > 0 && cambridgeBook.modules.length > 0 ? (
            <>
              <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 via-purple-50 to-green-50 border-2 border-gray-300 rounded-lg">
                ...
                <p className="text-sm text-gray-700">{t("curriculum_comparison_description")}</p>
              </div>

              <UnifiedCurriculumTimeline bupGrade={displayGrade} cambridgeBook={cambridgeBook} />

              {/* Connection Info */}
              <div className="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                <h4 className="font-semibold text-amber-900 mb-2">
                  💡 {t("how_connections_work")}
                </h4>
                <p className="text-sm text-amber-800">{t("connections_description")}</p>
              </div>
            </>
          ) : (
