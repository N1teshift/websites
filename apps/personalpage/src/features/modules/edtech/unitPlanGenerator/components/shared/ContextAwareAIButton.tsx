 ...
 */
const ContextAwareAIButton = React.memo(
  <K extends keyof UnitPlanData | string>({
    fieldName,
    unitPlanContext,
    onGenerate,
    size = "md",
    ...
    const handleClick = async () => {
      ...
      try {
        ...
        logger.info(
          `Generated content: ${typeof generatedContent === "string" ? generatedContent.substring(0, 100) + "..." : String(generatedContent)}`
        );
        ...
        logger.error(
          `Error generating content for ${String(fieldName)}: ${error instanceof Error ? error.message : String(error)}`
        );
        ...
        if (error instanceof Error) {
          if (error.message.includes("401") || error.message.includes("403")) {
            ...
            alert(
              "Please fill in more unit plan details to provide better context for AI generation."
            );
          } else {
    ...
    const buildFieldPrompt = (
      ...
      return (
        (unitPlanFieldPrompts[fieldName] ||
          `Generate content for ${fieldName} in "${unitTitle}" for ${subject}.${availableContext}`) +
        contextWarning
      );
    };
    ...
    const checkMissingCriticalDependencies = (
      ...
      return critical.filter(
        (dep) => !unitPlanContext[dep] || String(unitPlanContext[dep]).trim() === ""
      );
    };
    ...
    const generateFieldContent = async (
      ...
      try {
        const response = await getModelResponse(prompt, {
          systemPrompt:
            "You are an expert educational content generator. Generate high-quality, educational content that is appropriate for the specified field and subject area.",
          temperature: 0.7,
          maxTokens: 500,
          model: "gpt-4o-mini",
          textFormat: "text",
        ...
        const responseWithData = response as ResponsesResult & {
          success?: boolean;
          data?: { output_text?: string };
        };
        logger.info(`OpenAI API response received:`, {
          success: responseWithData.success,
          hasData: !!responseWithData.data,
        });
        ...
        logger.error(
          `Error in generateFieldContent: ${error instanceof Error ? error.message : String(error)}`
        );
        throw error;
      }
    };
