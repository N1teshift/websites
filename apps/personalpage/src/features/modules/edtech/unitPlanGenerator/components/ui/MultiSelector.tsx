...
import { getObjectiveStrands, isObjectiveId, isStrandId, getStrandByFullId, getObjectiveById } from '../../data/objectives';
...
const MultiSelector: React.FC<MultiSelectorProps> = ({
    ...
    const resolvedEmptyStateMessage = emptyStateMessage || t('no_items_selected') || 'No items selected yet. Choose from the options above.';
    ...
    const getTooltipContent = (option: Option): string => {
        ...
        if (isObjectiveId(option.id)) {
            ...
            const localizedObjective = t(`objective.${option.id}`) || `${option.id} - ${option.name || ''}`;
            const strands = getObjectiveStrands(option.id);

            if (strands.length > 0) {
                const strandsText = strands.map(strand => {
            ...
            const strandDescription = localizedStrand || (strandObj ? strandObj.description : option.description || '');
            const parentObjective = strandObj ? getObjectiveById(strandObj.objectiveId) : undefined;
            const localizedObjectiveName = parentObjective ? (t(`objective.${parentObjective.id}`) || parentObjective.name) : '';
    ...
    return (
        <div>
            ...
            {helpText && (
                <p className="text-xs text-text-muted mb-2">{helpText}</p>
            )}
