...
import { getCambridgeByGrade, CambridgeStrand, CambridgeSubstrand, CambridgeObjective } from '../../data/curriculumCambridge';
...
const CurriculumContentSelector: React.FC<CurriculumContentSelectorProps> = ({
    ...
    const formatCambridgeObjective = (objective: CambridgeObjective, substrandName: string, strandName: string): string => {
    ...
    // Connection creators
    const createBUPSubunitConnection = (subunit: BUPSubunit, unit: BUPUnit, module: BUPModule): CurriculumConnection => ({
        id: subunit.id,
        type: 'bup',
        level: 'subunit',
    ...
    const createCambridgeObjectiveConnection = (objective: CambridgeObjective, substrand: CambridgeSubstrand, strand: CambridgeStrand): CurriculumConnection => ({
        id: objective.id,
        type: 'cambridge',
        level: 'objective',
    ...
    const createCambridgeSubstrandConnection = (substrand: CambridgeSubstrand, strand: CambridgeStrand): CurriculumConnection => ({
        id: substrand.id,
        type: 'cambridge',
        level: 'substrand',
    ...
    const createBookSubsectionConnection = (subsection: BookSubsection, unit: BookUnit): CurriculumConnection => ({
        id: subsection.id,
        type: 'cambridge-book',
        ...
        <div className="mb-6">
            ...
            {isMainExpanded && (
                <div className="mt-2 bg-gradient-to-r from-blue-50 via-purple-50 to-green-50 border-2 border-gray-300 rounded-lg p-6 space-y-4">
                    <p className="text-sm text-gray-600 mb-4">
                        {t('curriculum_selector_description')}
                    </p>
                                ...
                                <div key={module.id} className="border border-gray-200 rounded-md overflow-hidden">
                                    <div className="flex items-center justify-between p-3 bg-blue-50 hover:bg-blue-100 transition-colors">
                                        ...
                                        <div className="flex gap-1">
                                            <button
                                                onClick={() => handleAddWithText(formatBUPModule(module), createBUPModuleConnection(module))}
                                            ...
                                            {module.units.map((unit: BUPUnit) => (
                                                <div key={unit.id} className="border border-gray-200 rounded-md overflow-hidden">
                                                    <div className="flex items-center justify-between p-2 bg-gray-50 hover:bg-gray-100 transition-colors">
                                                        ...
                                                        <div className="flex gap-1">
                                                            <button
                                                                onClick={() => handleAddWithText(formatBUPUnit(unit, module.name), createBUPUnitConnection(unit, module))}
                                                            ...
                                                            <button
                                                                onClick={() => handleAddLinkOnly(createBUPUnitConnection(unit, module))}
                                                                className="px-2 py-1 bg-purple-600 text-white rounded hover:bg-purple-700 transition-colors flex items-center gap-1 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                                                title={t('link_only')}
                                                                disabled={isConnected(unit.id)}
                                                            >
                                                            ...
                                                            {unit.subunits.map((subunit: BUPSubunit) => (
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="ml-2 flex gap-1 opacity-0 group-hover:opacity-100">
                                                                        <button
                                                                            onClick={() => handleAddWithText(formatBUPSubunit(subunit, unit.name, module.name), createBUPSubunitConnection(subunit, unit, module))}
                                                                            className="px-2 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors flex items-center gap-1 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                                                            title={t('add_text_and_link')}
                                                                        ...
                                                                        <button
                                                                            onClick={() => handleAddLinkOnly(createBUPSubunitConnection(subunit, unit, module))}
                                                                            className="px-2 py-1 bg-purple-600 text-white rounded hover:bg-purple-700 transition-colors flex items-center gap-1 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                                                            title={t('link_only')}
                                                                            disabled={isConnected(subunit.id)}
                                                                        >
                            ...
                            {cambridgeData.strands.map((strand: CambridgeStrand) => (
                                <div key={strand.id} className="border border-gray-200 rounded-md overflow-hidden">
                                    <div className="flex items-center justify-between p-3 bg-green-50 hover:bg-green-100 transition-colors">
                                        ...
                                        <div className="flex gap-1">
                                            <button
                                                onClick={() => handleAddWithText(formatCambridgeStrand(strand), createCambridgeStrandConnection(strand))}
                                                className="px-2 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors flex items-center gap-1 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                                title={t('add_text_and_link')}
                                            ...
                                            <button
                                                onClick={() => handleAddLinkOnly(createCambridgeStrandConnection(strand))}
                                                className="px-2 py-1 bg-purple-600 text-white rounded hover:bg-purple-700 transition-colors flex items-center gap-1 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                                title={t('link_only')}
                                                disabled={isConnected(strand.id)}
                                            >
                                    ...
                                    {expandedStrands.has(strand.id) && (
                                        <div className="p-2 space-y-2 bg-white">
                                            {strand.substrands.map((substrand: CambridgeSubstrand) => (
                                                <div key={substrand.id} className="border border-gray-200 rounded-md overflow-hidden">
                                                    <div className="flex items-center justify-between p-2 bg-gray-50 hover:bg-gray-100 transition-colors">
                                                        <button
                                                        ...
                                                        <div className="flex gap-1">
                                                            <button
                                                                onClick={() => handleAddWithText(formatCambridgeSubstrand(substrand, strand.name), createCambridgeSubstrandConnection(substrand, strand))}
                                                                className="px-2 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors flex items-center gap-1 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                                                title={t('add_text_and_link')}
                                                            ...
                                                            <button
                                                                onClick={() => handleAddLinkOnly(createCambridgeSubstrandConnection(substrand, strand))}
                                                                className="px-2 py-1 bg-purple-600 text-white rounded hover:bg-purple-700 transition-colors flex items-center gap-1 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                                                title={t('link_only')}
                                                                disabled={isConnected(substrand.id)}
                                                            >
                                                        ...
                                                        <div className="p-2 space-y-1 bg-white">
                                                            {substrand.objectives.map((objective: CambridgeObjective) => (
                                                                ...
                                                                >
                                                                    ...
                                                                    <div className="ml-2 flex gap-1 opacity-0 group-hover:opacity-100">
                                                                        <button
                                                                            onClick={() => handleAddWithText(formatCambridgeObjective(objective, substrand.name, strand.name), createCambridgeObjectiveConnection(objective, substrand, strand))}
                                                                            className="px-2 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors flex items-center gap-1 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                                                            title={t('add_text_and_link')}
                                                                        ...
                                                                        <button
                                                                            onClick={() => handleAddLinkOnly(createCambridgeObjectiveConnection(objective, substrand, strand))}
                                                                            className="px-2 py-1 bg-purple-600 text-white rounded hover:bg-purple-700 transition-colors flex items-center gap-1 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                                                            title={t('link_only')}
                                                                            disabled={isConnected(objective.id)}
                                                                        >
            ...
            {cambridgeBook && cambridgeBook.units.length > 0 && (
                <div className="bg-white rounded-lg border border-gray-300 overflow-hidden">
                    ...
                    >
                        <div className="flex items-center gap-2">
                            {cambridgeBookExpanded ? <FiChevronDown size={20} /> : <FiChevronRight size={20} />}
                            ...
                            {cambridgeBook.units
                                ...
                                <div key={unit.id} className="border border-gray-200 rounded-md overflow-hidden">
                                    <div className="flex items-center justify-between p-3 bg-indigo-50 hover:bg-indigo-100 transition-colors">
                                        ...
                                        <div className="flex gap-1">
                                            <button
                                                onClick={() => handleAddWithText(formatBookUnit(unit), createBookUnitConnection(unit))}
                                                className="px-2 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors flex items-center gap-1 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                                title={t('add_text_and_link')}
                                                disabled={isConnected(unit.id)}
                                            >
                                        ...
                                        <div className="p-2 space-y-1 bg-white">
                                            {unit.subsections.map((subsection: BookSubsection) => (
                                                ...
                                                >
                                                    ...
                                                    <div className="ml-2 flex gap-1 opacity-0 group-hover:opacity-100">
                                                        <button
                                                            onClick={() => handleAddWithText(formatBookSubsection(subsection, unit), createBookSubsectionConnection(subsection, unit))}
                                                            className="px-2 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors flex items-center gap-1 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                                            title={t('add_text_and_link')}
                                                        ...
                                                        <button
                                                            onClick={() => handleAddLinkOnly(createBookSubsectionConnection(subsection, unit))}
                                                            className="px-2 py-1 bg-purple-600 text-white rounded hover:bg-purple-700 transition-colors flex items-center gap-1 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                                                            title={t('link_only')}
                                                            disabled={isConnected(subsection.id)}
                                                        >
