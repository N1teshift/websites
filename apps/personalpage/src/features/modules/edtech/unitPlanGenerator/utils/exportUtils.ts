 ...
 */
export const exportUnitPlanAsHTML = (
  unitPlan: UnitPlanData,
  filename: string = "unit-plan.html"
): void => {
...
export const exportCombinedUnitPlansAsHTML = (
  ...
  const unitPlanSections = unitPlans
    .map((plan, index) => {
...
export const exportAllPlansAsZipHTML = async (
  ...
  try {
    ...
    unitPlans.forEach((plan, index) => {
      ...
      const orderPrefix =
        plan.data.unitOrder && plan.data.unitOrder > 0
          ? `${String(plan.data.unitOrder).padStart(2, "0")}_`
          : "";
