...
const textFields: (keyof UnitPlanData)[] = [
    'schoolName', 'unitTitle', 'academicYear', 'subject', 'conceptualUnderstandings',
    'globalContext', 'globalContextExplanation', 'inquiryStatement', 'assessmentTitle',
    'summativeAssessment', 'assessmentType', 'individualContext', 'localContext',
    'globalContextLens', 'atlStrategies', 'priorKnowledgeSubjectSpecific', 'priorKnowledgeLearningSkills',
    'resources', 'printedResources', 'digitalResources', 'guestsResources', 'communityEngagement', 'reflectionPriorToTeaching', 'reflectionDuringTeaching', 'reflectionAfterTeaching',
...
export const fieldThresholds: Record<keyof UnitPlanData, FieldThreshold> = {
    ...
    specifiedConcepts: { minWords: 3, minSentences: 1, description: "Specified concepts", minItems: 2, maxItems: 3 },
    keyConcepts: { minWords: 3, minSentences: 1, description: "Key concepts", minItems: 1, maxItems: 5 },
    relatedConcepts: { minWords: 3, minSentences: 1, description: "Related concepts", minItems: 0, maxItems: 5 },
    conceptualUnderstandings: { minWords: 10, minSentences: 2, description: "Conceptual understandings" },
    globalContext: { minWords: 2, minSentences: 1, description: "Global context" },
    globalContextExplanation: { minWords: 20, minSentences: 3, description: "Global context explanation" },
    ...
    objectives: { minWords: 2, minSentences: 1, description: "Objectives/Strands", minItems: 4, maxItems: 16 }, // Updated for strands
    ...
    summativeAssessmentRelationshipDescription: { minWords: 15, minSentences: 2, description: "Relationship between summative assessment and inquiry statement" },
    assessmentTasks: { minWords: 1, minSentences: 1, description: "Assessment tasks", minItems: 0, maxItems: 10 },
    commandTerms: { minWords: 2, minSentences: 1, description: "Command terms", minItems: 2, maxItems: 3 },
    ...
    learningExperienceCards: { minWords: 1, minSentences: 1, description: "Learning Experience cards", minItems: 0, maxItems: 10 },
    ...
    printedResources: { minWords: 1, minSentences: 1, description: "Printed resources", minItems: 0, maxItems: 10 },
    digitalResources: { minWords: 1, minSentences: 1, description: "Digital resources", minItems: 0, maxItems: 10 },
    guestsResources: { minWords: 1, minSentences: 1, description: "Guests resources", minItems: 0, maxItems: 10 },
    communityEngagement: { minWords: 15, minSentences: 2, description: "Community engagement" },
    
    // Reflection fields - high thresholds
    reflectionPriorToTeaching: { minWords: 20, minSentences: 3, description: "Reflection prior to teaching" },
    reflectionDuringTeaching: { minWords: 20, minSentences: 3, description: "Reflection during teaching" },
    reflectionAfterTeaching: { minWords: 25, minSentences: 3, description: "Reflection after teaching" },
    reflectionFuturePlanning: { minWords: 25, minSentences: 3, description: "Reflection future planning" },
    
    // Subunit fields - will be handled separately
    subunits: { minWords: 0, minSentences: 0, description: "Subunits" },
    priorKnowledgeSubjectSpecific: { minWords: 10, minSentences: 2, description: "Subject specific prior knowledge" },
    priorKnowledgeLearningSkills: { minWords: 10, minSentences: 2, description: "Learning skills" },
    topicsTerminology: { minWords: 1, minSentences: 1, description: "Topics/Terminology", minItems: 0, maxItems: 10 },
    conceptualKnowledge: { minWords: 1, minSentences: 1, description: "Conceptual Knowledge", minItems: 0, maxItems: 10 },
    proceduralKnowledge: { minWords: 1, minSentences: 1, description: "Procedural Knowledge", minItems: 0, maxItems: 10 },
    informalFormativeAssessment: { minWords: 1, minSentences: 1, description: "Informal formative assessment", minItems: 0, maxItems: 10 },
    formalFormativeAssessment: { minWords: 1, minSentences: 1, description: "Formal formative assessment", minItems: 0, maxItems: 10 },
    differentiationByAccess: { minWords: 1, minSentences: 1, description: "Differentiation by access", minItems: 0, maxItems: 10 },
    differentiationByProcess: { minWords: 1, minSentences: 1, description: "Differentiation by process", minItems: 0, maxItems: 10 },
    differentiationByProduct: { minWords: 1, minSentences: 1, description: "Differentiation by product", minItems: 0, maxItems: 10 },
    lessonCount: { minWords: 1, minSentences: 1, description: "Lesson count" },
    curriculumConnections: { minWords: 1, minSentences: 1, description: "Curriculum connections", minItems: 0, maxItems: 20 }
};
...
export const isFieldCompleted = (fieldName: keyof UnitPlanData, value: UnitPlanData[keyof UnitPlanData]): boolean => {
    ...
    if (fieldName === 'subunits' && Array.isArray(value)) {
        return value.length > 0 && value.every(subunit => 
            subunit.content && countWords(subunit.content) >= 1
        );
    }
    ...
    return words >= threshold.minWords && 
           sentences >= threshold.minSentences;
};

// Get field completion status with details
export const getFieldCompletionStatus = (fieldName: keyof UnitPlanData | string, value: UnitPlanData[keyof UnitPlanData] | string) => {
    ...
    const threshold = (fieldThresholds as Record<string, FieldThreshold>)[fieldName] || lessonFieldThresholds[fieldName];
    ...
    if (Array.isArray(value)) {
        ...
        const completedItems = value.filter(item => 
        ...
        if (threshold.minItems !== undefined && threshold.maxItems !== undefined) {
            ...
            } else if (completedItems.length >= threshold.minItems && completedItems.length <= threshold.maxItems) {
                return {
                    completed: true,
                    progress: 100,
    ...
    if (fieldName === 'subunits' && Array.isArray(value)) {
        ...
        const completedSubunits = value.filter(subunit => 
            subunit.content && countWords(subunit.content) >= 1
        );
...
export const calculateOverallProgress = (unitPlan: Partial<UnitPlanData>): number => {
    ...
    const completedFields = fields.filter(field => {
        ...
        return value !== undefined && isFieldCompleted(field, value as UnitPlanData[keyof UnitPlanData]);
    });
...
export const getSectionProgress = (unitPlan: Partial<UnitPlanData>) => {
    const sections = {
        'basic-info': ['schoolName', 'academicYear', 'subject', 'mypYear', 'contributingTeachers', 'unitTitle'],
        'inquiry': ['specifiedConcepts', 'conceptualUnderstandings', 'globalContext', 'globalContextExplanation', 'inquiryStatement', 'factualQuestions', 'conceptualQuestions', 'debatableQuestions'],
        'planning': ['objectives', 'assessmentTitle', 'assessmentType', 'summativeAssessment', 'commandTerms', 'atlSkills', 'atlStrategies'],
        'resources': ['resources'],
        'community-engagement': ['communityEngagement'],
        'reflection': ['reflectionPriorToTeaching', 'reflectionDuringTeaching', 'reflectionAfterTeaching', 'reflectionFuturePlanning'],
    ...
    Object.entries(sections).forEach(([section, fields]) => {
        const completedFields = fields.filter(field => {
            ...
            return value !== undefined && isFieldCompleted(field as keyof UnitPlanData, value as UnitPlanData[keyof UnitPlanData]);
        });
