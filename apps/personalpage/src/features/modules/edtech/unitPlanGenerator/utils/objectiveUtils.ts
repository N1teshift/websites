 ...
 */
export const getSelectedObjectivesSummary = (
  objectiveIds: string[]
): {
  count: number;
  names: string[];
  ...
  const objectives = objectiveIds.map((id) => getObjectiveById(id)).filter(Boolean) as Objective[];
 ...
 */
export const validateObjectiveSelection = (
  objectiveIds: string[]
): {
  isValid: boolean;
  message: string;
...
• Assessment methods align with the nature of each objective`;
  ...
  objectives.forEach((obj) => {
    if (obj.assessmentNotes) {
      ...
      specificGuidance[obj.id] =
        `Focus on the ${obj.strands.length} strands for this objective and ensure assessment tasks provide opportunities for students to demonstrate each strand.`;
    }
  });
 ...
 */
export const checkYearAppropriateness = (
  objectiveIds: string[],
  mypYear: number
): {
  ...
  if (mypYear <= 2) {
    if (objectiveIds.includes("B")) {
      considerations.push(
        "Objective B investigations should be appropriately scaffolded for younger students"
      );
    }
  }
  ...
  if (mypYear >= 3) {
    if (objectiveIds.includes("B")) {
      considerations.push(
        "Objective B can include more complex pattern recognition and rule formulation"
      );
    }
 ...
 */
export const exportObjectivesData = (
  objectiveIds: string[]
): {
  summary: string;
  detailed: Array<{
