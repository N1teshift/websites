...
export function extractEventDetailsFromQuery(req: NextApiRequest): SharedEventDetails {
    ...
    if (typeof summary !== 'string' || typeof startDateTime !== 'string' || typeof endDateTime !== 'string') {
    ...
    return {
        ...
        attendees: [
            {
                email: process.env.NEXT_PUBLIC_CALENDAR_EMAIL ?? (() => { throw new Error("NEXT_PUBLIC_CALENDAR_EMAIL is not set"); })(),
...
export function extractEventDetailsFromBody(req: NextApiRequest): SharedEventDetails {
    const { summary, startDateTime, endDateTime, userEmail, userName, language, userPhone } = req.body;

    if (!summary || !startDateTime || !endDateTime || !userEmail || !language) {
        throw new Error("Missing event details.");
    }

    if (typeof summary !== 'string' || typeof startDateTime !== 'string' || typeof endDateTime !== 'string' || typeof userEmail !== 'string' || typeof language !== 'string') {
        throw new Error("Invalid event details format.");
    }
