...
import {
  ...
  parseRegistrationInfoFromUrl,
} from "@calendar/utils/eventRegistrationUtils";
import {
  RegistrationMethod,
  EventDetails,
  TemporaryEvent,
...
export const useCalendarEventHandlers = ({
  ...
  const handleSelectSlot = useCallback(
    (slotInfo: CalendarSlotSelectInfo) => {
  ...
  const handleEventClick = useCallback(
    (event: CalendarEventClickInfo) => {
      setSelectedEvent(event);
    },
    [setSelectedEvent]
  );

  const handleGuestEventCreated = useCallback(
    (success: boolean, details?: EventDetails) => {
      if (success && details) {
...
export const useEventSuccessHandler = ({
  ...
  const confirmEventPresence = useCallback(
    (tempEvent: TemporaryEvent, details: EventDetails) => {
      const deadline = Date.now() + CONFIRMATION_TIMEOUT_MS;
      const startDateTime = new Date(details.startTime).toISOString();
      const endDateTime = new Date(
        new Date(details.startTime).getTime() + details.duration * 60 * 1000
      ).toISOString();
      ...
      const poll = async () => {
        try {
          ...
          const result = await apiRequest<AvailabilityCheckResult>(
            `/api/calendar/check-availability-google?${params.toString()}`,
            "GET"
          );
          ...
          logger.warn("Failed to confirm event presence, will retry", {
