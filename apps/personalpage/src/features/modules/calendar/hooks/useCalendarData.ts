...
export function useCalendarData() {
  ...
  const lang =
    typeof window !== "undefined" ? window.localStorage.getItem("i18nextLng") || "en" : "en";
  ...
  const addTemporaryEvent = useCallback((tempEvent: TemporaryEvent) => {
    setTemporaryEventsRef.current((prev: TemporaryEvent[]) => {
      ...
      const existingEvent = prev.find(
        (event: TemporaryEvent) => event.start === tempEvent.start && event.end === tempEvent.end
      );
...
function doesGraphEventMatchTemporaryEvent(
  tempEvent: TemporaryEvent,
  events: GraphEvent[]
): boolean {
  return events.some(
    (graphEvent) =>
      graphEvent.start.dateTime === tempEvent.start && graphEvent.end.dateTime === tempEvent.end
  );
}
