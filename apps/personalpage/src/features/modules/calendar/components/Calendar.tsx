...
import { startOfWeek, isBefore, format } from "@websites/infrastructure/date-fns";
...
const LessonScheduler: React.FC<LessonSchedulerProps> = ({
  ...
  const { eventStyleGetter, dayPropGetter, slotPropGetter } = useCalendarStyling(
    clickedEventId,
    selectedSlotId
  );
  ...
  if (!isMounted || !currentDate) {
    return (
      <div className="h-[600px] w-full flex items-center justify-center text-text-primary">
        Loading calendar...
      </div>
    );
  }
      ...
      <BigCalendar
        ...
        components={{
          toolbar: (toolbarProps) => (
            <CalendarToolbar {...toolbarProps} canNavigatePrev={canNavigatePrev} />
          ),
        }}
      />
      <ToastNotification
