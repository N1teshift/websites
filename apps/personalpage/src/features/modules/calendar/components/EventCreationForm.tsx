...
const EventCreationForm: React.FC<EventCreationFormProps> = ({
    ...
    const createEventDetails = useCallback((email?: string): EventDetails => ({
        duration,
        startTime: startTime.toISOString(),
        ...(email && { email })
    }), [duration, startTime]);

    const handleProviderLogin = useCallback((provider: 'google' | 'microsoft') => {
    ...
    return (
        <div>
            ...
            <div className="mb-4">
                <label className="block mb-2 text-lg font-semibold text-text-primary">{t("selected_lesson_start_time")}</label>
                <p className="text-lg text-text-primary">{formatDateTime(startTime)}</p>
            </div>

            <div className="mb-4">
                <label className="block mb-2 text-lg font-semibold text-text-primary">{t("duration")}</label>
                    ...
                    {availableDurations.map((availableDuration) => (
                        <label key={availableDuration}>
                            <input
                                ...
                                className="mr-2"
                            />
                            <span className="text-text-primary">{availableDuration} {t("minutes_kilmininkas")}</span>
                        </label>
                    ))}
                </div>
            </div>
            ...
            {canAutoRegisterWithGoogle ? (
                ...
                <>
                    <div className="flex flex-col gap-2 mb-4">
                        ...
                        >
                            <Image src="/google-logo.svg" alt="Google Icon" width={24} height={24} className="mr-2" />
                        ...
                        <Button
                            ...
                            className="w-full justify-start"
                        >
                            <Image src="/ms-logo.svg" alt="Microsoft Icon" width={24} height={24} className="mr-2" />
                    ...
                    {!isGuestFormVisible ? (
                        ...
                        <div className="guest-registration relative p-4 border border-border-default rounded-md bg-surface-card">
                            <form onSubmit={handleGuestSubmit}>
                                <div className="mb-4">
                                    <label className="block mb-2 text-lg font-semibold text-text-primary">{t("your_name")}</label>
                                    <input
                                        type="text"
                                        value={guestName}
                                ...
                                <div className="mb-4">
                                    <label className="block mb-2 text-lg font-semibold text-text-primary">{t("your_email")}</label>
                                    <input
                                        type="email"
                                        value={guestEmail}
                                ...
                                <div className="mb-4">
                                    <label className="block mb-2 text-lg font-semibold text-text-primary">{t("your_phone_number")}</label>
                                    <input
                                        type="tel"
                                        value={guestPhone}
            ...
            {errorMessage && (
                <div className="mt-4 text-danger-600 font-semibold">
                    {errorMessage}
                </div>
            )}
        </div>
