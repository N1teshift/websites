...
export abstract class TestCase<S extends Record<string, unknown>> {
  ...
  protected buildName(): string {
    ...
    throw new Error(
      `buildName must be implemented by the subclass or a nameBuilder must be provided`
    );
  }
  ...
  protected buildPrompt(): string {
    ...
    // For backward compatibility
    throw new Error(
      `buildPrompt must be implemented by the subclass or a promptBuilder must be provided`
    );
  }
  ...
  protected buildTestId(): string {
    ...
    throw new Error(
      `buildTestId must be implemented by the subclass or an idBuilder must be provided`
    );
  }
  ...
  public additionalChecks?(_result: { objects: MathInput[]; testCase: TestCase<S> }): {
    passed: boolean;
    message?: string;
  } {
    return { passed: true };
