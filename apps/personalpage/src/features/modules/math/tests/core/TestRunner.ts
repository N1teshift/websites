 ...
 * @async
 */
async function generateSettingsWithCache(
  ...
  const cacheKey = makeCacheKey(
    TEST_RESULTS_NAMESPACE,
    `${test.prompt}-${test.objectType || ""}-${system}`
  );
...
export async function runTests(
  ...
  if (invalidTests.length > 0) {
    console.warn(
      `Skipping ${invalidTests.length} invalid tests (missing required properties):`,
