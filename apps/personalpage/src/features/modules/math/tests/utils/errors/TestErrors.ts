...
export function createGenerationError(
    ...
    const errorMessage = `AI generation failed: ${originalError instanceof Error ? 
    ...
    return new TestRunnerError(
        ...
        'GENERATION',
        {
            ...
            details: [
                ...details,
                `Original error: ${originalError instanceof Error ? 
                    originalError.stack : String(originalError)}`
            ]
        }
    );
}
...
export function createExtractionError(
    ...
    const errorMessage = `Extraction failed: ${originalError instanceof Error ? 
        originalError.message : String(originalError)}`;
    ...
    return new TestRunnerError(
        ...
        'EXTRACTION',
        {
            ...
            details: [
                ...details,
                `Failed to extract from response objects`
            ]
        }
    );
}
