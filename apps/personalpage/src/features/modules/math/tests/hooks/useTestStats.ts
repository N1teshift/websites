...
export const useTestStats = () => {
  ...
  const [testStats, setTestStats] = useState<TestStat[]>(() => {
    ...
    const cache = getCache<{ data: TestStat[]; timestamp: number; version: number }>(
      STATS_CACHE_KEY
    );
    if (cache?.data) {
  ...
  const lastRefreshTimeRef = useRef<number>(
    getCache<{ timestamp: number }>(STATS_CACHE_KEY)?.timestamp || 0
  );
  ...
  const statsVersionRef = useRef<number>(
    getCache<{ version: number }>(STATS_CACHE_KEY)?.version || 0
  );
        ...
        if (currentVersion === statsVersionRef.current) {
          if (response.success && response.data?.tests) {
            ...
            if (newStats.length === 0) {
              ...
              logger.info(
                `Test statistics have been updated from database: ${newStats.length} tests`
              );
            }
   ...
   */
  const enrichWithSuccessRates = useCallback(
    (testCases: TestCase<Record<string, unknown>>[]): EnrichedTestCase[] => {
      return testCases.map((test) => {
        const enrichedTest = { ...test } as EnrichedTestCase;
        const stat = testStats.find((s) => s.id === test.testId);
        if (
          stat &&
          typeof stat.runCount === "number" &&
          stat.runCount > 0 &&
          typeof stat.passCount === "number"
        ) {
   ...
   */
  const getTestStat = useCallback(
    (test: TestCase<Record<string, unknown>>): TestStat | undefined => {
