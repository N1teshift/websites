...
import MathItemsDisplay from "./MathItemsDisplay";
...
const MathDisplay: React.FC<MathDisplayProps> = ({ result, isCurrentRunningTest = false }) => {
  // Handle when there's no result yet
  if (!result) {
    return (
      <span className="text-gray-400 text-sm">{isCurrentRunningTest ? "Generating..." : ""}</span>
    );
  }
  ...
  // Display math items
  return (
    <MathItemsDisplay mathItems={result.mathItems} fallbackMessage="No math items available" />
  );
};
