...
const Tag: React.FC<TagProps> = ({ text, type, matched = true }) => {
  ...
  const classes = [styles.tag, propertyStyles[type], !matched && styles.failed]
    .filter(Boolean)
    .join(" ");
...
export const PropertyTags: React.FC<PropertyTagsProps> = ({ test, result }) => {
  ...
  if (test.objectType === "coefficients") {
    ...
    if (
      ...
      Array.isArray(currentSettings.rules) &&
    ...
    if (collectionTags.length > 0) {
      rows.push(
        <div key="collection-tags-row" className={styles.propertyRow}>
          {collectionTags}
        </div>
      );
    }
    ...
    if (currentSettings.coefficients && Array.isArray(currentSettings.coefficients)) {
      currentSettings.coefficients.forEach((individualCoeffSettings, coeffIndex) => {
        ...
        if (
          individualCoeffSettings.range &&
          Array.isArray(individualCoeffSettings.range) &&
          individualCoeffSettings.range.length === 2
        ) {
        ...
        if (individualCoeffTags.length > 0) {
          rows.push(
            <div key={`${baseKey}-row`} className={styles.propertyRow}>
              {individualCoeffTags}
            </div>
          );
        }
  ...
  function hasGetSettings(obj: unknown): obj is { getSettings: () => unknown } {
    return (
      typeof obj === "object" &&
      obj !== null &&
      typeof (obj as { getSettings?: unknown }).getSettings === "function"
    );
  }
  ...
  if (test.objectType === "coefficient") {
    ...
    if (settings.range && Array.isArray(settings.range) && settings.range.length === 2) {
      ...
      tags.push(<Tag key="range" text={`[${min},${max}]`} type="range" matched={result?.passed} />);
    }
  ...
  if (test.objectType === "terms") {
    ...
    if (settings.power && Array.isArray(settings.power) && settings.power.length === 2) {
      ...
      tags.push(
        <Tag key="power" text={`Power: [${min},${max}]`} type="range" matched={result?.passed} />
      );
    }
  }
  ...
  return <div className={styles.tagContainer}>{tags}</div>;
};
