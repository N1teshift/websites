...
import { CoefficientSettings, TermSettings, TermsSettings, ExpressionSettings, EquationSettings, InequalitySettings, FunctionSettings, PointSettings, SetSettings, IntervalSettings, InterfaceType } from "@math/types/index";
import { getRangeSetDescription, getRuleSetDescription } from "../mathObjectSettings/utils/descriptionUtils";
 ...
 */
export const generateCoefficientDescription = (settings: CoefficientSettings, index: number): string => {
...
export const generateExpressionDescription = (
    ...
    if (!settings || !settings.expressions) {
        ...
        return { expressionDescription: "Invalid expression settings: missing expressions array", nextIndex: startIndex };
    }
    ...
    settings.expressions.forEach((subExpr) => {
        if ("coefficients" in subExpr && "termIds" in subExpr) {
            ...
            const termsDescResult = generateTermsDescription(subExpr as TermsSettings, coefficientIndex, false);
            subExpressionDescriptions.push(termsDescResult.termDescription);
            coefficientIndex = termsDescResult.nextIndex;
        } else {
            const exprDescResult = generateExpressionDescription(subExpr as ExpressionSettings, coefficientIndex, false);
...
export const generateFunctionDescription = (
  ...
  const exprDescResult = generateExpressionDescription(settings.expression, coefficientIndex, false);
...
export const generatePointDescription = (settings: PointSettings) => {
    ...
    return settings.showName
        ? `$${settings.name} = ${pointCoordinates}$`
        : `$${pointCoordinates}$`;
};
...
export const generateIntervalDescription = (settings: IntervalSettings) => {
    const interval =
        settings.intervalType === "closed" ? `[a_{1}; a_{2}]` :
            settings.intervalType === "open" ? `(a_{1}; a_{2})` :
                settings.intervalType === "closed_open" ? `[a_{1}; a_{2})` :
                    settings.intervalType === "open_closed" ? `(a_{1}; a_{2}]` :
