...
export class Term extends MathObject<TermSettings, Coefficients> {
	...
	protected formatObject(parts: string[]): string {
		const term = formatTerm(parts, this.settings.power, this.settings.termIds, this.settings.powerOrder, this.settings.variableName);
		if (!term) {
			...
			throw new Error("Term formatting failed unexpectedly. Check inputs and formatTerm implementation.");
		}
		return term;
	}
}
