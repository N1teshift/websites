...
export class Coefficients extends MathObject<CoefficientsSettings> {
	...
	protected getParts(): string[] {
		...
		if (!this.settings.collectionCount || this.settings.collectionCount <= 0) {
			console.error(`Invalid collectionCount (${this.settings.collectionCount}) for Coefficients generation. Must be a positive integer.`);
			return [];
		}
		...
		if (this.settings.collectionCount > this.settings.coefficients.length) {
			console.warn(`collectionCount (${this.settings.collectionCount}) is greater than the number of provided coefficient settings (${this.settings.coefficients.length}). Only ${this.settings.coefficients.length} coefficients will be generated.`);
		...
		for (let i = 0; i < this.settings.collectionCount; i++) {
			...
			do {
				if (retryCount++ > maxRetries) {
					...
					// Option 1: Throw an error to halt generation
					 throw new Error(`Failed to generate unique coefficient at index ${i}. Check settings and constraints.`);
			...
			if (coefficient!.generatedItem !== null) { // Ensure item was generated successfully
