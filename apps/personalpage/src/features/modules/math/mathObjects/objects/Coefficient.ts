...
export class Coefficient extends MathObject<CoefficientSettings> {
  ...
  protected getParts(): string[] {
    const generated = generateCoefficient(
      this.settings.numberSet,
      this.settings.rules,
      this.settings.range
    );
    const formatted = formatCoefficient(
      generated.raw,
      this.settings.representationType,
      generated.numberSetUsed
    );
  ...
  private calculateApproxValue(rawCoefficient: number[]): number {
    ...
    if (
      denominator === 0 &&
      (this.settings.numberSet === "rational" || this.settings.numberSet === "real")
    ) {
