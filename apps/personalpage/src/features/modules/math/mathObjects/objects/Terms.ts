...
export class Terms extends MathObject<TermsSettings, Coefficients[]> {
  ...
  protected getParts(coefficientsArray: Coefficients[]): string[] {
    ...
    if (termSettingsArray.length !== coefficientsArray.length) {
      throw new Error(
        `The number of term settings (${termSettingsArray.length}) and Coefficients objects (${coefficientsArray.length}) must match.`
      );
    }
    ...
    for (let i = 0; i < termSettingsArray.length; i++) {
      ...
      if (!termSetting || !termCoefficients) {
        console.warn(
          `Skipping term generation at index ${i} due to missing settings or coefficients.`
        );
        continue;
      }
      ...
      if (!term.generatedItem) {
        console.error(
          `Term at index ${i} failed to generate. Settings: ${JSON.stringify(termSetting)}, Coefficients: ${JSON.stringify(termCoefficients.generatedItems)}`
        );
  ...
  protected formatObject(parts: string[]): string {
    return formatCombinationTerm(
      this.settings.combinationType,
      parts,
      this.settings.power,
