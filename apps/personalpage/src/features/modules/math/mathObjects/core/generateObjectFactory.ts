...
class CoefficientGenerator extends BaseObjectGenerator<Coefficient, CoefficientSettings> {
  ...
  initiateObject(): void {
    this.object = new Coefficient(this.settings);
  }
...
class CoefficientsGenerator extends BaseObjectGenerator<Coefficients, CoefficientsSettings> {
  ...
  initiateObject(): void {
    this.object = new Coefficients(this.settings);
  }
...
class TermGenerator extends BaseObjectGenerator<Term, TermSettings> {
  ...
  generateContent(): void {
    if (!this.coefficients) {
      throw new Error(
        "TermGenerator Error: Coefficients must be generated before generating Term content."
      );
    }
...
class TermsGenerator extends BaseObjectGenerator<Terms, TermsSettings> {
  ...
  generateMaterials(): void {
    ...
    this.coefficientsArray = this.settings.terms.map((termSetting) => {
      if (!termSetting.coefficients) {
        throw new Error(
          `TermsGenerator Error: Coefficients settings missing for term: ${JSON.stringify(termSetting)}`
        );
      }
  ...
  generateContent(): void {
    if (!this.coefficientsArray || this.coefficientsArray.length === 0) {
      ...
      throw new Error(
        "TermsGenerator Error: Coefficients array must be generated before generating Terms content."
      );
    }
...
class ExpressionGenerator extends BaseObjectGenerator<Expression, ExpressionSettings> {
  ...
  generateMaterials(): void {
    ...
    this.subComponents = this.settings.expressions.map((subSettings) => {
      ...
      if ("expressions" in subSettings) {
        ...
        throw new Error(
          `ExpressionGenerator Error: Unknown sub-component type in expressions array: ${JSON.stringify(subSettings)}`
        );
      }
    });
  }
  ...
  generateContent(): void {
    if (this.subComponents.length === 0 && this.settings.expressions?.length > 0) {
      ...
      throw new Error(
        "ExpressionGenerator Error: Sub-components array is empty despite settings having expressions. Material generation might have failed."
      );
    }
...
export class EquationGenerator extends BaseObjectGenerator<Equation, EquationSettings> {
  ...
  generateMaterials(): void {
    if (!this.settings.terms) {
      throw new Error(
        "EquationGenerator Error: Terms (representing sides) are missing in settings."
      );
    }
    ...
    if (this.settings.terms.length === 1 && this.settings.terms[0]) {
      ...
      if (!exprSettings.expressions || exprSettings.expressions.length !== 2) {
        throw new Error(
          "EquationGenerator Error: Simple equation mode requires settings with exactly 2 expressions (LHS and RHS) defined within the first 'terms' element."
        );
      }
      const leftGen = new ExpressionGenerator({
        ...exprSettings,
        expressions: [exprSettings.expressions[0]],
      });
      const rightGen = new ExpressionGenerator({
        ...exprSettings,
        expressions: [exprSettings.expressions[1]],
      });
      this.sides = [leftGen.generate(), rightGen.generate()];
    } else if (
      this.settings.terms.length === 2 &&
      this.settings.terms[0] &&
      this.settings.terms[1]
    ) {
      ...
      throw new Error(
        `EquationGenerator Error: Invalid number of terms provided (${this.settings.terms?.length ?? 0}). Expected 1 (for simple mode) or 2 (for complex mode).`
      );
    }
  }
  ...
  generateContent(): void {
    if (this.sides.length !== 2) {
      throw new Error(
        "EquationGenerator Error: Exactly two sides (LHS and RHS) must be generated before generating Equation content."
      );
    }
...
export class InequalityGenerator extends BaseObjectGenerator<Inequality, InequalitySettings> {
  ...
  generateMaterials(): void {
    if (!this.settings.terms) {
      throw new Error(
        "InequalityGenerator Error: Terms (representing sides) are missing in settings."
      );
    }
    // Explicitly check length for tuple types, which is valid typescript
    if (this.settings.terms.length === 1 && this.settings.terms[0]) {
      ...
      if (!exprSettings.expressions || exprSettings.expressions.length < 2) {
        throw new Error(
          "InequalityGenerator Error: Simple inequality mode requires settings with at least 2 expressions to split into LHS and RHS."
        );
      }
      const midPoint =
      ...
      const leftGen = new ExpressionGenerator({
        ...exprSettings,
        expressions: exprSettings.expressions.slice(0, midPoint),
      });
      const rightGen = new ExpressionGenerator({
        ...exprSettings,
        expressions: exprSettings.expressions.slice(midPoint),
      });
      this.sides = [leftGen.generate(), rightGen.generate()];
    } else if (
      this.settings.terms.length === 2 &&
      ...
      throw new Error(
        `InequalityGenerator Error: Invalid number of terms provided (${this.settings.terms?.length ?? 0}). Expected 1 (for simple mode) or 2 (for complex mode).`
      );
    }
  }
  ...
  generateContent(): void {
    if (this.sides.length !== 2) {
      throw new Error(
        "InequalityGenerator Error: Exactly two sides (LHS and RHS) must be generated before generating Inequality content."
      );
    }
    this.object!.generate(this.sides);
  }
...
export class FunctionGenerator extends BaseObjectGenerator<Function, FunctionSettings> {
  ...
  generateContent(): void {
    if (!this.expression) {
      throw new Error(
        "FunctionGenerator Error: Expression must be generated before generating Function content."
      );
    }
...
export class PointGenerator extends BaseObjectGenerator<Point, PointSettings> {
  ...
  generateContent(): void {
    if (!this.coefficients) {
      throw new Error(
        "PointGenerator Error: Coefficients must be generated before generating Point content."
      );
    }
    this.object!.generate(this.coefficients);
  }
...
class IntervalGenerator extends BaseObjectGenerator<Interval, IntervalSettings> {
  ...
  private parseCoefficientToNumber(coefficientString: string): number {
    ...
    const fractionMatch = coefficientString.match(
      /(?:\\frac\{(\d+(?:\.\d+)?)\}\{(\d+(?:\.\d+)?)\}|(\d+(?:\.\d+)?)\/(\d+(?:\.\d+)?))/
    );
    if (fractionMatch) {
  ...
  generateContent(): void {
    if (!this.coefficients) {
      ...
      throw new Error(
        "IntervalGenerator Error: Coefficients must be generated before generating Interval content."
      );
    }
...
export class SetGenerator extends BaseObjectGenerator<Set, SetSettings> {
  ...
  generateContent(): void {
    if (!this.coefficients) {
      throw new Error(
        "SetGenerator Error: Coefficients must be generated before generating Set content."
      );
    }
    this.object!.generate(this.coefficients);
  }
...
function generateObjectFactory(input: MathInput): MathObjects {
  ...
  if (!settings) {
    ...
    throw new Error(
      `Settings for object type '${input.objectType}' are missing in the input object.`
    );
  }
  ...
  switch (input.objectType) {
    case "coefficient":
      return createGenerator(
        GeneratorClass as GeneratorConstructor<Coefficient, CoefficientSettings>,
        settings as CoefficientSettings
      ).generate();
    case "coefficients":
      return createGenerator(
        GeneratorClass as GeneratorConstructor<Coefficients, CoefficientsSettings>,
        settings as CoefficientsSettings
      ).generate();
    case "term":
      return createGenerator(
        GeneratorClass as GeneratorConstructor<Term, TermSettings>,
        settings as TermSettings
      ).generate();
    case "terms":
      return createGenerator(
        GeneratorClass as GeneratorConstructor<Terms, TermsSettings>,
        settings as TermsSettings
      ).generate();
    case "expression":
      return createGenerator(
        GeneratorClass as GeneratorConstructor<Expression, ExpressionSettings>,
        settings as ExpressionSettings
      ).generate();
    case "equation":
      return createGenerator(
        GeneratorClass as GeneratorConstructor<Equation, EquationSettings>,
        settings as EquationSettings
      ).generate();
    case "function":
      return createGenerator(
        GeneratorClass as GeneratorConstructor<Function, FunctionSettings>,
        settings as FunctionSettings
      ).generate();
    case "point":
      return createGenerator(
        GeneratorClass as GeneratorConstructor<Point, PointSettings>,
        settings as PointSettings
      ).generate();
    case "interval":
      return createGenerator(
        GeneratorClass as GeneratorConstructor<Interval, IntervalSettings>,
        settings as IntervalSettings
      ).generate();
    case "set":
      return createGenerator(
        GeneratorClass as GeneratorConstructor<Set, SetSettings>,
        settings as SetSettings
      ).generate();
    case "inequality":
      return createGenerator(
...
function extractSpecificSettings(input: MathInput): MathObjectSettingsType {
  switch (input.objectType) {
    case "coefficient":
      return input.coefficientSettings;
    case "coefficients":
      return input.coefficientsSettings;
    case "term":
      return input.termSettings;
    case "terms":
      return input.termsSettings;
    case "expression":
      return input.expressionSettings;
    case "equation":
      return input.equationSettings;
    case "function":
      return input.functionSettings;
    case "point":
      return input.pointSettings;
    case "interval":
      return input.intervalSettings;
    case "set":
