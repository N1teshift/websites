...
const PowerOrderToggle: React.FC<PowerOrderToggleProps> = ({
  ...
  return (
    <div
      className={`${labelPosition === "left" ? "flex items-center space-x-4" : "flex flex-col"}`}
    >
      {/* Main Label */}
      <label
        className={`font-semibold ${disabled ? "text-text-muted" : "text-text-primary"} whitespace-nowrap text-center`}
      >
        {t("first_f")}
      </label>
      ...
      <div className={`flex ${layout === "horizontal" ? "flex-row gap-2" : "flex-col gap-2"}`}>
        ...
        <label className="flex items-center gap-1">
          ...
          />
          <span className={`${disabled ? "text-text-muted" : "text-text-primary"}`}>
            {t("power")}
          </span>
        </label>
