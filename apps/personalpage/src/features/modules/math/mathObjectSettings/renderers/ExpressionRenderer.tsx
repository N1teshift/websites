...
import {
    ExpressionSettings, TermSettings, TermsSettings,
    DEFAULT_TERM_SETTINGS, DEFAULT_TERMS_SETTINGS,
...
const ExpressionRenderer: React.FC<MathObjectContainerProps<ExpressionSettings>> = ({
    ...
    const renderChildren = () => {
        ...
        return settings.expressions.map((expr, index) => {
            ...
            const coefficientCount = "coefficients" in expr && "termIds" in expr
                ? (expr as TermSettings).coefficients.collectionCount
                : (expr as TermsSettings).terms.reduce((sum, term) => sum + term.coefficients.collectionCount, 0);
                ...
                <div key={index} className="mt-1">
                    <InterfaceToggleSwitch
                        ...
                        onChange={(newInterface) => {
                            ...
                            const newSettings = newInterface === "simple"
                    ...
                    >
                        {isTermSettings ? (
                            <TermRenderer
                                {...commonProps}
                                settings={expr as TermSettings}
                                objectType="term"
                            />
                        ) : (
                            <TermsRenderer
                                {...commonProps}
                                settings={expr as TermsSettings}
                                objectType="terms"
                            />
                        )}
                    </InterfaceToggleSwitch>
