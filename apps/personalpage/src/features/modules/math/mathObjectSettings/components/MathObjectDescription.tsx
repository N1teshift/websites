...
import {
    CoefficientSettings, CoefficientsSettings, TermSettings, TermsSettings, EquationSettings,
...
import {
    ...
    generateInequalityDescription, generateFunctionDescription, generatePointDescription,
    generateSetDescription, generateIntervalDescription
 ...
 */
const MathObjectDescription: React.FC<MathObjectDescriptionProps> = ({ objectType, interfaceType, startIndex, settings }) => {
    let description = "";
    ...
    switch (objectType) {
        case "coefficient":
            ...
            break;
        case "coefficients":
            description = generateCoefficientsDescription(settings as CoefficientsSettings, interfaceType, startIndex);
            break;
        case "term":
        ...
        case "terms":
            ...
            break;
        case "expression":
            const expressionDescResult = generateExpressionDescription(settings as ExpressionSettings, startIndex, true);
            description = expressionDescResult.expressionDescription;
            break
        case "equation":
            const equationDescResult = generateEquationDescription(settings as EquationSettings, interfaceType, startIndex);
            description = equationDescResult.equationDescription;
            break;
        case "inequality":
            const inequalityDescResult = generateInequalityDescription(settings as InequalitySettings, interfaceType, startIndex);
            description = inequalityDescResult.equationDescription;
            break;
        case "function":
            const functionDescResult = generateFunctionDescription(settings as FunctionSettings, startIndex);
