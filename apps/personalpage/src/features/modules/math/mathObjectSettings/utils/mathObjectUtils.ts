import {
    ...
    DEFAULT_TERMS_SETTINGS, DEFAULT_EXPRESSION_SETTINGS, DEFAULT_SIMPLE_EQUATION_SETTINGS,
    DEFAULT_FUNCTION_SETTINGS, DEFAULT_POINT_SETTINGS, DEFAULT_SET_SETTINGS,
    DEFAULT_INTERVAL_SETTINGS, DEFAULT_SIMPLE_INEQUALITY_SETTINGS,
    MathObjectSettings, CoefficientSettings, MathInput
...
export function convertToMathInput(settings: MathObjectSettings): MathInput {
    ...
    switch (objectType) {
        case "coefficient":
            ...
            return { objectType, coefficientSettings: settings.coefficientSettings, priority, dependency, example };
        case "coefficients":
            if (!settings.coefficientsSettings) {
                throw new Error("Missing coefficientsSettings for coefficients object");
            }
            return { objectType, coefficientsSettings: settings.coefficientsSettings, priority, dependency, example };
        case "term":
        ...
        case "expression":
            ...
            return { objectType, expressionSettings: settings.expressionSettings, priority, dependency, example };
        case "equation":
            if (!settings.equationSettings) {
                throw new Error("Missing equationSettings for equation object");
            }
            return { objectType, equationSettings: settings.equationSettings, priority, dependency, example };
        case "inequality":
            if (!settings.inequalitySettings) {
                throw new Error("Missing inequalitySettings for inequality object");
            }
            return { objectType, inequalitySettings: settings.inequalitySettings, priority, dependency, example };
        case "function":
            if (!settings.functionSettings) {
                throw new Error("Missing functionSettings for function object");
            }
            return { objectType, functionSettings: settings.functionSettings, priority, dependency, example };
        case "point":
        ...
        case "interval":
            ...
            return { objectType, intervalSettings: settings.intervalSettings, priority, dependency, example };
        case "set":
...
function deepMergeWithMissing<T extends object>(
    ...
    for (const key in defObj) {
        ...
        if (inpObj[key] === undefined) {
            ...
            if (key === 'expressions' || key === 'terms' || key === 'termIds' || key === 'range') {
                console.warn(`Critical field missing: ${currentPath} in object type: ${inpObj["objectType"] ?? 'unknown'}`);
            }
        } else {
            const defaultVal = defObj[key];
            const inputVal = inpObj[key];
            if (isPlainObject(defaultVal) && isPlainObject(inputVal)) {
                const { merged, missing: subMissing } = deepMergeWithMissing(defaultVal, inputVal, currentPath);
    ...
    if (missing.length > 0) {
        ...
        if (defObj["objectType"] === 'expression' && !(result as Record<string, unknown>)["expressions"]) {
    ...
    try {
        switch (input.objectType) {
            case "coefficient": {
                ...
                const coeffResult = deepMergeWithMissing(DEFAULT_COEFFICIENT_SETTINGS, input.coefficientSettings);
            ...
            case "coefficients": {
                ...
                const coeffsResult = deepMergeWithMissing(DEFAULT_COEFFICIENTS_SETTINGS, input.coefficientsSettings);
            ...
            case "term": {
                ...
                if (input.termSettings.coefficients) {
                    ...
                    if (!('coefficients' in coeffSettings) || !Array.isArray(coeffSettings.coefficients)) {
                        console.error("Invalid coefficients format. Must be a collection with 'coefficients' array.");
                        throw new Error("Invalid coefficients format. Must be a collection with 'coefficients' array.");
                    }
                    ...
                    termResult.merged.coefficients = {
                        coefficients: coeffSettings.coefficients.map((coeff: CoefficientSettings) => ({
                            ...
                            representationType: coeff.representationType || DEFAULT_COEFFICIENT_SETTINGS.representationType,
            ...
            case "expression": {
                ...
                const exprResult = deepMergeWithMissing(DEFAULT_EXPRESSION_SETTINGS, input.expressionSettings);
            ...
            case "equation": {
                ...
                const eqResult = deepMergeWithMissing(DEFAULT_SIMPLE_EQUATION_SETTINGS, input.equationSettings);
            ...
            case "inequality": {
                ...
                const ineqResult = deepMergeWithMissing(DEFAULT_SIMPLE_INEQUALITY_SETTINGS, input.inequalitySettings);
            ...
            case "interval": {
                ...
                const intervalResult = deepMergeWithMissing(DEFAULT_INTERVAL_SETTINGS, input.intervalSettings);
 ...
 */
export function buildInterfaceMapForInput(input: MathInput, index: number): Record<string, "simple" | "complex"> {
