...
const ExpressionSettingsContainer: React.FC<MathObjectContainerProps<ExpressionSettings>> = ({
  ...
  const parentContainerId =
    containerId.includes("-expression") || containerId.includes("-side-")
  ...
  const isSimpleParent =
    parentInterfaceType === "simple" && (objectType === "equation" || objectType === "inequality");
  ...
  const filteredCombinationOptions = combinationTypeOptions
    .filter((option) => {
