...
const CoefficientsSettingsContainer: React.FC<MathObjectContainerProps<CoefficientsSettings>> = ({
  ...
  useEffect(() => {
    if (
      objectType === "interval" &&
      !settings.rules.every((rule) => ["increasing", "neq"].includes(rule))
    ) {
  ...
  const handleCheckboxChange = (option: CoefficientsRule) => {
    ...
    if (option === "increasing")
      updatedRules = updatedRules.filter((rule) => rule !== "decreasing");
    if (option === "decreasing")
      updatedRules = updatedRules.filter((rule) => rule !== "increasing");
