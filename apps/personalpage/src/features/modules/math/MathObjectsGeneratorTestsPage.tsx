...
export default function MathObjectsGeneratorTestsPage() {
  ...
  const { results, isRunning, currentTestIndex, executeTests, summary, saveResultsMessage } =
  ...
  const { selectedTests, toggleTest, toggleSelectAll } = useTestSelection();
  ...
  const getTestStat = (test: TestCase<Record<string, unknown>>) =>
    testStats.find((s) => s.name === test.name);
  const getTestResult = (test: TestCase<Record<string, unknown>>) =>
    results.find((r) => r.test.name === test.name);
  ...
  const columns: ColumnDefinition<TestCase<Record<string, unknown>>>[] = [
    // Status column (shows pass/fail/running)
    {
      ...
      renderCell: (test: TestCase<Record<string, unknown>>) => {
        ...
        const isCurrentRunningTest =
          isRunning && selectedTests[currentTestIndex]?.name === test.name;
        return (
          <StatusIndicator
    ...
    // Name column
    {
      ...
      className: "w-48",
      renderCell: (test: TestCase<Record<string, unknown>>) => (
        <div className="font-mono text-sm">{test.name}</div>
      ),
    },
    // Type column
    {
      ...
      renderCell: (test: TestCase<Record<string, unknown>>) => {
        ...
        return (
          <div className="text-sm">
            ...
            <div className="text-xs text-text-secondary">{result.elapsedTime}ms</div>
          </div>
        );
      },
    },
      ...
      renderCell: (test: TestCase<Record<string, unknown>>) => {
        ...
        return (
          <div className="text-xs">
            ...
            <div className="text-text-secondary">{getFormattedDate(test)}</div>
          </div>
        );
      },
    },
  ...
  const selectionOptions = {
    ...
    onSelectAll: (tests: TestCase<Record<string, unknown>>[], select: boolean) =>
      toggleSelectAll(tests, select),
