 ...
 */
import { ObjectType, objectTypeOptions, NumberSet, numberSetOptions, RepresentationType, representationTypeOptions, CoefficientRule, coeficientRuleOptions, CoefficientsRule, coefficientsRuleOptions, VariableName, variableNameOptions, CombinationType, combinationTypeOptions, InequalityType, inequalityTypeOptions, FunctionName, functionNameOptions, CapitalLetters, capitalLettersOptions, IntervalType, intervalTypeOptions } from "@math/types/mathTypes";
import { MathInput, CoefficientSettings, CoefficientsSettings, TermSettings, TermsSettings, ExpressionSettings, EquationSettings, InequalitySettings, FunctionSettings, PointSettings, SetSettings, IntervalSettings } from '@math/types/mathObjectSettingsInterfaces';
...
export function isCoefficientSettings(settings: unknown): settings is CoefficientSettings {
    ...
    if (typeof obj.representationType !== 'string' || !isRepresentationType(obj.representationType)) return false;
...
export function isCoefficientsSettings(settings: unknown): settings is CoefficientsSettings {
    ...
    // Check required properties and their types
    if (typeof obj.collectionCount !== 'number' || !Number.isInteger(obj.collectionCount) || obj.collectionCount < 0) return false;
...
export function isTermsSettings(settings: unknown): settings is TermsSettings {
    ...
    if (!isArrayOfTwoNumbers(obj.power)) return false;
    if (typeof obj.powerOrder !== 'boolean') return false;
    if (typeof obj.combinationType !== 'string' || !isCombinationType(obj.combinationType)) return false;
...
export function isExpressionSettings(settings: unknown, depth = 0): settings is ExpressionSettings {
    ...
    for (const expr of obj.expressions as unknown[]) {
        ...
        const isExpr = 'expressions' in subObj && !('terms' in subObj) && !('termIds' in subObj) && isExpressionSettings(expr, depth + 1);
...
export function isMathInput(data: unknown): data is MathInput {
  ...
  if (typeof obj.objectType !== 'string' || !objectTypeOptions.includes(obj.objectType as ObjectType)) {
    return false;
  }
  ...
  if (typeof settings !== 'object' || settings === null) {
    ...
    console.warn(`isMathInput failed: Missing or invalid settings object for type '${objectType}' at key '${settingsKey}'.`);
    return false;
  }
  ...
  switch (objectType) {
    ...
    default:
        ...
        console.error(`isMathInput Error: Unhandled objectType '${objectType}' in validation switch.`);
