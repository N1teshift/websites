 ...
 */
export async function complete_current_level(
  ...
  if (state.contextStack.length === 0) {
    ...
    if (completedObjectSettings) {
      logger.info(
        `Collected ${updatedCollectedSubSettings.length} sub-settings for parent ${parentContext.parentObjectType}.`
      );
    } else {
      logger.info(
        `Sub-object failed, collected ${updatedCollectedSubSettings.length} settings so far for parent ${parentContext.parentObjectType}.`
      );
    }
      ...
      if (parentContext.subSettingsKey) {
        ...
        if (parentContext.expectsSingleSubObject) {
          if (updatedCollectedSubSettings.length === 1) {
            ...
            logger.info(
              `Merged single sub-setting into key '${parentContext.subSettingsKey}' for parent ${parentContext.parentObjectType}.`
            );
          } else {
            logger.error(
              `Error: Expected single sub-object for key '${parentContext.subSettingsKey}' in parent ${parentContext.parentObjectType}, but found ${updatedCollectedSubSettings.length}. Storing array as fallback.`
            );
          ...
          assembledParentSettings[parentContext.subSettingsKey] = updatedCollectedSubSettings;
          logger.info(
            `Merged ${updatedCollectedSubSettings.length} sub-settings into key '${parentContext.subSettingsKey}' for parent ${parentContext.parentObjectType}.`
          );
        }
        // --- END MODIFICATION ---
      } else {
        logger.warn(
          `Warning: No subSettingsKey found for parent ${parentContext.parentObjectType}. Sub-settings stored under generic 'children' key.`
        );
