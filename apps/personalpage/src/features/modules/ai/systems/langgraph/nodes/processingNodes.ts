...
export async function extract_settings(state: GraphStateType): Promise<Partial<GraphStateType>> {
  ...
  if (
    currentObjectType === "coefficients" &&
    ...
    if (
      parentContext.parentObjectType === "term" &&
      Array.isArray(parentTermIds) &&
      parentTermIds.length > 0
    ) {
      const termIdsLength = parentTermIds.length;
      logger.info(
        `Applying default coefficients generation logic based on parent term's termIds length: ${termIdsLength}`
      );
      ...
      logger.info(
        `Default prompt for coefficients, but parent context doesn't match expected term structure. Proceeding with LLM extraction.`
      );
    }
  }
...
export const push_context_and_prepare_descend = async (
  ...
  try {
    ...
    if (!state.contextStack) {
      logger.info(
        "push_context_and_prepare_descend: contextStack is undefined, initializing as empty array"
      );
      state.contextStack = [];
    }
