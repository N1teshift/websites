...
export async function invokeMathObjectGenerator(userPrompt: string): Promise<MathInput[]> {
  ...
  try {
    ...
    if (finalState.error) {
      logger.error("Graph execution resulted in an error", finalState.error instanceof Error ? finalState.error : new Error(String(finalState.error)));
      // Include validation errors if they exist
      const errorDetails = finalState.validationErrors && finalState.validationErrors.length > 0
    ...
    if (!Array.isArray(finalState.accumulatedObjects)) {
        logger.error("Graph finished, but 'accumulatedObjects' is not an array", new Error(`accumulatedObjects is not an array: ${JSON.stringify(finalState.accumulatedObjects)}`));
        throw new Error("Graph execution finished, but the final output (accumulatedObjects) was not in the expected array format.");
    }

    // Successfully generated objects are in the accumulatedObjects array
    logger.info("Successfully generated MathInput objects", { accumulatedObjects: finalState.accumulatedObjects });
    ...
    logger.error("An unexpected error occurred during graph invocation", error instanceof Error ? error : new Error(String(error)));
    // Re-throw the error to be handled by the caller
    throw new Error(`Failed to invoke math object generator graph: ${
