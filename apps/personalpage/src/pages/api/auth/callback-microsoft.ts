...
import {
  ...
  getMicrosoftConfig,
} from "@websites/infrastructure/api/microsoft";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const meetingLink =
      (onlineMeeting?.joinUrl as string) ||
      (createdEvent.webLink as string) ||
      (createdEvent.onlineMeetingUrl as string) ||
      "";

    logger.debug("Preparing to send email notification", {
      hasEmail: !!userEmail,
      hasMeetingLink: !!meetingLink,
      meetingLinkSource: onlineMeeting?.joinUrl
    ...
    const registrationInfo = {
      method: "microsoft",
      duration: Math.round(
        (new Date(sharedEventDetails.endDateTime).getTime() -
          new Date(sharedEventDetails.startDateTime).getTime()) /
          (1000 * 60)
      ),
