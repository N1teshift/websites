...
import { checkGoogleAvailability } from '@/features/modules/calendar/api';

const logger = createComponentLogger('CheckAvailabilityGoogle', 'handler');

/**
 * API route handler for checking Google Calendar availability.
 *
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    logger.info('Checking Google Calendar availability', { 
      startDateTime, 
      endDateTime, 
    ...
    const result = await checkGoogleAvailability(
      startDateTime,
      endDateTime,
      typeof calendarId === 'string' ? calendarId : 'primary'
    );

    logger.info('Google Calendar availability check completed', { 
    ...
    logger.error('Failed to check Google Calendar availability', 
