...
async function inspectTestResults() {
  ...
  try {
    ...
    const missingInGenerator = dbTests.filter(
      (test) => !generatedTestNames.has(test.name || test.id)
    );
    ...
    if (missingInGenerator.length > 0) {
      ...
      missingInGenerator.forEach((test) => {
        ...
        console.log(
          `    Last Run: ${test.lastRun ? new Date(test.lastRun.seconds * 1000).toLocaleString() : "Never"}`
        );
