...
async function runMissingTests() {
    ...
    try {
        ...
        const dbTestNames = new Set(dbTests.map((test: { name?: string; id?: string; }) => test.name || test.id));
        ...
        const testsByType = missingTests.reduce((acc: Record<string, TestCase<Record<string, unknown>>[]>, test) => {
        ...
        console.log('\nRunning missing tests...');
        const results: TestResultData<Record<string, unknown>>[] = await runTests(missingTests, undefined, (result) => {
