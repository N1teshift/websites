...
async function runFailedTests() {
  ...
  try {
    ...
    console.log(
      `Selecting ${limitedFailedTests.length} failed tests to re-run${limit ? ` (limited to ${limit})` : ""}\n`
    );
    ...
    if (testsToRun.length === 0) {
      console.log(
        "No matching tests found to run. Check if test names match between database and registry."
      );
      return;
    }
