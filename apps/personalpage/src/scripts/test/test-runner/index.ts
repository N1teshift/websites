...
export const main = async () => {
  ...
  if (process.argv.includes("--count-only")) {
    ...
    // Count the tests
    const totalTests =
      allCoefficientTests.length + allTermTests.length + allCoefficientsTests.length;
  ...
  try {
    ...
    if (options.noFilter) {
      console.log(
        `${colors.yellow}Database filtering disabled with --no-filter option.${colors.reset}`
      );
    } else {
      ...
      if ((options.failedLast || options.passedLast) && statsCount === 0) {
        console.log(
          `${colors.red}ERROR: Cannot filter by previous test results - no test statistics available${colors.reset}`
        );
        console.log(
          `Please run tests without the --failed-last or --passed-last flags first to generate statistics.`
        );
    ...
    if (options.noFilter && (options.passedLast || options.failedLast)) {
      console.log(
        `${colors.yellow}Note: --passed-last and --failed-last options are ignored when --no-filter is used.${colors.reset}`
      );
    }
    console.log(
      "Filter options:",
      JSON.stringify({ ...options, noFilter: options.noFilter ? true : undefined })
    );

    // Filter by object type
    const selectedCoefficientTests =
      options.objectType === "coefficient" || options.objectType === undefined
        ? allCoefficientTests
        : [];

    const selectedTermTests =
      options.objectType === "term" || options.objectType === undefined ? allTermTests : [];

    const selectedCoefficientsTests =
      options.objectType === "coefficients" || options.objectType === undefined
        ? allCoefficientsTests
        : [];

    // Log the selected tests by type before applying status filters
    console.log(
      `Selected by type: ${selectedCoefficientTests.length} coefficient tests, ${selectedTermTests.length} term tests, ${selectedCoefficientsTests.length} coefficients tests`
    );

    // Count total tests selected by type filtering
    const totalSelectedByType =
      selectedCoefficientTests.length + selectedTermTests.length + selectedCoefficientsTests.length;

    // Early check - if we're filtering by type but didn't filter anything, warn user
    if (options.objectType && totalSelectedByType === 0) {
      console.log(
        `${colors.yellow}WARNING: No tests match the specified object type: ${options.objectType}${colors.reset}`
      );
    ...
    if ((options.passedLast || options.failedLast) && !options.noFilter) {
      console.log(
        `Applying ${options.failedLast ? "failed-last" : "passed-last"} filter to selected tests`
      );
      ...
      if (options.failedLast) {
        ...
        expectedFilteredCount = Object.values(testStats).filter(
          (stat) =>
            stat.lastPassed === false ||
            (stat.lastPassed === undefined && stat.lastPassed === false)
        ).length;
        console.log(
          `Based on database statistics, expecting approximately ${expectedFilteredCount} tests to match the failed-last filter`
        );
        ...
        console.log(
          `Based on database statistics, expecting approximately ${expectedFilteredCount} tests to match the passed-last filter`
        );
      }

      // Apply filters to each type of test
      const filteredCoefficientTests = filterTests(selectedCoefficientTests, options, testStats);

      const filteredTermTests = filterTests(selectedTermTests, options, testStats);

      const filteredCoefficientsTests = filterTests(selectedCoefficientsTests, options, testStats);
      ...
      console.log(
        `After status filtering: ${filteredCoefficientTests.length} coefficient, ${filteredTermTests.length} term, ${filteredCoefficientsTests.length} coefficients tests selected (total: ${filteredTestsCount})`
      );
      ...
      console.log(
        `Tests with lastPassed=true: ${Object.values(testStats).filter((stat) => stat.lastPassed === true).length}`
      );
      console.log(
        `Tests with lastPassed=false: ${Object.values(testStats).filter((stat) => stat.lastPassed === false).length}`
      );
      console.log(
        `Tests with passed=true: ${Object.values(testStats).filter((stat) => stat.lastPassed === true).length}`
      );
      console.log(
        `Tests with passed=false: ${Object.values(testStats).filter((stat) => stat.lastPassed === false).length}`
      );
      console.log(colors.cyan + "===================================" + colors.reset);

      // Safety check - if we didn't filter anything when we should have
      if (
        filteredTestsCount === totalSelectedByType &&
        expectedFilteredCount > 0 &&
        expectedFilteredCount < totalSelectedByType
      ) {
        console.log(`${colors.red}ERROR: Test filtering did not work as expected${colors.reset}`);
        console.log(
          `Expected around ${expectedFilteredCount} tests after filtering, but got ${filteredTestsCount} (same as before filtering).`
        );

        // Display a sample of tests we expected to filter out but didn't
        const failedStatsSample = Object.values(testStats)
          .filter((stat) =>
            options.failedLast ? stat.lastPassed === false : stat.lastPassed === true
          )
          .slice(0, 5);
        ...
        const answer = readlineSync.question(
          `Do you want to continue running all ${filteredTestsCount} tests anyway? (y/n): `
        );
    ...
    if (options.countOnly) {
      ...
      console.log(
        `- Coefficient tests: ${testsToRun.filter((t) => t.objectType === "coefficient").length}`
      );
      console.log(`- Term tests: ${testsToRun.filter((t) => t.objectType === "term").length}`);
      console.log(
        `- Coefficients tests: ${testsToRun.filter((t) => t.objectType === "coefficients").length}`
      );
      ...
      if (options.passedLast && !options.noFilter) {
        ...
        console.log(
          `${colors.yellow}Note: Pass/fail filters were ignored because --no-filter was specified${colors.reset}`
        );
      }
    ...
    testsToRun.forEach((test, i) => {
      if (i < 10 || testsToRun.length <= 20) {
        console.log(
          `${i + 1}. ${test.name}: "${(test as TestCase<Record<string, unknown>>).prompt}"`
        );
    ...
    const testResults = await runTests(testsToRun, undefined, (result) => {
      console.log(
        `Test completed: ${result.test.name} - ${result.passed ? colors.green + "PASS" : colors.red + "FAIL"}${colors.reset}`
      );
    });
