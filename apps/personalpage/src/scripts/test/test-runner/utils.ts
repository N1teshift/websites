...
export const printResults = (results: TestResultData<Record<string, unknown>>[]) => {
  ...
  console.log(`${passRate >= 75 ? colors.green : passRate >= 50 ? colors.yellow : colors.red}Pass rate: ${passRate.toFixed(2)}%${colors.reset}`);
 ...
 */
export const findMatchingTestStat = (test: TestCase<Record<string, unknown>>, stats: Record<string, TestStat>, debugMatching?: boolean): TestStat | null => {
  ...
  for (const key in stats) {
    ...
    if (
      ...
      (stat.name && stat.name.toLowerCase() === test.name.toLowerCase()) &&
      (stat.category && stat.category.toLowerCase() === test.objectType.toLowerCase())
    ) {
      if (debugMatching) console.log(`    Found match by iterating stats: ${key} (name:${stat.name}, type:${stat.category})`);
      return stat;
    }
  }
  ...
  if (debugMatching) {
    ...
    for (const key in stats) {
      const stat = stats[key];
      if ((stat.category === test.objectType) && 
          stat.name && sameTypeTests < 5) {
