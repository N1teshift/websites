...
import {
  ...
  TestResultData,
  TestStat,
} from "../../../features/modules/math/types/testsTypes";
  ...
  try {
    ...
    if (response.success && response.data?.tests) {
      ...
      if (response.data.tests.length > 0) {
        console.log(
          "Sample test stat (first entry):",
          JSON.stringify(response.data.tests[0], null, 2)
        );

        // Check if the lastPassed field exists in any entries
        const hasLastPassed = response.data.tests.some(
          (stat: unknown) => (stat as TestStat).lastPassed !== undefined
        );
        const hasPassed = response.data.tests.some(
          (stat: unknown) => (stat as TestStat).lastPassed !== undefined
        );
        console.log(
          `Status fields present in data: lastPassed=${hasLastPassed}, passed=${hasPassed}`
        );
        ...
        const failedTests = response.data.tests.filter((stat: unknown) => {
          ...
          return (
            testStat.lastPassed === false ||
            (testStat.lastPassed === undefined && testStat.lastPassed === false)
          );
        }).length;
...
export const saveResults = async (results: TestResultData<Record<string, unknown>>[]) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.log(
        colors.red +
          `Error saving results: ${response.status} ${response.statusText}` +
          colors.reset
      );
      return;
    }
    ...
    console.log(
      colors.green +
        `Results saved successfully! Run ID: ${data?.runId || "unknown"}` +
        colors.reset
    );
  } catch (error) {
