...
export const analyzeTestStats = (stats: Record<string, TestStat>, statsCount: number): void => {
  ...
  sampleStats.forEach((stat, i) => {
    ...
    console.log(
      `- Pass Status: ${JSON.stringify(stat.lastPassed !== undefined ? stat.lastPassed : stat.lastPassed !== undefined ? stat.lastPassed : "unknown")}`
    );
  ...
  const withLastPassed = Object.values(stats).filter(
    (stat) => stat.lastPassed !== undefined
  ).length;
  ...
  const failedTestsCount = Object.values(stats).filter(
    (stat) =>
  ...
  const sortedDbTypes = Array.from(dbObjectTypes.entries()).sort((a, b) => b[1] - a[1]);
...
export const analyzeGeneratedTests = (
  ...
  const allGeneratedTests = [...allCoefficientTests, ...allTermTests, ...allCoefficientsTests];
  ...
  // Sort by count, descending
  const sortedGenTypes = Array.from(generatedTestTypes.entries()).sort((a, b) => b[1] - a[1]);
...
export const compareTestTypes = (
  ...
  if (noFilter) {
    ...
    console.log(
      `${colors.yellow}No database comparison available (--no-filter option used).${colors.reset}`
    );
    ...
    allTypes.forEach((type) => {
      const dbCount =
        statsCount > 0 ? Object.values(stats).filter((stat) => stat.category === type).length : 0;
      ...
      if (dbCount > 0 && genCount > 0) {
        ...
        console.log(
          `${colors.yellow}Type "${type}" exists in DB (${dbCount} tests) but not in generated tests${colors.reset}`
        );
      } else if (genCount > 0) {
        console.log(
          `${colors.yellow}Type "${type}" exists in generated tests (${genCount} tests) but not in DB${colors.reset}`
        );
      }
    });
  }
