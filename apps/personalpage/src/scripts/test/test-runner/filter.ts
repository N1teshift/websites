 ...
 */
export const filterTests = (tests: TestCase<Record<string, unknown>>[], options: FilterOptions, stats: Record<string, TestStat>) => {
  console.log(`\n=== Test Filtering ===`);
  ...
  console.log(`Found ${passedTestNames.size} passed tests and ${failedTestNames.size} failed tests in database`);
  ...
  if (failedTestNames.size > 0) {
    ...
    Array.from(failedTestNames).slice(0, 5).forEach((name, i) => {
  ...
  for (const test of tests) {
    ...
    if (statMatch) {
      ...
      if (isPassed !== undefined && 
        ...
        if (options.debugMatching) {
          console.log(`  ${colors.green}✓ Strategy 2: findMatchingTestStat succeeded${colors.reset}`);
          console.log(`    - Matched with stat: id=${statMatch.id}, name=${statMatch.name}, lastPassed=${statMatch.lastPassed}`);
        }
        continue;
      } else if (options.debugMatching) {
        console.log(`  ${colors.yellow}~ Strategy 2: findMatchingTestStat found a match, but pass status doesn't match filter${colors.reset}`);
        console.log(`    - Found stat: id=${statMatch.id}, name=${statMatch.name}`);
        console.log(`    - Pass status: ${isPassed}, Filter: ${options.failedLast ? 'failed' : 'passed'}`);
      }
    } else if (options.debugMatching) {
      console.log(`  ${colors.red}✗ Strategy 2: findMatchingTestStat failed - no match found${colors.reset}`);
    }
    ...
    for (const nameVar of nameVariations) {
      ...
      for (const dbName of Array.from(targetSet)) {
        ...
        if (options.debugMatching) {
          console.log(`    Checking against DB name: "${dbName}" (normalized: "${dbNameNormalized}")`);
        }
        
        if (nameVar === dbNameNormalized || 
    ...
    if (!matched && options.debugMatching) {
      ...
      console.log(`  ${colors.red}✗ FINAL RESULT: Test excluded - no matching record found in ${options.failedLast ? 'failed' : 'passed'} tests${colors.reset}`);
    }
  }
  
  console.log(`After filtering: ${filteredTests.length} tests match ${options.failedLast ? 'failed' : 'passed'} criteria`);
  ...
  if (filteredTests.length === 0 && tests.length > 0) {
    ...
    for (let i = 0; i < Math.min(testCodeNames.length, dbTestNames.length); i++) {
      ...
      if (codeName && dbName) {
        ...
        const codeNormalized = codeName.replace(/\s/g, '').replace(/[\[\]\(\)]/g, '').toLowerCase();
        const dbNormalized = dbName.replace(/\s/g, '').replace(/[\[\]\(\)]/g, '').toLowerCase();
