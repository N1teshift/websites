...
import { CLASS_SHEET_MAPPING, STANDARD_COLUMNS } from '../src/features/modules/edtech/progressReport/student-data/config/columnMapping';
 ...
 */
async function readExcelData(excelPath: string): Promise<Map<string, Map<string, ExcelStudentData>>> {
    ...
    for (const [sheetName, className] of Object.entries(CLASS_SHEET_MAPPING)) {
        ...
        worksheet.eachRow((row, rowNumber) => {
            ...
            if (resolvedName.firstName !== String(firstName).trim()) {
                Logger.info(`Resolved name alias: ${firstName} ${lastName} → ${resolvedName.firstName} ${resolvedName.lastName}`);
            }
            
            Logger.info(`Loaded: ${studentKey} - SD1[P:${sd1_p}, MYP:${sd1_myp}, C1:${sd1_c1}, C2:${sd1_c2}], SD2[P:${sd2_p}, MYP:${sd2_myp}, C1:${sd2_c1}, C2:${sd2_c2}], SD3[P:${sd3_p}, MYP:${sd3_myp}, C1:${sd3_c1}, C2:${sd3_c2}]`);
        });
...
async function overrideSummativeData(
    ...
    for (const student of masterData.students) {
        ...
        if (!excelStudent) {
            Logger.warn(`Student not found in Excel: ${student.first_name} ${student.last_name} (${className})`);
            continue;
        }
        ...
        for (const assessment of student.assessments) {
            if (assessment.column === 'SD1') {
                ...
                if (updated) {
                    ...
                    assessmentsUpdated++;
                    Logger.info(`Updated SD1 for ${student.first_name} ${student.last_name}: P=${excelStudent.SD1_P}, MYP=${excelStudent.SD1_MYP}, C1=${excelStudent.SD1_C1}, C2=${excelStudent.SD1_C2}`);
                }
            }
                ...
                if (updated) {
                    studentModified = true;
                    assessmentsUpdated++;
                    Logger.info(`Updated SD2 for ${student.first_name} ${student.last_name}: P=${excelStudent.SD2_P}, MYP=${excelStudent.SD2_MYP}, C1=${excelStudent.SD2_C1}, C2=${excelStudent.SD2_C2}`);
                }
            }
                ...
                if (updated) {
                    studentModified = true;
                    assessmentsUpdated++;
                    Logger.info(`Updated SD3 for ${student.first_name} ${student.last_name}: P=${excelStudent.SD3_P}, MYP=${excelStudent.SD3_MYP}, C1=${excelStudent.SD3_C1}, C2=${excelStudent.SD3_C2}`);
                }
            }
        }
...
async function main() {
    ...
    if (args.length < 3) {
        ...
        console.log('Example: npx tsx scripts/overrideSummativeData.ts stud_data.xlsx master_student_data_main_v4_51.json master_student_data_main_v4_52.json');
        process.exit(1);
    }
