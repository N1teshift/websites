...
class AssessmentAnalyzer {
    ...
    async analyze(filePath: string): Promise<void> {
        ...
        try {
            ...
            if (this.options.filterClass) {
                ...
                console.log(`Filtered to class: ${this.options.filterClass} (${students.length} students)\n`);
            }
    ...
    private summarize(students: StudentData[]): void {
        ...
        console.log('By Type:');
        for (const [type, assessments] of Object.entries(byType).sort((a, b) => b[1].length - a[1].length)) {
        ...
        const numericScores = withScores
        ...
        for (const [type, assessments] of Object.entries(byType).sort((a, b) => b[1].length - a[1].length)) {
            ...
            if (type === 'test' || type === 'summative') {
                ...
                if (withDetails.length > 0) {
                    const withPercentage = withDetails.filter(a => a.evaluation_details?.percentage_score != null).length;
                    const withMyp = withDetails.filter(a => a.evaluation_details?.myp_score != null).length;
                    const withCambridge = withDetails.filter(a => a.evaluation_details?.cambridge_score != null).length;
    ...
    private deepAnalysis(students: StudentData[]): void {
        ...
        const completionData = students.map(student => {
            ...
            const withScores = assessments.filter(a => a.score && a.score !== '' && a.score !== 'n').length;
        ...
        if (this.options.showMissingOnly) {
            ...
            for (const student of incomplete) {
                ...
                console.log(`  Completed: ${student.completed}/${student.total} (${student.rate.toFixed(1)}%)\n`);
            }
            ...
            completionData.slice(-10).reverse().forEach((s, i) => {
                console.log(`  ${(i + 1).toString().padStart(2)}. ${s.name.padEnd(30)} ${s.completed}/${s.total} (${s.rate.toFixed(1)}%)`);
            });
        ...
        for (const column of sortedColumns) {
            ...
            console.log(`  ${indicator} ${column.padEnd(15)} ${studentsWithColumn}/${students.length} (${coverage.toFixed(1)}%)`);
        }
    }
