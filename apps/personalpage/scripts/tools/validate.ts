...
class DataValidator {
    ...
    private validateStudent(student: StudentData): void {
        ...
        // Quick checks
        if (!student.id) {
            this.addIssue('error', 'missing_id', studentName, undefined,
                'Student missing ID', false);
        }

        if (!student.assessments || student.assessments.length === 0) {
            this.addIssue('warning', 'no_assessments', studentName, undefined,
                'Student has no assessments', false);
            return;
        }
    ...
    private quickValidation(student: StudentData, assessment: Assessment, index: number): void {
        ...
        if (!assessment.assessment_id) {
            this.addIssue('error', 'missing_field', studentName, assessmentLabel,
                'Missing assessment_id', true);
    ...
    private standardValidation(student: StudentData, assessment: Assessment, index: number): void {
        ...
        if (!assessment.assessment_title) {
            this.addIssue('warning', 'missing_field', studentName, assessmentLabel,
                'Missing assessment_title', true);
        ...
        if (isHomework && isOldND && isBinaryScore && missingOnTime) {
            this.addIssue('warning', 'old_format', studentName, assessmentLabel,
                'Old ND format: binary score should be in on_time field', true);
    ...
    private deepValidation(student: StudentData, assessment: Assessment, index: number): void {
        ...
        if (assessment.assessment_id && !this.taskNameMatchesConvention(assessment)) {
            this.addIssue('info', 'naming_convention', studentName, assessmentLabel,
                `task_name "${assessment.task_name}" doesn't match convention`, true);
        ...
        if (['test', 'summative'].includes(assessment.type)) {
            if (!assessment.evaluation_details) {
                this.addIssue('info', 'missing_details', studentName, assessmentLabel,
                    'Missing evaluation_details for test/summative', false);
            }
        }
    }
...
Examples:
  ...
  npx tsx scripts/tools/validate.ts data.json --fix --no-dry-run
    ...
    console.log(`Mode: ${options.dryRun ? 'Dry-run (no changes)' : options.autoFix ? 'Auto-fix' : 'Report only'}\n`);
