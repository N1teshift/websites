...
function detectColumnPositions(worksheet: ExcelJS.Worksheet): SheetColumnMap {
  ...
  for (const range of unitRanges) {
    ...
    for (let col = range.startCol; col <= range.endCol && col <= 120; col++) {
      ...
      if (header.includes("listen") && (header.includes("1") || header.includes("and 2"))) {
        ...
        (header.includes("voc") && header.includes("1")) ||
        (header.includes("voc") && header.includes("and 2"))
      ) {
        ...
        (header.includes("gr") || header.includes("grammar")) &&
        !header.includes("1") &&
        !header.includes("2") &&
        !header.includes("3")
      ) {
  ...
  for (let col = 1; col <= 120; col++) {
    ...
    if (inDiag1) {
      if ((header.includes("reading") || header.includes("1 paper")) && !colMap.d1_p1)
        colMap.d1_p1 = col;
      else if ((header.includes("listening") || header.includes("2paper")) && !colMap.d1_p2)
        colMap.d1_p2 = col;
      else if ((header.includes("writing") || header.includes("3 paper")) && !colMap.d1_p3)
        colMap.d1_p3 = col;
      else if (header.includes("total") && !colMap.d1_total) colMap.d1_total = col;
      else if (header === "%" && !foundD1Percent) {
        colMap.d1_percent = col;
        foundD1Percent = true;
      }
    } else if (inDiag2) {
      if ((header.includes("reading") || header.includes("1 paper")) && !colMap.d2_p1)
        colMap.d2_p1 = col;
      else if ((header.includes("listening") || header.includes("2paper")) && !colMap.d2_p2)
        colMap.d2_p2 = col;
      else if ((header.includes("writing") || header.includes("3 paper")) && !colMap.d2_p3)
        colMap.d2_p3 = col;
      else if (header.includes("total") && !colMap.d2_total) colMap.d2_total = col;
      else if (header === "%" && !foundD2Percent) {
        colMap.d2_percent = col;
        foundD2Percent = true;
      }
    } else if (inDiag3) {
      if ((header.includes("reading") || header.includes("1 paper")) && !colMap.d3_p1)
        colMap.d3_p1 = col;
      else if ((header.includes("listening") || header.includes("2paper")) && !colMap.d3_p2)
        colMap.d3_p2 = col;
      else if ((header.includes("writing") || header.includes("3 paper")) && !colMap.d3_p3)
        colMap.d3_p3 = col;
      else if (header.includes("total") && !colMap.d3_total) colMap.d3_total = col;
      else if (header === "%" && !foundD3Percent) {
        colMap.d3_percent = col;
        foundD3Percent = true;
      }
    }
  }
