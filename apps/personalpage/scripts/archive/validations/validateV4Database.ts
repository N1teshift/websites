...
async function validateDatabase() {
  try {
    ...
    console.log(
      `✓ Assessments with assessment_id: ${hasAssessmentId} (${((hasAssessmentId / totalAssessments) * 100).toFixed(1)}%)`
    );
    console.log(
      `✓ Assessments with assessment_title: ${hasAssessmentTitle} (${((hasAssessmentTitle / totalAssessments) * 100).toFixed(1)}%)`
    );
    ...
    data.students.forEach((student) => {
      const studentHomework = student.assessments.filter((a) => homeworkTypes.includes(a.type));
    ...
    const studentsWithCorrectHomework = Array.from(homeworkCounts.values()).filter(
      (c) => c === expectedHomework
    ).length;

    console.log(`\n📊 Homework Check:`);
    console.log(
      `✓ Students with ${expectedHomework} homework records: ${studentsWithCorrectHomework}/${data.students.length}`
    );
