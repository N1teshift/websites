...
import {
  StudentDataV3,
  LearningLevel,
} from "../../../src/features/modules/edtech/progressReport/student-data/types/StudentDataTypesV3";

const DATA_DIR = path.join(process.cwd(), "src", "features", "student-data", "data");
const BACKUP_DIR = path.join(
  process.cwd(),
  "backups",
  `v2_backup_${new Date().toISOString().split("T")[0]}`
);
...
function calculateComputedMetrics(v2Data: StudentData): any {
  ...
  const hwCompletion =
    homework.length > 0
      ? homework.filter((a) => parseFloat(a.score) > 0).length / homework.length
      : 0;
  ...
  const matCompletion =
    materials.length > 0
      ? materials.reduce((sum: number, m: any) => sum + m.percentage, 0) / (materials.length * 100)
      : 0;
...
function migrateStudent(v2Data: StudentData, id: string): StudentDataV3 {
  ...
  const v3Data: StudentDataV3 = {
    ...
    communication:
      v2Data.reporting_checkpoints && v2Data.reporting_checkpoints.length > 0
        ? {
            parent_contacts: [],
            teacher_notes: [],
    ...
    conduct:
      v2Data.conduct_notes && v2Data.conduct_notes.length > 0
        ? {
  ...
  try {
    ...
    for (const [className, students] of Object.entries(studentsByClass)) {
      ...
      students.forEach(({ file, data }, index) => {
        ...
        console.log(
          `  ✅ ${data.first_name} ${data.last_name} → ${id} (${oldSize}B → ${newSize}B)`
        );
        migrated++;
      });
    }
