...
async function migrate() {
    ...
    try {
        ...
        if (v3Data.metadata.schema_version !== '3.0') {
            console.warn(`⚠️  Warning: Expected schema version 3.0, got ${v3Data.metadata.schema_version}`);
        }
        ...
        console.log(`   Total assessments: ${v4Students.reduce((sum, s) => sum + s.assessments.length, 0)}`);
        console.log(`\n📝 Changes made:`);
