...
class DatabaseValidator {
    ...
    private validateStudent(student: StudentData, autoFix: boolean) {
        ...
        if (!student.assessments || student.assessments.length === 0) {
            this.addIssue('warning', 'missing_data', studentName, undefined, 
    ...
    private validateAssessment(
        ...
        if (assessment.assessment_id && 
            ...
            this.addIssue('warning', 'wrong_format', studentName, assessmentLabel,
                `assessment_id has prefix: ${assessment.assessment_id}`, true);
            
            if (autoFix) {
                const oldId = assessment.assessment_id;
                assessment.assessment_id = oldId
                    .replace('classwork-', '')
                    .replace('test-', '');
                this.fixesApplied++;
            }
        ...
        if (assessment.assessment_title === 'Classwork' && assessment.column?.match(/^EXT\d+$/)) {
            this.addIssue('info', 'generic_title', studentName, assessmentLabel,
                'Generic "Classwork" title for EXT column', true);
        ...
        if (isHomework && isOldND && isBinaryScore && missingOnTime) {
            this.addIssue('warning', 'old_structure', studentName, assessmentLabel,
        ...
        if (assessment.assessment_id && !this.taskNameMatchesConvention(assessment)) {
            this.addIssue('info', 'task_name_mismatch', studentName, assessmentLabel,
                `task_name "${assessment.task_name}" doesn't match V5 convention`, true);
