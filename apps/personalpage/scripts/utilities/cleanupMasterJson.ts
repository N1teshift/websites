...
function convertAssessmentToV4(assessment: Assessment): Assessment {
    ...
    if (!v4Assessment.assessment_title) {
        v4Assessment.assessment_title = generateAssessmentTitle(assessment.column, assessment.task_name);
    }
...
async function cleanup() {
    ...
    try {
        ...
        if (args.length < 1) {
            ...
            console.log('\nUsage: npx tsx scripts/cleanupMasterJson.ts [input-file] [output-file]');
            console.log('Example: npx tsx scripts/cleanupMasterJson.ts progress_report_data_2025-11-03_v5.json progress_report_data_v4_clean.json\n');
            process.exit(1);
        }
        ...
        // Create backup
        const backupPath = inputPath.replace('.json', `_backup_${new Date().toISOString().split('T')[0]}.json`);
        ...
        for (const student of v4Students) {
            ...
            for (const assessment of student.assessments) {
                ...
                if (assessment.column.match(/^SD\d+$/i) && assessment.type === 'summative') finalCheck.sdSummative++;
        ...
        console.log(`   Total assessments: ${v4Students.reduce((sum, s) => sum + (s.assessments?.length || 0), 0)}`);
        ...
        if (finalCheck.participation > 0 || finalCheck.sdSummative > 0 || finalCheck.summativeDetails > 0 || finalCheck.missingIds > 0) {
