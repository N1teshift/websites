...
async function readCambridgeSheets(excelPath: string): Promise<CambridgeSheetData[]> {
    ...
    for (const [sheetName, className] of Object.entries(CLASS_SHEET_MAPPING)) {
        ...
        row1.eachCell((cell, colNumber) => {
            if (colNumber > 3) { // Skip ID, Vardas, Pavardė
...
function findStudent(
    ...
    if (bestMatch) {
        Logger.warn(`Fuzzy match: "${firstName} ${lastName}" -> "${bestMatch.first_name} ${bestMatch.last_name}" (${(bestScore * 100).toFixed(0)}%)`);
    }
...
function buildObjectiveHistory(
    ...
    for (const assessment of student.assessments) {
        const assessmentId = assessment.assessment_id?.toUpperCase() || 
                           assessment.column?.toUpperCase();
 ...
 */
async function importCambridgeObjectives(
    excelPath: string,
    jsonPath: string,
    outputPath: string
) {
    ...
    for (const classData of cambridgeData) {
        ...
        for (const excelStudent of classData.students) {
            ...
            if (!jsonStudent) {
                Logger.warn(`  ❌ Student not found: ${excelStudent.firstName} ${excelStudent.lastName} (resolved to: ${resolvedName.firstName} ${resolvedName.lastName})`);
                unmatchedCount++;
                continue;
            }
            ...
            Logger.info(`  ✅ ${jsonStudent.first_name} ${jsonStudent.last_name}: ${classData.objectives.length} objectives`);
        }
    }
...
importCambridgeObjectives(excelPath, jsonPath, outputPath)
    .catch(error => {
