...
const BUILD_DIR = path.join(path.dirname(path.dirname(fileURLToPath(import.meta.url))), "build");
...
function shouldSkipImport(importPath) {
  ...
  return EXTERNAL_PACKAGES.some((pkg) => importPath.includes(pkg) || importPath === pkg);
}
...
function fixImports(content, filePath) {
  ...
  for (const pattern of IMPORT_PATTERNS) {
    updatedContent = updatedContent.replace(pattern, (match, start, importPath, end) => {
      ...
      hasChanges = true;
      console.log(
        `Added .js extension in ${filePath}:\n  From: ${importPath}\n  To:   ${importPath}.js`
      );
