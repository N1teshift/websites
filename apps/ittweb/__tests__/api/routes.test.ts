...
const scheduledService = jest.requireMock(
  "@/features/modules/scheduled-games/lib/scheduledGameService"
);
const entryService = jest.requireMock("@/features/modules/entries/lib/entryService");
const standingsService = jest.requireMock(
...
describe("Games API", () => {
  test("GET /api/games returns games with filters", async () => {
    ...
    expect(gameService.getGames).toHaveBeenCalledWith(
      expect.objectContaining({ gameId: 5, limit: 10 })
    );
...
describe("Standings and Analytics API", () => {
  ...
  test("GET /api/analytics/elo-history returns data", async () => {
    ...
    const { json } = await runHandler(
      handlerAnalyticsEloHistory,
      createMockRequest({ method: "GET", query: { playerName: "P", category: "1v1" } })
    );
    expect(json).toHaveBeenCalledWith({ success: true, data: [] });
  });
...
describe("User API", () => {
  ...
  test("GET /api/user/data-notice-status returns status", async () => {
    (getServerSession as jest.Mock).mockResolvedValue(mockSession);
    userDataService.getUserDataByDiscordId.mockResolvedValue({
