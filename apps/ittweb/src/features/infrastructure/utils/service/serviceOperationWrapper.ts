...
export async function withServiceOperation<T>(
  ...
  const { provideTimestampFactory = false, context = {}, errorMessage, rethrow = true } = options;
...
export function withServiceOperationSync<T>(
  ...
  const { context = {}, errorMessage, rethrow = true } = options;
...
export async function withServiceOperationNullable<T>(
  ...
  const { provideTimestampFactory = false, context = {}, errorMessage } = options;
