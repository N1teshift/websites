...
describe("PostDeleteDialog", () => {
  ...
  it("renders dialog with post title when provided", () => {
    ...
    expect(
      screen.getByText(
        '"Test Post Title" will be permanently deleted. This action cannot be undone.'
      )
    ).toBeInTheDocument();
  });
  ...
  it("renders dialog without post title when not provided", () => {
    ...
    expect(
      screen.getByText("This post will be permanently deleted. This action cannot be undone.")
    ).toBeInTheDocument();
  });
