...
import { transformPostDoc, sortPostsByDate } from "./postService.helpers";
  ...
  return queryWithIndexFallback({
    ...
    executeQuery: async () => {
      let adminQuery:
        ...
        adminDb.collection(POSTS_COLLECTION);

      if (!includeUnpublished) {
        adminQuery = adminQuery.where("published", "==", true) as ReturnType<
          ReturnType<typeof adminDb.collection>["where"]
        >;
      }

      adminQuery = adminQuery.orderBy("date", "desc") as ReturnType<
        ReturnType<ReturnType<typeof adminDb.collection>["where"]>["orderBy"]
      >;
