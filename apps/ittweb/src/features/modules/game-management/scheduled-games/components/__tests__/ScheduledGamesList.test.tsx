...
jest.mock("../ScheduledGameCard", () => ({
  ScheduledGameCard: ({
    ...
    <div data-testid={`scheduled-game-card-${game.id}`}>
      ...
      {onGameClick && (
        <button onClick={() => onGameClick(game)} data-testid={`game-click-${game.id}`}>
          View Game
        </button>
      )}
      {onJoin && (
        <button onClick={() => onJoin(game.id)} data-testid={`join-${game.id}`}>
          Join
        </button>
      )}
      {onLeave && (
        <button onClick={() => onLeave(game.id)} data-testid={`leave-${game.id}`}>
          Leave
        </button>
      )}
      {onEdit && userIsAdmin && (
        <button onClick={() => onEdit(game)} data-testid={`edit-${game.id}`}>
          Edit
        </button>
      )}
      {onRequestDelete && userIsAdmin && (
        <button onClick={() => onRequestDelete(game)} data-testid={`delete-${game.id}`}>
          Delete
        </button>
      )}
      {onUploadReplay && (
        <button onClick={() => onUploadReplay(game)} data-testid={`upload-${game.id}`}>
          Upload Replay
        </button>
      )}
      ...
      {isUploadingReplay === game.id && (
        <span data-testid={`uploading-${game.id}`}>Uploading...</span>
      )}
    </div>
  ),
}));
