...
export function deriveWinningTeamId(
  ...
  const playersWithResult = players.filter((p): p is PlayerWithResult => {
    ...
    return (
      playerWithResult.result !== undefined ||
      ...
      if (
        (normalizedKey.includes("winner") ||
          ...
          (normalizedKey === "result" &&
            (normalizedValue.includes("win") || normalizedValue === "1"))) &&
        ...
        const matchedPlayer = players.find((p) => {
          ...
          return (
            normalizedName === normalizedMissionKey ||
...
export function deriveFlag(
  ...
  if (
    playerWithResult.result === "win" ||
    playerWithResult.status === "winner" ||
    playerWithResult.won === true
  ) {
    return "winner";
  }
  if (
    playerWithResult.result === "loss" ||
    playerWithResult.status === "loser" ||
    playerWithResult.won === false
  ) {
    return "loser";
  }
  ...
  for (const [missionKey, missionData] of Object.entries(w3mmdLookup)) {
    const normalizedMissionKey = missionKey.toLowerCase().replace(/[^a-z0-9]/g, "");
    const matchesPlayer = missionKeys.some(
      (mk) =>
