...
describe("updateEloScores", () => {
  ...
  it("updates winners and losers with calculated changes", async () => {
    ...
    expect(payloads).toContainEqual({
      elochange: expect.closeTo(15.08, 2),
      eloBefore: 1000,
      eloAfter: expect.closeTo(1015.08, 2),
    });
    expect(payloads).toContainEqual({
      elochange: expect.closeTo(-15.08, 2),
      eloBefore: 980,
      eloAfter: expect.closeTo(964.92, 2),
    });
...
describe("recalculateFromGame", () => {
  ...
  it("throws error when game not found", async () => {
    ...
    await expect(recalculateFromGame("game-1")).rejects.toThrow(
      "Game not found or invalid: game-1"
    );
  });
  ...
  it("throws error when game is not completed", async () => {
    ...
    await expect(recalculateFromGame("game-1")).rejects.toThrow(
      "Can only recalculate ELO for completed games"
    );
  });
});
