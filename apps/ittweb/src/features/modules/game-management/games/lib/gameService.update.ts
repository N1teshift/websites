/**
 * Game Service - Update Operations (Client Stub)
 *
 * This file is a client-side stub. The actual server-only implementation
 * is in gameService.update.server.ts
 *
 * These functions should only be called from API routes (server-side).
 * Client code should use API endpoints instead.
 */

import type { UpdateGame } from "../types";

/**
 * Update a game
 * @throws Error - This function is server-only. Use API routes instead.
 */
export async function updateGame(_id: string, _updates: UpdateGame): Promise<void> {
  throw new Error("updateGame is server-only. Use /api/games/[id] API endpoint instead.");
}
