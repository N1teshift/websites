...
function StatCard({
  ...
  return (
    <div className="bg-black/20 rounded-lg p-4 border border-amber-500/10 text-center">
      ...
      <div className={`text-xl font-bold ${colorClass}`}>{value.toLocaleString()}</div>
...
export function ITTStatsOverview({ category, startDate, endDate }: ITTStatsOverviewProps) {
  ...
  const hasData =
    stats.totalGames > 0 || stats.totalDamageDealt > 0 || stats.totalAnimalKills?.total > 0;
  ...
  return (
    <Card variant="medieval" className="p-6">
      <h2 className="text-xl font-semibold text-amber-400 mb-4">Community ITT Statistics</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
        ...
        <StatCard
          icon="💚"
          label="Self Healing"
          value={stats.totalHealing?.selfHealing || 0}
          colorClass="text-green-400"
        />
        <StatCard
          icon="💙"
          label="Ally Healing"
          value={stats.totalHealing?.allyHealing || 0}
        ...
        <StatCard
          ...
          label="Gold Acquired"
          value={stats.totalGoldAcquired || 0}
