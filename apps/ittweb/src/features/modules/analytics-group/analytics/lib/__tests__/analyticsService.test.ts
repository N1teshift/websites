...
describe("analyticsService", () => {
  ...
  describe("getActivityData", () => {
    ...
    it("should filter by player name", async () => {
      ...
      // Assert
      expect(getGames).toHaveBeenCalledWith(expect.objectContaining({ player: "TestPlayer" }));
    });
    ...
    it("should filter by category", async () => {
      ...
      // Assert
      expect(getGames).toHaveBeenCalledWith(expect.objectContaining({ category: "category1" }));
    });
  ...
  describe("getWinRateData", () => {
    ...
    it("should aggregate across all players when no player specified", async () => {
      ...
      (getGameById as jest.Mock).mockResolvedValue({
        players: [{ flag: "winner" }, { flag: "loser" }, { flag: "drawer" }],
      });
