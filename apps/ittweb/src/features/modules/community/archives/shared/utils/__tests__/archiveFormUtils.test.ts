...
describe("archiveFormUtils", () => {
  ...
  describe("normalizeSectionOrder", () => {
    ...
    it("returns canonical order for empty input", () => {
      expect(normalizeSectionOrder()).toEqual([
        "images",
        "video",
        "twitch",
        "replay",
  ...
  describe("extractFilenameFromUrl", () => {
    ...
    it("handles URLs with both query and fragment", () => {
      expect(extractFilenameFromUrl("https://example.com/file.png?param=value#section")).toBe(
        "file.png"
      );
    });

    it("handles URL encoded filenames", () => {
      expect(extractFilenameFromUrl("https://example.com/files/hello%20world.txt")).toBe(
        "hello world.txt"
      );
    });

    it("handles multiple URL encodings", () => {
      expect(extractFilenameFromUrl("https://example.com/files/hello%20world%21.txt")).toBe(
        "hello world!.txt"
      );
    });
