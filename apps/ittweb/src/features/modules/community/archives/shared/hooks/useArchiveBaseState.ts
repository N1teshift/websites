...
export function useArchiveBaseState(mode: "create" | "edit", initialEntry?: ArchiveEntry) {
  ...
  const [existingReplayUrl, setExistingReplayUrl] = useState<string>(initialEntry?.replayUrl || "");
  ...
  useEffect(() => {
    if (mode === "edit" && initialEntry) {
      ...
      const dateType: "single" | "undated" =
        initialEntry.dateInfo.type === "undated" ? "undated" : "single";
      ...
      setFormData({
        ...
        mediaType: initialEntry.videoUrl
          ? "video"
          : initialEntry.images?.length
            ? "image"
      ...
      const initialImages =
        initialEntry.images && initialEntry.images.length > 0 ? initialEntry.images : [];
      setCurrentImages(initialImages);
      const initialOrder = normalizeSectionOrder(
        initialEntry.sectionOrder as SectionKey[] | undefined
      );
      setSectionOrder(initialOrder);
      setExistingReplayUrl(initialEntry.replayUrl || "");
    }
  }, [mode, initialEntry]);
