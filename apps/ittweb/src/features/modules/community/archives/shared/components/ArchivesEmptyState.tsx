...
const ArchivesEmptyState: React.FC<ArchivesEmptyStateProps> = memo(
  ({ isAuthenticated, onAddClick, onSignInClick }) => {
    return (
      <div className="text-center py-12">
        <div className="bg-black/30 backdrop-blur-sm border border-amber-500/30 rounded-lg p-8">
          <h3 className="font-medieval-brand text-2xl mb-4 text-amber-400">No Archives Yet</h3>
          <p className="text-gray-300 mb-6">
            Be the first to contribute to our archives! Share your memories, screenshots, or videos
            from your Island Troll Tribes experience.
          </p>
          {isAuthenticated ? (
            <button
              onClick={onAddClick}
