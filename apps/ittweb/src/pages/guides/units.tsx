...
function UnitCard({ unit }: { unit: UnitData }) {
  ...
  const icon = <GuideIcon category="units" name={unit.name} size={48} />;
          ...
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-6 text-amber-100">
            ...
            <p className="text-sm text-amber-100/90">
              ...
              <code className="px-1 py-0.5 bg-black/40 rounded text-amber-200">
                node scripts/convert-extracted-to-typescript.mjs
              </code>{" "}
              to rebuild{" "}
              <code className="px-1 py-0.5 bg-black/40 rounded text-amber-200">allUnits.ts</code>.
            </p>
          </div>
        </div>
      </ErrorBoundary>
