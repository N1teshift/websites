...
function ItemCard({ item, category }: { item: ItemData; category?: ItemCategory | "all" }) {
  ...
  const craftedAtIcon = craftedAtBuilding ? (
    <IconWithTooltip tooltipText={craftedAtBuilding.name}>
      ...
      >
        <GuideIcon category="units" name={craftedAtBuilding.name} size={32} />
      </button>
    </IconWithTooltip>
  ) : null;

  const recipeIcons = (item.recipe || [])
    .map((ingredientSlug, index) => {
  ...
  if (item.stats) {
    if (typeof item.stats.damage === "number")
      statBadges.push({ label: `Damage: ${item.stats.damage}`, variant: "red" });
    if (typeof item.stats.armor === "number")
      statBadges.push({ label: `Armor: ${item.stats.armor}`, variant: "blue" });
    if (typeof item.stats.health === "number")
      statBadges.push({ label: `Health: +${item.stats.health}`, variant: "green" });
    if (typeof item.stats.mana === "number")
      statBadges.push({ label: `Mana: +${item.stats.mana}`, variant: "purple" });
  }
  ...
  return (
    <div className="mb-8">
      ...
      >
        ...
        <span
          className={`text-amber-400 transform transition-transform ${expanded ? "rotate-180" : ""}`}
        >
          ▼
        </span>
      </button>
...
export default function ItemsPage() {
  ...
  const [selectedCategory, setSelectedCategory] = useState<ItemCategory | "all">(() => {
    ...
    if (
      categoryParam &&
      (categoryParam === "all" || Object.keys(categoryDisplayNames).includes(categoryParam))
    ) {
        ...
        <div className="min-h-[calc(100vh-8rem)] px-6 py-10 max-w-5xl mx-auto">
          ...
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-6 text-amber-100">
            ...
            <p className="text-sm text-amber-100/90">
              ...
              to rebuild{" "}
              <code className="px-1 py-0.5 bg-black/40 rounded text-amber-200">items.ts</code> and
              its category files before viewing this page.
            </p>
          </div>
        </div>
      </ErrorBoundary>
      ...
      <div className="min-h-[calc(100vh-8rem)] px-6 py-10 max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-300 mb-6 text-lg leading-relaxed">
            Comprehensive catalog of all items available in Island Troll Tribes. From basic
            materials to powerful artifacts, weapons, and buildings.
          </p>

          <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4 mb-6">
            <p className="text-amber-200 text-sm">
              <strong>Total Items:</strong> {ITEMS_DATA.length} •<strong> Categories:</strong>{" "}
              {Object.keys(categoryDisplayNames).length} •<strong> Craftable Items:</strong>{" "}
              {ITEMS_DATA.filter((item) => item.recipe).length}
            </p>
          </div>
        </div>
        ...
        {searchQuery.trim() && (
          <div className="mb-6">
            <p className="text-gray-300">
              Found <span className="text-amber-400 font-semibold">{filteredItems.length}</span>{" "}
