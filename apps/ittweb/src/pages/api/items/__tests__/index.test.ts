...
describe("GET /api/items", () => {
  ...
  it("filters items by category", async () => {
    ...
    expect(
      responseData.items.every((item: { category: string }) => item.category === "weapons")
    ).toBe(true);
  ...
  it("filters items by search query", async () => {
    ...
    if (responseData.items.length > 0) {
      expect(
        responseData.items.some((item: { name: string }) =>
  ...
  it("filters items by category and search query together", async () => {
    ...
    if (responseData.items.length > 0) {
      expect(
        responseData.items.every((item: { category: string }) => item.category === "armor")
      ).toBe(true);
    }
