...
let mockGetAllEntries: jest.Mock;
...
jest.mock("@/features/modules/game-management/entries/lib/entryService", () => ({
  getAllEntries: jest.fn((...args: unknown[]) => {
    // Access mock via closure - this will be evaluated when the function is called
    return mockGetAllEntries(...args);
  }),
  createEntry: jest.fn((...args: unknown[]) => {
    // Access mock via closure - this will be evaluated when the function is called
    return mockCreateEntry(...args);
  }),
}));
