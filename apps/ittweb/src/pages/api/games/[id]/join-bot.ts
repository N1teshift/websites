...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!gameId || !discordId || !displayName) {
      return res
        .status(400)
        .json({ error: "Missing required fields: gameId, discordId, displayName" });
    }
    ...
    if (!user) {
      ...
      return res
        .status(404)
        .json({ error: "User not found. Please visit the website first to create your account." });
    }
    ...
    logger.error("Bot join game failed", err, {
      gameId: req.query.id,
