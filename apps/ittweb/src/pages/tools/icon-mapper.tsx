...
} from "@/features/modules/tools-group/tools/utils/icon-mapper.utils";
...
export default function IconMapper() {
  ...
  const filteredIcons = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter((icon) => {
        ...
        const mappingsMatch = getAllMappingsForIcon(icon.filename).some(
          (m) =>
            m.gameName.toLowerCase().includes(query) || m.category.toLowerCase().includes(query)
        );
    ...
    if (showMappedOnly) {
      filtered = filtered.filter((icon) => getAllMappingsForIcon(icon.filename).length > 0);
    }
    ...
    <ErrorBoundary>
      <div className="min-h-[calc(100vh-8rem)] px-6 py-10 max-w-7xl mx-auto">
        ...
        <p className="text-gray-300 mb-8">
          Map icon filenames to their game names. Enter the name as it appears in the game next to
          each icon.
        </p>
        ...
        <div className="mb-6 space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              title={
                markedForDeletion.size > 0
                  ? `Export mappings and ${markedForDeletion.size} marked icons for deletion`
                  : "No icons marked for deletion"
              }
            >
