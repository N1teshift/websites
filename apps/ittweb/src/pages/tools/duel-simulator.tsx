...
export default function DuelSimulator() {
  ...
  const placeItem = (side: TrollSide, index: number, item: ItemData | null) => {
    if (side === "left") {
      setLeft((prev) => {
  ...
  const swapItems = (aSide: TrollSide, aIndex: number, bSide: TrollSide, bIndex: number) => {
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-[calc(100vh-8rem)] px-4 md:px-8 py-8 md:py-10">
        <h1 className="font-medieval-brand text-2xl md:text-4xl mb-6 text-center">
          Duel Simulator
        </h1>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_minmax(320px,420px)_1fr] gap-6 items-start">
          ...
          <div className="order-4 xl:order-2">
            <div className="mb-2 text-center text-xs text-gray-400 min-h-[20px]">
              {itemsError && (
                <span className="text-red-300">Failed to load items: {itemsError.message}</span>
              )}
        ...
        <div className="mt-6 text-center text-gray-400 text-sm">
          This is the first version of the two-troll setup with WC3-style 8-slot inventories. Damage
          math to follow.
        </div>
      </div>
    </ErrorBoundary>
