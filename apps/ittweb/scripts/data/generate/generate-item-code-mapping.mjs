...
const REPLAY_ITEM_MAPPING_FILE = path.join(ITEMS_TS_DIR, 'replayItemMapping.ts');
...
async function main() {
  ...
  let existingContent = '';
  if (fs.existsSync(REPLAY_ITEM_MAPPING_FILE)) {
    existingContent = fs.readFileSync(REPLAY_ITEM_MAPPING_FILE, 'utf-8');
  } else {
...
`;
    console.log('  Created replayItemMapping.ts template file');
  }
  ...
  if (existingContent.includes(mappingStart)) {
    ...
    console.error(`   File exists: ${fs.existsSync(REPLAY_ITEM_MAPPING_FILE)}`);
    console.error(`   File path: ${REPLAY_ITEM_MAPPING_FILE}`);
    process.exit(1);
  }

  // Write the updated file
  fs.writeFileSync(REPLAY_ITEM_MAPPING_FILE, newContent, 'utf-8');
  
  console.log(`\n✅ Updated item code mapping in: ${path.relative(ROOT_DIR, REPLAY_ITEM_MAPPING_FILE)}`);
