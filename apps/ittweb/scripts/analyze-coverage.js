...
function analyzeCoverage(fileCoverage) {
  ...
  for (const [file, cov] of files) {
    ...
    const stmtPct =
      cov.statements.total > 0 ? (cov.statements.covered / cov.statements.total) * 100 : 0;
    const branchPct =
      cov.branches.total > 0 ? (cov.branches.covered / cov.branches.total) * 100 : 0;
    const funcPct =
      cov.functions.total > 0 ? (cov.functions.covered / cov.functions.total) * 100 : 0;
  ...
  if (analysis.summary.averageCoverage.statements < THRESHOLDS.statements) {
    analysis.recommendations.push(
      `Statements coverage (${analysis.summary.averageCoverage.statements.toFixed(1)}%) is below threshold (${THRESHOLDS.statements}%)`
    );
  }
  if (analysis.summary.averageCoverage.branches < THRESHOLDS.branches) {
    analysis.recommendations.push(
      `Branches coverage (${analysis.summary.averageCoverage.branches.toFixed(1)}%) is below threshold (${THRESHOLDS.branches}%)`
    );
  }
  if (analysis.summary.averageCoverage.functions < THRESHOLDS.functions) {
    analysis.recommendations.push(
      `Functions coverage (${analysis.summary.averageCoverage.functions.toFixed(1)}%) is below threshold (${THRESHOLDS.functions}%)`
    );
  }

  if (analysis.summary.filesWithLowCoverage.length > 10) {
    analysis.recommendations.push(
      `${analysis.summary.filesWithLowCoverage.length} files have low coverage - prioritize testing these files`
    );
  }
...
function main() {
  ...
  try {
    ...
    if (avgStatements < THRESHOLDS.statements - 10) {
      // Allow some tolerance
      console.error(
        `❌ Coverage too low: ${avgStatements.toFixed(1)}% < ${THRESHOLDS.statements}%`
      );
      process.exit(1);
    }
  } catch (error) {
